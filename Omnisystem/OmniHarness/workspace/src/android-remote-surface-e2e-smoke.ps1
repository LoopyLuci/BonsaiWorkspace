...
function Invoke-AdbWithTimeout(
  ...
  $stdoutPath = Join-Path $env:TEMP ("workspace-adb-stdout-" + [Guid]::NewGuid().ToString("N") + ".log")
  $stderrPath = Join-Path $env:TEMP ("workspace-adb-stderr-" + [Guid]::NewGuid().ToString("N") + ".log")
...
function Start-MainAndWait([string]$adbPath, [string]$serial) {
  $args = @("-s", $serial, "shell", "cmd", "activity", "start-activity", "-n", "com.workspace.workspace/.MainActivity")
  return (& $adbPath @args) | Out-String
}
...
if (-not $SkipForceStop) {
  & $adb -s $serial shell am force-stop com.workspace.workspace | Out-Null
}
...
try {
  for ($attempt = 1; $attempt -le 3; $attempt++) {
    ...
    $phase2Start = Start-And-Wait -adbPath $adb -serial $serial -component "com.workspace.workspace/.RemoteSurfaceEntryActivity" -desktopHost $DesktopHost -port $DesktopPort
