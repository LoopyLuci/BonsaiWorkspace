...
const TAURI_DIR = path.join(ROOT_DIR, 'workspace', 'src-tauri');
...
const PID_FILE = path.join(ROOT_DIR, '.workspace-launcher.pid');

function readConfiguredApiPort() {
  const envPort = Number(process.env.WORKSPACE_API_PORT);
...
function printHelp() {
  log('Workspace Ecosystem Orchestrator');
  log('');
  log('Usage: node ./orchestrate-workspace-ecosystem.mjs [options]');
  log('');
  log('Options:');
...
function isReclaimableWorkspaceProcess(imageName) {
  const n = String(imageName || '').toLowerCase();
  return n === 'workspace.exe' || n === 'workspace';
}
...
function isReclaimableWorkspaceViteProcess(imageName, commandLine) {
  const img = String(imageName || '').toLowerCase();
  if (img !== 'node.exe' && img !== 'node') return false;
  const cmd = String(commandLine || '').toLowerCase();
  return cmd.includes('vite') && cmd.includes('workspaceworkspace') && cmd.includes('workspace\\src');
}
...
function tryReleasePortFromStaleVite(port) {
  ...
  for (const pid of pids) {
    ...
    if (!isReclaimableWorkspaceViteProcess(image, cmd)) {
      continue;
    }
...
function tryReleasePortFromStaleWorkspace(port) {
  if (process.platform !== 'win32') return false;
  const pids = getListeningPidsOnPort(port);
  if (pids.length === 0) return false;
  ...
  for (const pid of pids) {
    const image = getProcessImageName(pid);
    if (!isReclaimableWorkspaceProcess(image)) {
      continue;
    }
    log(`[preflight] reclaiming stale Workspace listener on port ${port} (PID ${pid}, ${image})...`);
    try {
      const kill = spawnSync('taskkill', ['/PID', String(pid), '/T', '/F'], { encoding: 'utf8' });
      if (kill.status === 0) {
        killedAny = true;
...
function spawnTauriDev() {
  ...
  return spawn('cargo', ['tauri', 'dev'], {
    ...
    env: {
      ...process.env,
      WORKSPACE_DEV_ALL_NO_BACKEND: '1',
...
function tryReleaseDesktopBinaryLock() {
  if (process.platform !== 'win32') return false;
  try {
    const list = spawnSync('tasklist', ['/FI', 'IMAGENAME eq workspace.exe'], { encoding: 'utf8' });
    const text = `${list.stdout || ''}${list.stderr || ''}`;
    if (/No tasks are running/i.test(text) || !/workspace\.exe/i.test(text)) {
      return false;
    }

    log('[preflight] reclaiming stale workspace.exe process lock...');
    const kill = spawnSync('taskkill', ['/IM', 'workspace.exe', '/T', '/F'], { encoding: 'utf8' });
...
function runUsbRegression(cfg) {
  ...
  if (cfg.strictApp) env.WORKSPACE_REQUIRE_APP = '1';
...
function acquirePidLock() {
  if (fs.existsSync(PID_FILE)) {
    ...
    if (Number.isFinite(existing) && existing > 0 && isProcessAlive(existing) && existing !== process.pid) {
      log(`[launcher] Workspace is already running (PID ${existing}). Exiting.`);
      process.exit(0);
    }
  }
...
async function run() {
  ...
  try {
    ...
    if (!apiFree && !report.api_healthy && !cfg.allowPortInUse && !cfg.preflightOnly) {
      ...
      const reclaimed = tryReleasePortFromStaleWorkspace(cfg.apiPort);
      if (reclaimed) {
        const releasedAfterKill = await waitForPortToBecomeAvailable(cfg.apiPort, 8000);
        if (releasedAfterKill) {
          apiFree = true;
          log(`[preflight] port ${cfg.apiPort} reclaimed from stale Workspace process.`);
        }
      }
    }
    ...
    if (!report.attached_existing_runtime) {
      // A stale desktop process can lock target/debug/workspace.exe and
    ...
    // Spawn workspace-bot sidecar if the release binary exists and nothing is already listening.
    // If port 11421 already responds to /health, assume a prior bot instance is running and skip.
    let botProc = null;
    {
      const botBin = path.join(ROOT_DIR, 'workspace-bot', 'target', 'release',
        'workspace-bot' + (process.platform === 'win32' ? '.exe' : ''));
      if (!fs.existsSync(botBin)) {
        log('[bot] workspace-bot binary not found — messaging bots disabled');
      } else {
        ...
        if (botAlreadyRunning) {
          log('[bot] workspace-bot already running on port 11421 — skipping spawn');
        } else {
          ...
          log(`[bot] started workspace-bot (pid ${botProc.pid})`);
        }
      }
    }
