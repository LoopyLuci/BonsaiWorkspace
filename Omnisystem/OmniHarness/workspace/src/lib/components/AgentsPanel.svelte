<script lang="ts">
  ...
  import SwarmTemplatesPanel from '$lib/components/SwarmTemplatesPanel.svelte';
  import SwarmCommanderPanel from '$lib/components/SwarmCommanderPanel.svelte';
  import { kernelStatus, refreshKernelStatus } from '$lib/stores/kernelBridge';
  import { orchestratorModels, refreshOrchestratorModels } from '$lib/stores/orchestratorBridge';

  let kernelPollHandle: ReturnType<typeof setInterval> | undefined;

  const dispatch = createEventDispatcher<{ close: void }>();

  let activeTab: 'agents' | 'personas' | 'templates' | 'commander' | 'settings' | 'about' = 'agents';
  ...
  async function toggleAgent(resolved: ResolvedAgent) {
    const previous = resolved.config.enabled;
    resolved.config.enabled = !previous;
    try {
      await upsertAgentConfig(resolved.config);
      agentError = '';
    } catch (e) {
      // Roll back the optimistic local flip so the toggle UI doesn't show
      // "on" when the backend never actually saved that state.
      resolved.config.enabled = previous;
      agentError = String(e);
    }
  }
  ...
  onMount(async () => {
    ...
    kernelPollHandle = setInterval(refreshKernelStatus, 15000);
    await refreshOrchestratorModels();
  });

  onDestroy(() => {
    if (kernelPollHandle) clearInterval(kernelPollHandle);
  });
...
<div class="agents-overlay" on:click|self={() => dispatch('close')} role="presentation">
  <div class="agents-panel" role="dialog" aria-modal="true" aria-label="Agents">
    ...
    <nav class="tab-bar">
      ...
      <button class="tab-btn" class:active={activeTab === 'templates'} on:click={() => activeTab = 'templates'}>Templates</button>
      <button class="tab-btn" class:active={activeTab === 'commander'} on:click={() => activeTab = 'commander'}>Commander</button>
      ...
      {:else if activeTab === 'personas'}
        ...
        {#if showPersonaForm}
          <div class="persona-form">
            ...
            <div class="form-row">
              <input class="emoji-input" type="text" maxlength="2" data-workspace-action="Agents:EditPersonaEmoji" style="-webkit-app-region: no-drag;" bind:value={editingPersona.icon_emoji} placeholder="🤖" />
              <input class="color-swatch" type="color" data-workspace-action="Agents:EditPersonaColor" style="-webkit-app-region: no-drag;" bind:value={editingPersona.color} />
              <input class="label-input" type="text" data-workspace-action="Agents:EditPersonaName" style="-webkit-app-region: no-drag;" bind:value={editingPersona.name} placeholder="Persona name" />
            </div>
            <select class="model-select full-width" data-workspace-action="Agents:EditPersonaModel" style="-webkit-app-region: no-drag;" bind:value={editingPersona.model_id}>
              <option value={null}>Default model</option>
              {#each $availableModels as m}
                <option value={m.id}>{m.name} ({m.ram_label})</option>
              {/each}
            ...
            <textarea
              ...
              data-workspace-action="Agents:EditPersonaPrompt"
      ...
      {:else if activeTab === 'settings'}
        ...
        <div class="settings-grid">
          <label class="setting-field" title={settingHelp('chain_strategy')}>
            ...
            <select
              data-workspace-action="Agents:ChainStrategy"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-row" title={settingHelp('stop_on_first_satisfactory')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:StopOnFirstSatisfactory"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-field" title={settingHelp('satisfaction_threshold')}>
            ...
            <input
              type="number"
              data-workspace-action="Agents:SatisfactionThreshold"
              style="-webkit-app-region: no-drag;"
              min="0"
              max="100"
          ...
          <label class="setting-field" title={settingHelp('preferred_primary_slot')}>
            ...
            <input
              type="number"
              data-workspace-action="Agents:PreferredPrimarySlot"
              style="-webkit-app-region: no-drag;"
              min="1"
              max="24"
          ...
          <label class="setting-row" title={settingHelp('force_all_workers_before_decision')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:ForceAllWorkersBeforeDecision"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-field" title={settingHelp('heavy_work_delegate_mode')}>
            ...
            <select
              data-workspace-action="Agents:HeavyWorkDelegateMode"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-field" title={settingHelp('configured_heavy_worker_slot')}>
            ...
            <input
              type="number"
              data-workspace-action="Agents:ConfiguredHeavyWorkerSlot"
              style="-webkit-app-region: no-drag;"
              min="1"
              max="24"
          ...
          <label class="setting-row" title={settingHelp('heavy_work_delegate_auto_fallback')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:HeavyWorkDelegateAutoFallback"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-row" title={settingHelp('auto_repair_delegate_routing')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:AutoRepairDelegateRouting"
              style="-webkit-app-region: no-drag;"
          ...
          {#each $swarmRuntimeSettings.agent_chain_policies as policy (policy.slot_index)}
            <div class="policy-card">
              ...
              <label class="setting-field" title={settingHelp('execution_tier')}>
                ...
                <input
                  type="number"
                  data-workspace-action="Agents:PolicyExecutionTier"
                  style="-webkit-app-region: no-drag;"
                  min="0"
                  max="32"
              ...
              <label class="setting-field" title={settingHelp('response_weight')}>
                ...
                <input
                  type="number"
                  data-workspace-action="Agents:PolicyResponseWeight"
                  style="-webkit-app-region: no-drag;"
                  min="0"
                  max="10"
              ...
              <label class="setting-field" title={settingHelp('early_exit_confidence_threshold')}>
                ...
                <input
                  type="number"
                  data-workspace-action="Agents:PolicyEarlyExitConfidenceThreshold"
                  style="-webkit-app-region: no-drag;"
                  min="0"
                  max="100"
              ...
              <label class="setting-row" title={settingHelp('always_run')}>
                ...
                <input
                  type="checkbox"
                  data-workspace-action="Agents:PolicyAlwaysRun"
                  style="-webkit-app-region: no-drag;"
              ...
              <label class="setting-row" title={settingHelp('can_be_early_exit_gate')}>
                ...
                <input
                  type="checkbox"
                  data-workspace-action="Agents:PolicyCanBeEarlyExitGate"
                  style="-webkit-app-region: no-drag;"
              ...
              <label class="setting-row" title={settingHelp('allow_heavy_work')}>
                ...
                <input
                  type="checkbox"
                  data-workspace-action="Agents:PolicyAllowHeavyWork"
                  style="-webkit-app-region: no-drag;"
              ...
              <div class="setting-field compact-field" title={settingHelp('can_review_from_slots')}>
                ...
                <div class="slot-presets" role="group" aria-label="Review slot presets">
                  <button type="button" data-workspace-action="Agents:PolicyReviewPresetAll" style="-webkit-app-region: no-drag;" on:click={() => applySlotPreset(policy.slot_index, 'can_review_from_slots', 'all')}>All</button>
                  <button type="button" data-workspace-action="Agents:PolicyReviewPresetCore" style="-webkit-app-region: no-drag;" on:click={() => applySlotPreset(policy.slot_index, 'can_review_from_slots', 'core')}>Core</button>
                  <button type="button" data-workspace-action="Agents:PolicyReviewPresetNone" style="-webkit-app-region: no-drag;" on:click={() => applySlotPreset(policy.slot_index, 'can_review_from_slots', 'none')}>None</button>
                </div>
                <div class="slot-chip-grid">
                  {#each availablePolicySlots as slot (slot)}
                    <button
                      type="button"
                      data-workspace-action="Agents:ToggleReviewSlot"
                      style="-webkit-app-region: no-drag;"
                      class="slot-chip"
              ...
              <div class="setting-field compact-field" title={settingHelp('can_delegate_to_slots')}>
                ...
                <div class="slot-presets" role="group" aria-label="Delegation slot presets">
                  <button type="button" data-workspace-action="Agents:PolicyDelegatePresetAll" style="-webkit-app-region: no-drag;" on:click={() => applySlotPreset(policy.slot_index, 'can_delegate_to_slots', 'all')}>All</button>
                  <button type="button" data-workspace-action="Agents:PolicyDelegatePresetCore" style="-webkit-app-region: no-drag;" on:click={() => applySlotPreset(policy.slot_index, 'can_delegate_to_slots', 'core')}>Core</button>
                  <button type="button" data-workspace-action="Agents:PolicyDelegatePresetNone" style="-webkit-app-region: no-drag;" on:click={() => applySlotPreset(policy.slot_index, 'can_delegate_to_slots', 'none')}>None</button>
                </div>
                <div class="slot-chip-grid">
                  {#each availablePolicySlots as slot (slot)}
                    ...
                    <button
                      type="button"
                      data-workspace-action="Agents:ToggleDelegateSlot"
                      style="-webkit-app-region: no-drag;"
                      class="slot-chip"
        ...
        <div class="settings-grid">
          <label class="setting-row" title={settingHelp('leader_plan_required')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:LeaderPlanRequired"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-row" title={settingHelp('allow_worker_tools')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:AllowWorkerTools"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-row" title={settingHelp('enable_worker_cross_review')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:EnableWorkerCrossReview"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-row" title={settingHelp('parallel_workers')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:ParallelWorkers"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-row" title={settingHelp('include_worker_summaries')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:IncludeWorkerSummaries"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-row" title={settingHelp('retry_failed_workers')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:RetryFailedWorkers"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-row" title={settingHelp('stream_worker_tokens')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:StreamWorkerTokens"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-row" title={settingHelp('emit_debug_events')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:EmitDebugEvents"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-row" title={settingHelp('include_original_prompt_in_worker_context')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:IncludeOriginalPrompt"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-row" title={settingHelp('allow_leader_as_worker')}>
            ...
            <input
              type="checkbox"
              data-workspace-action="Agents:AllowLeaderAsWorker"
              style="-webkit-app-region: no-drag;"
          ...
          <label class="setting-field" title={settingHelp('max_worker_subtasks')}>
            ...
            <input
              type="number"
              data-workspace-action="Agents:MaxWorkerSubtasks"
              style="-webkit-app-region: no-drag;"
              min="1"
              max="24"
          ...
          <label class="setting-field" title={settingHelp('worker_timeout_ms')}>
            ...
            <input
              type="number"
              data-workspace-action="Agents:WorkerTimeoutMs"
              style="-webkit-app-region: no-drag;"
              min="5000"
              max="600000"
              step="1000"
          ...
          <label class="setting-field" title={settingHelp('max_worker_response_chars')}>
            ...
            <input
              type="number"
              data-workspace-action="Agents:MaxWorkerResponseChars"
              style="-webkit-app-region: no-drag;"
              min="400"
              max="50000"
              step="100"
          ...
          <label class="setting-field" title={settingHelp('synthesis_style')}>
            ...
            <select
              data-workspace-action="Agents:SynthesisStyle"
              style="-webkit-app-region: no-drag;"
        ...
        <div class="settings-actions">
          <button class="add-worker-btn" data-workspace-action="Agents:ResetDefaults" style="-webkit-app-region: no-drag;" on:click={() => resetSwarmRuntimeSettings()}>Reset Defaults</button>
        ...
        <div class="about-body">
          ...
          <div class="about-settings-grid">
            <article class="about-setting-card">
              <h5>OmniHarness Kernel {$kernelStatus.connected ? '🟢 Connected' : '⚪ Not running'}</h5>
              {#if $kernelStatus.connected}
                <p><strong>What it does:</strong> The separate Rust kernel process (OmniHarness/kernel, port 50051) is the cross-language trust anchor shared with the Python and Clojure orchestrators — a hash-chained audit log and model registry.</p>
                <p><strong>Operational detail:</strong> version {$kernelStatus.version}, up {$kernelStatus.uptime_secs}s, {$kernelStatus.events_stored} events chained (tip {$kernelStatus.tip_hash?.slice(0, 12)}…). Every tool decision logged here is mirrored into its chain.</p>
              {:else}
                <p><strong>What it does:</strong> Optional — Workspace runs fully standalone without it. When started (see OmniHarness/start.ps1 or run the kernel binary directly), this audit trail and model registry become visible to the Python/Clojure orchestrators too.</p>
                <p><strong>Operational detail:</strong> Not currently reachable at 127.0.0.1:50051. Rechecked automatically every 15s.</p>
              {/if}
            </article>
            <article class="about-setting-card">
              <h5>Orchestrator Cloud Models {$orchestratorModels.length > 0 ? `🟢 ${$orchestratorModels.length} available` : '⚪ None visible'}</h5>
              {#if $orchestratorModels.length > 0}
                <p><strong>What it does:</strong> The Python OmniHarness orchestrator (the same service the VS Code extension talks to) has these provider-backed cloud models configured. Workspace's own local model routing is unaffected — this is visibility only.</p>
                <p><strong>Operational detail:</strong> {$orchestratorModels.map((m) => `${m.provider}/${m.id}`).join(', ')}</p>
              {:else}
                <p><strong>What it does:</strong> Optional — when the Python orchestrator is running (default http://127.0.0.1:8080) and has cloud providers (Anthropic/OpenAI/etc.) configured, their models appear here for visibility.</p>
                <p><strong>Operational detail:</strong> None currently visible — orchestrator not reachable or has no providers configured.</p>
              {/if}
            </article>
          </div>
