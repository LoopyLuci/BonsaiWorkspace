<script lang="ts">
  ...
  import { requestAskWorkspace, type AskWorkspaceAction } from '$lib/stores/chat';
  ...
  function renderDiff(hunks: DiffHunk[], rawDiff: string) {
    ...
    hunks.forEach((hunk) => {
      ...
      const widget: monaco.editor.IContentWidget = {
        getId:       () => `workspace-hunk-${hunk.hunkIndex}`,
  ...
  function buildAskPrompt(action: AskWorkspaceAction, selectedText: string): string {
  ...
  function askWorkspaceFromSelection(action: AskWorkspaceAction) {
    if (!currentFilePath) {
    ...
    if (!selectedText.trim()) {
      addToast('Select code first to Ask Workspace.', 'info');
      return;
    }
  ...
  function setupAskWorkspaceActions() {
    editor.addAction({
      id: 'workspace.ask.explain',
      label: 'Ask Workspace: Explain Selection',
      precondition: 'editorHasSelection',
      contextMenuGroupId: 'navigation',
      contextMenuOrder: 1.1,
      run: () => askWorkspaceFromSelection('explain'),
    });

    editor.addAction({
      id: 'workspace.ask.fix',
      label: 'Ask Workspace: Fix Selection',
      precondition: 'editorHasSelection',
      contextMenuGroupId: 'navigation',
      contextMenuOrder: 1.2,
      run: () => askWorkspaceFromSelection('fix'),
    });

    editor.addAction({
      id: 'workspace.ask.refactor',
      label: 'Ask Workspace: Refactor Selection',
      precondition: 'editorHasSelection',
      contextMenuGroupId: 'navigation',
      contextMenuOrder: 1.3,
      run: () => askWorkspaceFromSelection('refactor'),
    });
  }

  function setupToolingContextActions() {
    const actions: Array<{ kind: ToolingCommandKind; label: string; order: number }> = [
      { kind: 'load', label: 'Workspace: Load Tools For File', order: 2.1 },
      { kind: 'lint', label: 'Workspace: Lint Current File', order: 2.2 },
      { kind: 'format', label: 'Workspace: Format Current File', order: 2.3 },
      { kind: 'test', label: 'Workspace: Run Tests For Profile', order: 2.4 },
    ];

    for (const action of actions) {
      editor.addAction({
        id: `workspace.tooling.${action.kind}`,
        label: action.label,
        contextMenuGroupId: '2_workspace',
...
<div class="editor-host">
  ...
  {#if !currentFilePath}
    <div class="empty-state">
      ...
      <div class="empty-title">Workspace</div>
