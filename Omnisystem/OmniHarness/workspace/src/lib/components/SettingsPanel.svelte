<script lang="ts">
  ...
  import {
    WORKSPACE_CATALOG, findRegistryModel,
  ...
  let crashReportsLoading = false;
  let crashReportsExpanded: Set<number> = new Set();
  ...
  async function clearCrashReports() {
    try {
      ...
      crashReports = [];
      crashReportsExpanded = new Set();
    } catch (e) {
      console.warn('Failed to clear crash reports', e);
    }
  }

  function toggleCrashReport(i: number) {
    const next = new Set(crashReportsExpanded);
    if (next.has(i)) next.delete(i); else next.add(i);
    crashReportsExpanded = next;
  }
  ...
  async function startTrainingMonitor(): Promise<void> {
    ...
    try {
      const adapterPath = await invoke<string>('start_training_cycle', {
        ...
        dataPath: 'data/workspace_core/workspace_core_train_v2.jsonl',
        outputPath: null,
      });
  ...
  async function handleDownload(entry: typeof WORKSPACE_CATALOG[number]) {
  ...
  let usbApkPath = '';
  let usbPackageName = 'com.workspace.workspace';
  let usbActivity = '';
  let usbWifiHost = '';
  ...
  function parseWorkspaceConnectUrl(scanned: string): { ip: string; token: string } {
    let parsed: URL;
    try {
  ...
  async function scanMobilePairQr() {
    ...
    try {
      ...
      const connection = parseWorkspaceConnectUrl(scanned);
  ...
  let emailSmtpFrom = '';
  let emailSubjectPrefix = '[WORKSPACE]';
...
<div class="settings-overlay" on:click|self={() => dispatch('close')} role="presentation">
  <div class="settings-panel" role="dialog" aria-modal="true" aria-label="Settings">
    ...
    <section class="section crash-reports-section">
      <h3 class="section-title">Crash Reports</h3>
      <p class="section-desc">
        Backend panics and frontend errors are persisted here across restarts —
        the last {crashReports.length} report{crashReports.length === 1 ? '' : 's'}, most recent first.
      </p>
      <div class="crash-reports-actions">
        <button class="btn-small" on:click={loadCrashReports} disabled={crashReportsLoading}>
          {crashReportsLoading ? 'Loading…' : 'Refresh'}
        </button>
        {#if crashReports.length > 0}
          <button class="btn-small btn-warning" on:click={clearCrashReports}>Clear all</button>
        {/if}
      </div>
      {#if crashReports.length === 0}
        <p class="crash-reports-empty">No crash reports recorded.</p>
      {:else}
        <div class="crash-reports-list">
          {#each crashReports as report, i}
            <div class="crash-report-row">
              <button class="crash-report-summary" on:click={() => toggleCrashReport(i)}>
                <span class="crash-report-kind">{report.kind}</span>
                <span class="crash-report-time">{new Date(report.timestamp).toLocaleString()}</span>
                <span class="crash-report-message">{report.message}</span>
              </button>
              {#if crashReportsExpanded.has(i)}
                <div class="crash-report-detail">
                  {#if report.location}<div><strong>Location:</strong> {report.location}</div>{/if}
                  {#if report.backtrace}<pre class="crash-report-backtrace">{report.backtrace}</pre>{/if}
                  {#if report.details}<pre class="crash-report-backtrace">{JSON.stringify(report.details, null, 2)}</pre>{/if}
                </div>
              {/if}
            </div>
          {/each}
        ...
        <div class="slot-list">
          {#each $orchestratorStatus.slots as slot}
            <div class="slot-row">
              ...
              {#if slot.fallback_note}
                <span class="slot-fallback-note" title={slot.fallback_note}>⚠ {slot.fallback_note}</span>
              {/if}
            </div>
          {/each}
        </div>
    ...
    <section class="section api-settings">
      ...
      <div class="form-group">
        ...
        <input id="api-host" type="text" data-workspace-action="Settings:ApiHost" style="-webkit-app-region: no-drag;" bind:value={$apiHost} />
      </div>
      <div class="form-group">
        <label for="api-port">API Port</label>
        <input id="api-port" type="number" min="1" max="65535" data-workspace-action="Settings:ApiPort" style="-webkit-app-region: no-drag;" bind:value={$apiPort} />
      </div>
      <div class="form-note">
    ...
    <section class="section">
      ...
      <div class="model-list">
        {#each WORKSPACE_CATALOG as entry (entry.catalogId)}
        ...
        {#each $availableModels.filter(m => !WORKSPACE_CATALOG.some(e => findRegistryModel(e, $availableModels)?.id === m.id)) as model (model.id)}
    ...
    <section class="section connection-section">
      ...
      <p class="section-desc">
        Scan the QR code with the Workspace Android app, or paste the token into
        the <strong>Workspace Runner</strong> VSCode extension settings.
      </p>
      <div class="pair-row">
        <div class="qr-area">
          {#if pairQrSvg}
    ...
    <section class="section bots-section">
      ...
      <p class="section-desc">Connect Workspace to Discord, Telegram, and Email. Tokens are stored in the OS keychain.</p>
      ...
      <details class="bot-platform-details">
        ...
        <div class="bot-form">
          <label class="bot-label">Bot Token <span class="secret-hint">(write-only)</span>
            <input class="bot-input" type="password" data-workspace-action="Settings:DiscordToken" style="-webkit-app-region: no-drag;" placeholder="Paste new token to update" bind:value={discordToken} autocomplete="off"/>
          </label>
          <label class="bot-label">Guild IDs (comma-separated, empty = any)
            <input class="bot-input" type="text" data-workspace-action="Settings:DiscordGuildIds" style="-webkit-app-region: no-drag;" placeholder="123456789012345678, ..." bind:value={discordGuildIds}/>
          </label>
          <label class="bot-label">Channel IDs (comma-separated, empty = any)
            <input class="bot-input" type="text" data-workspace-action="Settings:DiscordChannelIds" style="-webkit-app-region: no-drag;" placeholder="optional" bind:value={discordChannelIds}/>
          </label>
          <label class="bot-label">User IDs (comma-separated, empty = any)
            <input class="bot-input" type="text" data-workspace-action="Settings:DiscordUserIds" style="-webkit-app-region: no-drag;" placeholder="optional" bind:value={discordUserIds}/>
          </label>
      ...
      <details class="bot-platform-details">
        ...
        <div class="bot-form">
          <label class="bot-label">Bot Token <span class="secret-hint">(write-only)</span>
            <input class="bot-input" type="password" data-workspace-action="Settings:TelegramToken" style="-webkit-app-region: no-drag;" placeholder="Paste new token to update" bind:value={telegramToken} autocomplete="off"/>
          </label>
          <label class="bot-label">Allowed Chat IDs (comma-separated)
            <input class="bot-input" type="text" data-workspace-action="Settings:TelegramChatIds" style="-webkit-app-region: no-drag;" placeholder="-100123456789, ..." bind:value={telegramChatIds}/>
          </label>
      ...
      <details class="bot-platform-details">
        ...
        <div class="bot-form">
          <label class="bot-label">IMAP Host
            <input class="bot-input" type="text" data-workspace-action="Settings:EmailImapHost" style="-webkit-app-region: no-drag;" placeholder="imap.gmail.com" bind:value={emailImapHost}/>
          </label>
          <label class="bot-label">IMAP Port
            <input class="bot-input" type="number" data-workspace-action="Settings:EmailImapPort" style="-webkit-app-region: no-drag;" placeholder="993" bind:value={emailImapPort}/>
          </label>
          <label class="bot-label">IMAP Username
            <input class="bot-input" type="text" data-workspace-action="Settings:EmailImapUsername" style="-webkit-app-region: no-drag;" placeholder="you@gmail.com" bind:value={emailImapUsername}/>
          </label>
          <label class="bot-label">IMAP Password <span class="secret-hint">(write-only)</span>
            <input class="bot-input" type="password" data-workspace-action="Settings:EmailImapPassword" style="-webkit-app-region: no-drag;" placeholder="Paste new password to update" bind:value={emailImapPassword} autocomplete="off"/>
          </label>
          <label class="bot-label">SMTP Host
            <input class="bot-input" type="text" data-workspace-action="Settings:EmailSmtpHost" style="-webkit-app-region: no-drag;" placeholder="smtp.gmail.com" bind:value={emailSmtpHost}/>
          </label>
          <label class="bot-label">SMTP Username
            <input class="bot-input" type="text" data-workspace-action="Settings:EmailSmtpUsername" style="-webkit-app-region: no-drag;" placeholder="you@gmail.com" bind:value={emailSmtpUsername}/>
          </label>
          <label class="bot-label">SMTP From
            <input class="bot-input" type="text" data-workspace-action="Settings:EmailSmtpFrom" style="-webkit-app-region: no-drag;" placeholder="Workspace &lt;you@gmail.com&gt;" bind:value={emailSmtpFrom}/>
          </label>
          <label class="bot-label">SMTP Password <span class="secret-hint">(write-only)</span>
            <input class="bot-input" type="password" data-workspace-action="Settings:EmailSmtpPassword" style="-webkit-app-region: no-drag;" placeholder="Paste new password to update" bind:value={emailSmtpPassword} autocomplete="off"/>
          </label>
          <label class="bot-label">Subject Prefix
            <input class="bot-input" type="text" data-workspace-action="Settings:EmailSubjectPrefix" style="-webkit-app-region: no-drag;" placeholder="[WORKSPACE]" bind:value={emailSubjectPrefix}/>
          </label>
          <label class="bot-label">Allowed From Addresses (comma-separated)
            <input class="bot-input" type="text" data-workspace-action="Settings:EmailAllowedFrom" style="-webkit-app-region: no-drag;" placeholder="alice@example.com, bob@example.com" bind:value={emailAllowedFrom}/>
          </label>
    ...
    <section class="section thinking-section">
      ...
      <div class="flags-grid">
        ...
        <label class="flag-row">
          <span class="flag-key">Micro-OmniAI (router) thinking</span>
    ...
    <section class="section feature-flags-section">
      ...
      {#if showAdvanced}
        ...
        <div class="training-section">
          <h4 class="flags-heading">OmniAI-Core Training Monitor</h4>
          ...
          <div class="tm-model-credit">
            ...
            Workspace supports Bonsai Models and all compatible GGUF models.
          </div>
          {#if selectedGgufPath}
    ...
    <section class="section">
      ...
      <div class="vision-tool-grid">
        ...
        <div class="vision-card">
          ...
          <code class="vision-path">~/.workspace/sidecars/opencv_worker.py</code>
        </div>
        ...
        <div class="vision-card">
          ...
          <code class="vision-path">~/.workspace/models/depth/Depth-Anything-V2-Small-F16.gguf</code>
        ...
        <div class="vision-card">
          ...
          <code class="vision-path">~/.workspace/models/yolo/yolov8n.pt</code>
        </div>
        ...
        <div class="vision-card">
          ...
          <code class="vision-path">~/.workspace/models/kokoro/kokoro-v1.9.onnx</code>
...
<style>
  ...
  .btn-small:hover { background: var(--bg-hover, rgba(255,255,255,0.06)); }
  ...
  .crash-report-summary:hover { background: rgba(255,255,255,0.04); }
  .crash-report-kind {
    font-weight: 600;
    color: #f87171;
    white-space: nowrap;
