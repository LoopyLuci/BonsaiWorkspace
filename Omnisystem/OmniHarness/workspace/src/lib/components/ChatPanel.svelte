<script lang="ts">
  ...
  const TOOL_PREFS_KEY = 'workspace-tool-prefs-v1';
  const SKILL_PREFS_KEY = 'workspace-skill-prefs-v1';
  ...
  async function sendText(text: string) {
    ...
    // "Custom Swarm" must be an explicit choice (isCustomSwarm, from the
    // model picker), not inferred from agent count ($swarmEnabled) — the
    // dropdown selection used to be purely cosmetic: whenever >=2 agents were
    // enabled, every message silently ran the swarm and overwrote whatever
    // specific model the user had actually picked, and picking "Custom
    // Swarm" with fewer than 2 agents silently fell through to ordinary
    // single-model chat with no explanation either way.
    if (isCustomSwarm && !$swarmEnabled) {
      addUserMessage(text);
      addAssistantMessage('Custom Swarm needs at least 2 enabled agents. Add or enable agents in the Agents panel, or pick a different model to send this message.');
      addToast('Custom Swarm needs at least 2 enabled agents.', 'warning');
      return;
    }

    if (!isCustomSwarm && get(activeModelId) !== WORKSPACE_AI_MODEL_ID) {
      await refreshStatus();
    ...
    try {
      ...
      streamThinking = '';
      streamResponse = '';
      // Belt-and-suspenders reset: the 'swarm-complete' event listener is the
      // normal path, but Rust's error branches for submit_swarm_chat (feature
      // disabled, RAM gate, missing leader, etc.) return Err without ever
      // emitting swarm-complete — previously that left activeSwarmRunId
      // stuck at 'pending' forever, and with it AgentsPanel's "Live Worker
      // Streams" panel stuck open until the app was reloaded.
  ...
  async function stopResponseGeneration() {
    stopRequested = true;
    stopSpeechPlayback();
    const runId = get(activeSwarmRunId);
    try {
      // A Custom Swarm run's actual work happens in
      // ModelOrchestrator/worker inference, not the single-agent generation
      // `stop_chat_generation` targets — without this, Stop appeared to do
      // nothing for swarm runs because it was never actually telling any of
      // the in-flight workers to cancel.
      if (runId && runId !== 'pending') {
        await invoke('cancel_swarm', { runId });
      } else {
  ...
  function buildVisionContext(userText: string): string | null {
    const seeded = typeof window !== 'undefined'
      ? (window as Window & {
          __WORKSPACE_TEST_VISION_CONTEXT?: {
            active?: boolean;
            snapshot?: any;
          };
        }).__WORKSPACE_TEST_VISION_CONTEXT
  ...
  onMount(async () => {
    ...
    const unlistenModelReady = await listen<{ cpu_mode?: boolean; fallback_note?: string | null }>('model-ready', ({ payload }) => {
      if (payload?.cpu_mode) {
        modelReadyCpuNotice = payload.fallback_note || 'Ready (CPU mode)';
      } else {
        modelReadyCpuNotice = '';
      }
      modelFallbackNotice = '';
    ...
    const unsubAskWorkspace = askWorkspaceRequest.subscribe(async (request) => {
      if (!request) return;

      if ($isThinking) {
        input = request.prompt;
        addToast('Queued prompt in input while current response finishes.', 'info');
        askWorkspaceRequest.set(null);
        return;
      }
    ...
    // The initial `activeSwarmRunId.set('pending')` in sendText() is a
    // placeholder — `submit_swarm_chat`'s promise doesn't resolve until the
    // whole run finishes, so this is the earliest point the UI learns the
    // real run_id needed to actually cancel an in-flight swarm run (see
    // stopResponseGeneration()).
    const unlistenSwarmStarted = await listen<{ run_id: string }>('swarm-started', (e) => {
      activeSwarmRunId.set(e.payload.run_id);
    });
...
<div class="chat-panel">
  ...
  <div class="messages" bind:this={scrollEl} on:scroll={onChatScroll} aria-live="polite" aria-label="Chat messages">
    {#if $messages.length === 0 && !$isThinking}
      <div class="empty-chat">
        ...
        <h3 class="empty-title">Ask OmniAI anything</h3>
      ...
      {#if $isThinking}
        <div class="msg-row assistant">
          ...
          <div class="msg-bubble" class:thinking={!streamResponse && !isStreaming}>
            ...
            {:else if !isStreaming}
              <TypingIndicator label="OmniAI is thinking…" />
            {/if}
          </div>
        </div>
      {/if}
  ...
  <div class="input-area">
    <textarea
      ...
      placeholder="Message Workspace… (Enter to send, Shift+Enter for newline)"
      rows={3}
      disabled={$isThinking}
