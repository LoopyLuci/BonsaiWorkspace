<script lang="ts">
  ...
  // Local editable copy
  let name = $activeProfile?.name ?? 'Workspace Buddy';
  let systemPrompt = $activeProfile?.system_prompt ?? 'You are Workspace Buddy, a helpful personal AI assistant.';
    ...
    <div class="body">
      ...
      <label>
        ...
        <button class="pick-avatar" on:click={() => showAvatarPicker = true}>
          {avatarId ? 'Custom avatar selected' : '🌿 Default (Workspace Buddy)'} ›
        </button>
      </label>
