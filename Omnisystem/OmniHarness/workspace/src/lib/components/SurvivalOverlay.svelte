<script lang="ts">
   ...
   *   1. "Ask OmniAI" — routes to the local model for an AI-generated fix.
  ...
  onMount(async () => {
    ...
    if (description) {
      try {
        ...
        if (fixed) {
          addToast('Workspace auto-repaired the issue. Please retry your action.', 'success', 5000);
          onDismiss();
          return;
        }
  ...
  async function recordManual() {
    ...
    try {
      ...
      addToast('Fix recorded — OmniAI will learn from this.', 'success', 4000);
      manualFix = '';
      loadFixes();
    } catch (e) {
...
<div class="survival-overlay" role="dialog" aria-label="Workspace Survival System">
  <header class="header">
    <span class="title">⚡ Workspace Survival System</span>
    <button class="close-btn" on:click={onDismiss} aria-label="Close">✕</button>
  </header>

  <p class="subtitle">
    An issue occurred. OmniAI will try to repair it automatically.
  ...
  <section class="section">
    ...
    <button class="btn primary" on:click={askAI} disabled={aiLoading || !description.trim()}>
      {aiLoading ? 'Analysing with OmniAI…' : '🤖 Ask OmniAI to fix'}
    </button>
    {#if aiResult}
      <div class="ai-result">
        <strong>OmniAI suggests:</strong>
  ...
  <!-- ── Manual fix ────────────────────────────────────────────────────── -->
  <section class="section">
    <label class="field-label" for="manual">I fixed it manually — teach OmniAI</label>
    <input
      id="manual"
      class="input"
