<script lang="ts">
  ...
  function withAuthHeaders(): Record<string, string> {
    if (!pairToken) return {};
    return { 'x-workspace-token': pairToken };
  }
  ...
  async function launchWorkspace() {
    const serial = serialOrThrow();
    await withBusy('Launch Workspace app', () => invoke('android_mobile_launch_workspace', { serial }));
  }
    ...
    <div class="viewer-layout">
      <div class="screen-column">
        <div class="top-strip">
          <label>
            Device
            <select data-workspace-action="MobileView:SelectDevice" style="-webkit-app-region: no-drag;" bind:value={selectedSerial} disabled={busy}>
      ...
      <aside class="ops-column">
        <div class="ops-card">
          ...
          <div class="ops-buttons">
            <button class="control" type="button" on:click={launchWorkspace} disabled={busy || !selectedSerial}>Launch Workspace</button>
