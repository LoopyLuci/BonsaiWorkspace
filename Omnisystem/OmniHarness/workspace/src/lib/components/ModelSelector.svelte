<script lang="ts">
  ...
  function tierColor(tier: ModelTier | undefined): string {
    switch (tier) {
      ...
      case 'lightweight': return '#86efac';   /* green */
  ...
  async function pickModel(entry: typeof WORKSPACE_CATALOG[number]) {
  ...
  // Must match ChatPanel's own `isCustomSwarm` gate exactly (activeModelId,
  // not agent count) — otherwise this trigger/label can claim "Custom Swarm"
  // is active while a message send would actually run a completely
  // different, explicitly-selected single model (or vice versa).
  $: isCustomSwarmActive = $activeModelId === CUSTOM_SWARM_MODEL_ID;
  $: isWorkspaceAIActive    = $activeModelId === WORKSPACE_AI_MODEL_ID;
    ...
    {#if open}
      <div class="dropdown" bind:this={dropdownEl} style={dropdownStyle}
        ...
        <div class="option option-special"
          class:selected={isCustomSwarmActive} class:needs-setup={!$swarmEnabled}
          role="option" aria-selected={isCustomSwarmActive}
          ...
          <div class="opt-body">
            ...
            <span class="opt-desc">
              Multi-agent orchestration with configured leader and workers
              {#if !$swarmEnabled}<span class="opt-warning-text">— needs 2+ enabled agents (open Agents panel)</span>{/if}
            </span>
          </div>
        ...
        <!-- OmniAI — first-class integrated option -->
        <div class="option option-special workspace-ai-option"
          class:selected={isWorkspaceAIActive} role="option" aria-selected={isWorkspaceAIActive}
          tabindex="0" on:click={pickWorkspaceAI}
          on:keydown={(e) => e.key === 'Enter' && pickWorkspaceAI()}>
          <span class="opt-icon workspace-ai-icon">🌿</span>
          <div class="opt-body">
            <div class="opt-name-row">
              <span class="opt-name">OmniAI</span>
              <span class="tier-badge" style="color: #34d399">Integrated</span>
            </div>
            <span class="opt-desc">Workspace's own AI · fast, private, always available</span>
          ...
          <div class="option" class:selected={isActive} class:unavailable={!reg}
            ...
            <div class="opt-body">
              <div class="opt-name-row">
                ...
                <span class="tier-badge" style="color: #86efac">Lightweight</span>
              </div>
...
<style>
  ...
  /* Unlike .unavailable (hard-blocked, e.g. an uninstalled catalog model),
     Custom Swarm stays selectable with fewer than 2 agents — picking it in
     advance while still setting up agents is a reasonable workflow. This is
     just a visual hint, not a disabled state. */
  .option.needs-setup { opacity: 0.8; }
  .opt-warning-text { color: #f59e0b; font-weight: 500; }
  ...
  .workspace-ai-option:hover { background: rgba(52,211,153,0.1) !important; }
  .workspace-ai-option.selected { background: rgba(52,211,153,0.12) !important; }
  .workspace-ai-icon { filter: drop-shadow(0 0 4px rgba(52,211,153,0.5)); }
</style>
