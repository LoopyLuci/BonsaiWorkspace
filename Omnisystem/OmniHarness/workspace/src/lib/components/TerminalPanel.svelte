<script lang="ts">
  ...
  type LogCategory = 'tool' | 'swarm' | 'chat' | 'system' | 'terminal' | 'ui';
  ...
  const HISTORY_KEY = 'workspace-terminal-history-v1';
  const RUN_START_FILTER_KEY = 'workspace-terminal-run-start-filter-v1';
  ...
  let tabCounter = 1;
  const LOG_SETTINGS_KEY = 'workspace-terminal-activity-settings-v1';
  const LOG_DATA_KEY = 'workspace-terminal-activity-log-v1';
  ...
  function classifyGenericEvent(eventName: string): { level: LogLevel; category: LogCategory } {
    // Explicit cases first for events whose name doesn't otherwise signal severity —
    // these carry real failure/crash information that must not be buried at 'info'/'debug'.
    if (eventName === 'model-load-fallback' || eventName === 'gpu-unhealthy'
        || eventName === 'buddy-api-unavailable' || eventName === 'workspace:services-failed') {
      return { level: 'error', category: 'system' };
    }
    if (eventName === 'startup-health') {
      return { level: 'warn', category: 'system' };
    }
    if (eventName.includes('error') || eventName.includes('rejected') || eventName.includes('failed')) {
      return { level: 'error', category: 'system' };
    }
    if (eventName.includes('unhealthy') || eventName.includes('crash') || eventName.includes('unavailable')) {
      return { level: 'warn', category: 'system' };
    }
  ...
  function summarizeGenericEvent(eventName: string, payload: any): string {
    ...
    if (eventName === 'model-load-fallback') {
      return `GPU load failed for ${String(payload?.model_id ?? 'model')} (slot ${String(payload?.slot ?? '?')}, exit ${String(payload?.exit_code ?? '?')}) — ${String(payload?.message ?? 'falling back to CPU')}`;
    }
    if (eventName === 'gpu-unhealthy') {
      return `GPU health probe failed — ${String(payload?.message ?? 'no VRAM reported')}`;
    }
    if (eventName === 'buddy-api-unavailable') {
      return `Workspace Buddy API failed to start: ${String(payload ?? 'unknown error')}`;
    }
    if (eventName === 'startup-health') {
      return String(payload?.message ?? (payload?.model_ready ? 'Startup health OK' : 'Startup health check reported a problem'));
    }
    if (eventName === 'workspace:services-ready') {
      return 'All backend services ready';
    }
    if (eventName === 'workspace:services-failed') {
      return `Backend service startup failed: ${String(payload ?? 'unknown error')}`;
    }
  ...
  function saveActivityLog() {
    ...
    a.href = url;
    a.download = `workspace-activity-log-${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
  ...
  $: filteredActivityLog = activityLog.filter((entry) => {
    ...
    if (!activitySettings.showUi && entry.category === 'ui') return false;
  ...
  $: runStartIssueTotals = activityLog.reduce((acc, entry) => {
    ...
    if (!activitySettings.showUi && entry.category === 'ui') return acc;
  ...
  onMount(async () => {
    ...
    const genericEvents = [
      ...
      // Backend error/crash/health signals — previously never wired up, so the
      // Activity Log silently missed real failures (e.g. GPU load crashes,
      // service-startup failures) that only ever appeared in the Rust log file.
      'model-load-fallback',
      'gpu-unhealthy',
      'buddy-api-unavailable',
      'startup-health',
      'workspace:services-ready',
      'workspace:services-failed',
    ...
    window.addEventListener('workspace-activity', onWorkspaceActivity);
  ...
  onDestroy(() => {
    ...
    window.removeEventListener('workspace-activity', onWorkspaceActivity);
  ...
  function onWindowError(event: ErrorEvent) {
    ...
    pushLog('error', 'system', 'window.error', event.message || 'Unhandled window error', details);
  ...
  function onUnhandledRejection(event: PromiseRejectionEvent) {
    const details = { reason: String(event.reason) };
    pushLog('error', 'system', 'unhandledrejection', 'Unhandled Promise rejection', details);
    invoke('report_frontend_error', {
  ...
  // Bridge the app-wide pushActivity() channel (lib/activity.ts) into this panel's
  // visible Activity Log. Previously nothing listened for this event here, so any
  // component calling pushActivity() — UI-click instrumentation, fetch/WebSocket
  // instrumentation, or a direct call — silently never appeared on screen; it only
  // ever reached the backend memory-node store (and only for non-debug, non-ui
  // events at that), never the log the user actually looks at.
  function onWorkspaceActivity(event: Event) {
    const detail = (event as CustomEvent).detail as {
      level?: LogLevel; category?: LogCategory | 'ui'; source?: string; summary?: string; details?: unknown;
    } | undefined;
    if (!detail) return;
    pushLog(
      detail.level ?? 'info',
      (detail.category as LogCategory) ?? 'system',
      detail.source ?? 'activity',
      detail.summary ?? 'Activity event',
      detail.details,
    );
  }
  ...
  {#if activeTabId === 'activity'}
    ...
    {#if showLogSettings}
      <div class="activity-settings">
        ...
        <label><input type="checkbox" checked={activitySettings.showUi} on:change={(e) => updateActivitySetting('showUi', checked(e))} /> UI clicks</label>
      </div>
    {/if}
