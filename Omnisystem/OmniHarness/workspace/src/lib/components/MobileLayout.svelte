<script lang="ts">
  /**
   * Mobile-first bottom-tab layout for the Workspace Android app.
   *
  ...
  const TAB_STORAGE_KEY = 'workspace.mobile.activeTab.v1';
...
<style>
  .mobile-root {
    ...
    height: var(--workspace-mobile-vh, 100dvh);
    width: 100vw;
    padding-top: var(--workspace-mobile-safe-top, env(safe-area-inset-top, 0px));
    padding-left: var(--workspace-mobile-safe-left, env(safe-area-inset-left, 0px));
    padding-right: var(--workspace-mobile-safe-right, env(safe-area-inset-right, 0px));
    background: var(--bg);
    color: var(--text);
    overflow: hidden;
  }
  ...
  .tab-bar {
    ...
    padding-bottom: var(--workspace-mobile-safe-bottom, env(safe-area-inset-bottom, 0px));
  }
