...
describe('activity instrumentation', () => {
  beforeEach(() => {
    ...
    (window as any).__workspace_network_installed = false;
    (window as any).__workspace_user_actions_installed = false;
  });

  afterEach(() => {
    // cleanup listeners
    // remove all workspace-activity listeners by cloning and removing
  });

  it('pushActivity dispatches workspace-activity event', () => {
    const received: any[] = [];
    const handler = (e: any) => received.push(e.detail);
    window.addEventListener('workspace-activity', handler as EventListener);
    ...
    window.removeEventListener('workspace-activity', handler as EventListener);
  });
  ...
  it('instrumentUserActions captures clicks on elements with data-workspace-action', () => {
    const events: any[] = [];
    const handler = (e: any) => events.push(e.detail);
    window.addEventListener('workspace-activity', handler as EventListener);
    ...
    try {
      ...
      btn.setAttribute('data-workspace-action', 'Testing:Click');
  ...
  it('instrumentUserActions captures clicks when the actual target is a nested node', () => {
    ...
    try {
      instrumentUserActions();
      const btn = document.createElement('button');
      btn.setAttribute('data-workspace-action', 'Nested:Click');
      btn.textContent = '';
