...
export function pushActivity(payload: ActivityPayload) {
  try {
    ...
    const ev = new CustomEvent('workspace-activity', { detail: safePayload });
    ...
    console.warn('[workspace] pushActivity failed', e, payload);
  }
}

// Instrument common UI actions so they're captured in the Activity Log.
export function instrumentUserActions() {
  if ((window as any).__workspace_user_actions_installed) return;
  (window as any).__workspace_user_actions_installed = true;
    ...
    try {
      ...
      if (Array.isArray(path) && path.length) {
        for (const node of path) {
          ...
          try {
            const attrAction = el.getAttribute && el.getAttribute('data-workspace-action');
            const datasetAction = (el as HTMLElement).dataset?.workspaceAction;
...
export function instrumentNetwork() {
  if ((window as any).__workspace_network_installed) return;
  (window as any).__workspace_network_installed = true;
