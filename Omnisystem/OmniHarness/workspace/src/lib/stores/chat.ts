...
export type AskWorkspaceAction = 'explain' | 'fix' | 'refactor';
...
export const askWorkspaceRequest = writable<AskWorkspaceRequest | null>(null);

const SESSION_STATE_KEY = 'workspace-current-session';
...
export function requestAskWorkspace(request: AskWorkspaceRequest) {
  askWorkspaceRequest.set(request);
}
