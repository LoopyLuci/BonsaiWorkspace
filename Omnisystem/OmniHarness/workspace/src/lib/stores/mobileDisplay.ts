...
const STORAGE_KEY = 'workspace.mobileDisplay.v1';
const CONFIRMED_STORAGE_KEY = 'workspace.mobileDisplay.confirmed.v1';
...
export const mobileDisplayStyle = derived(mobileDisplaySettings, ($s) => {
  return [
    `--workspace-mobile-safe-top: calc(env(safe-area-inset-top, 0px) + ${$s.topOffsetPx}px)`,
    `--workspace-mobile-safe-bottom: calc(env(safe-area-inset-bottom, 0px) + ${$s.bottomOffsetPx}px)`,
    `--workspace-mobile-safe-left: calc(env(safe-area-inset-left, 0px) + ${$s.leftOffsetPx}px)`,
    `--workspace-mobile-safe-right: calc(env(safe-area-inset-right, 0px) + ${$s.rightOffsetPx}px)`,
  ].join('; ');
});
...
export function initMobileDisplaySettings(): () => void {
  ...
  const syncViewportHeight = () => {
    ...
    document.documentElement.style.setProperty('--workspace-mobile-vh', `${Math.round(h)}px`);
  };
