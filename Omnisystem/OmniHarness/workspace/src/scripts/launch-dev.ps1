#!/usr/bin/env pwsh
# launch-dev.ps1 — Start Vite dev server then the Workspace binary.
# Use this instead of running the binary directly — the debug binary loads
# the UI from http://127.0.0.1:1420 (Vite), not from dist/.

param(
    [string]$BinaryPath = "Z:\Projects\WorkspaceWorkspace\target\debug\workspace.exe",
    [int]$VitePort = 1420,
...
$SrcDir = Split-Path $PSScriptRoot -Parent   # workspace/src
...
Get-CimInstance Win32_Process | Where-Object {
    ...
    $_.ExecutablePath -like "*workspace.exe*"
} | ForEach-Object {
...
$cfgPath = "$env:APPDATA\com.workspace.workspace\workspace-config.json"
...
# 4. Launch the binary
Write-Host "[launch] Starting Workspace..." -ForegroundColor Cyan
...
if ($apiReady) {
    Write-Host "[launch] Backend healthy. Workspace is running." -ForegroundColor Green
} else {
