...
const APP_WINDOW_API_PORT = Number(process.env.WORKSPACE_APP_WINDOW_API_PORT || 11369);
...
function appConfigPath() {
  ...
  return resolve(appData, 'com.workspace.workspace', 'workspace-config.json');
}
...
async function freeDevPortIfNeeded() {
  ...
  spawnSync('taskkill', ['/IM', 'workspace.exe', '/F'], {
    encoding: 'utf8',
    windowsHide: true,
  });
}

log('Launching Workspace in an independent desktop app window...');
...
log(`Dedicated app-window API port: ${APP_WINDOW_API_PORT} (override with WORKSPACE_APP_WINDOW_API_PORT).`);
