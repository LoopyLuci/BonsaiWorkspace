...
const UI_BASE = process.env.WORKSPACE_UI_BASE || 'http://localhost:1420';
const STEP_TIMEOUT_MS = Number(process.env.WORKSPACE_LIVE_STEP_TIMEOUT_MS || 45000);
const SCENARIO_TIMEOUT_MS = Number(process.env.WORKSPACE_LIVE_SCENARIO_TIMEOUT_MS || 90000);
const LIVE_MODE = process.env.WORKSPACE_LIVE_MODE === '0' ? false : true;
const HEADLESS = process.env.WORKSPACE_LIVE_HEADLESS === '1' ? true : !LIVE_MODE;
const SLOW_MO_MS = Number(process.env.WORKSPACE_LIVE_SLOW_MO_MS || (LIVE_MODE ? 80 : 0));
const KEEP_OPEN_MS = Number(process.env.WORKSPACE_UI_KEEP_OPEN_MS || (LIVE_MODE ? 8000 : 0));
const SCENARIO_PAUSE_MS = Number(process.env.WORKSPACE_LIVE_SCENARIO_PAUSE_MS || (LIVE_MODE ? 1500 : 0));
const AUDIT_PROFILE = process.env.WORKSPACE_AUDIT_PROFILE === 'smoke' ? 'smoke' : 'full';
const AUDIT_RUN_AUX = process.env.WORKSPACE_AUDIT_RUN_AUX === '1';
const SCENARIO_FILTER = (process.env.WORKSPACE_SCENARIO_FILTER || '').trim().toLowerCase();
...
async function collectAuxCoverageEvidence() {
  ...
  if (!AUDIT_RUN_AUX) {
    partial('launcher.preflight', 'Launcher and preflight scripts detected; run with WORKSPACE_AUDIT_RUN_AUX=1 for evidence execution.');
    partial('mobile.usb.lab', 'USB regression script exists; run with WORKSPACE_AUDIT_RUN_AUX=1 on device-capable machine.');
    partial('vscode.extension.unit', 'Unit tests present in vscode-extension/src/test; run with WORKSPACE_AUDIT_RUN_AUX=1 for evidence execution.');
    partial('frontend.unit', 'Frontend unit tests present in src/lib stores/utils; run with WORKSPACE_AUDIT_RUN_AUX=1 for evidence execution.');
    return;
  }
...
function buildMockInitScript() {
  return (config = {}) => {
    ...
    const state = {
      ...
      agentConfigs: [
        {
          config: { id: 'agent-leader', slot_index: 0, label: 'Leader', persona_id: 'persona-manager', model_id: 'workspace-1', color: '#f5a623', icon_emoji: '👑', enabled: true, max_tokens: 4096, created_at: Date.now(), updated_at: Date.now() },
          persona: { id: 'persona-manager', name: 'Manager', system_prompt: 'Plan and synthesize.', model_id: 'workspace-1', color: '#f5a623', icon_emoji: '👑', created_at: Date.now(), updated_at: Date.now() },
          system_prompt: 'Plan and synthesize.',
          effective_model_id: 'workspace-1',
          ram_required_mb: 1600,
        },
        {
          config: { id: 'agent-worker-1', slot_index: 1, label: 'Programmer 1', persona_id: 'persona-programmer', model_id: 'workspace-1', color: '#4a9eff', icon_emoji: '🤖', enabled: true, max_tokens: 4096, created_at: Date.now(), updated_at: Date.now() },
          persona: { id: 'persona-programmer', name: 'Programmer', system_prompt: 'Write robust code.', model_id: 'workspace-1', color: '#4a9eff', icon_emoji: '🤖', created_at: Date.now(), updated_at: Date.now() },
          system_prompt: 'Write robust code.',
          effective_model_id: 'workspace-1',
          ram_required_mb: 1600,
        },
        {
          config: { id: 'agent-worker-2', slot_index: 2, label: 'Programmer 2', persona_id: 'persona-reviewer', model_id: 'workspace-1', color: '#50e38a', icon_emoji: '🤖', enabled: true, max_tokens: 4096, created_at: Date.now(), updated_at: Date.now() },
          persona: { id: 'persona-reviewer', name: 'Reviewer', system_prompt: 'Review and improve.', model_id: 'workspace-1', color: '#50e38a', icon_emoji: '🧪', created_at: Date.now(), updated_at: Date.now() },
    ...
    function listMockFiles() {
      const root = String(window.__workspaceWorkspacePath || 'z:/Projects/WorkspaceWorkspace');
      return [
        { path: `${root}/Runner-Streaming_System.md`, rel: 'Runner-Streaming_System.md', name: 'Runner-Streaming_System.md', is_dir: false },
        { path: `${root}/workspace`, rel: 'workspace', name: 'workspace', is_dir: true },
        { path: `${root}/workspace/src`, rel: 'workspace/src', name: 'src', is_dir: true },
        { path: `${root}/workspace/src/App.svelte`, rel: 'workspace/src/App.svelte', name: 'App.svelte', is_dir: false },
    ...
    async function handleSubmitChat(args) {
      ...
      if (/list all files|list files|current directory/i.test(lastUser)) {
        const out = await runToolAction({ tool: 'list_all_files', args: { path: window.__workspaceWorkspacePath } });
      ...
      if (/create folder\s+tool_test|create\s+hello\.txt|containing\s+hello/i.test(lastUser)) {
        const queue = [
          makeToolApproval(
            { tool: 'create_dir', args: { path: `${window.__workspaceWorkspacePath}/${LIVE_TEST_DIR_REL_LOCAL}` } },
            'Create folder tool_test/live-testing',
            'Need a writable folder for hello.txt.',
          ),
          makeToolApproval(
            { tool: 'write_file', args: { path: `${window.__workspaceWorkspacePath}/${LIVE_TEST_HELLO_REL_LOCAL}`, content: 'Hello\n' } },
            'Write hello.txt',
      ...
      if (/deny/i.test(lastUser) && /writ(e|ing)/i.test(lastUser)) {
        const card = makeToolApproval(
          { tool: 'write_file', args: { path: `${window.__workspaceWorkspacePath}/${LIVE_TEST_DENIED_REL_LOCAL}`, content: 'Denied path\n' } },
          'Write denied.txt',
    ...
    window.__TAURI_INTERNALS__ = {
      ...
      async invoke(cmd, args = {}) {
        ...
        if (cmd === 'plugin:barcode-scanner|scan') {
          return { content: 'workspace://connect?ip=127.0.0.1&port=11369&token=PAIR-TEST-1234' };
        }
        ...
        if (cmd === 'upsert_agent_config') {
          ...
          if (idx >= 0) {
            ...
            state.agentConfigs.push({
              ...
              persona: null,
              system_prompt: '',
              effective_model_id: nextCfg.model_id || 'workspace-1',
              ram_required_mb: 1600,
            });
          }
        ...
        if (cmd === 'get_orchestrator_status') {
          return {
            slots: [{ index: 0, port: 15000, state: { state: 'ready', model_id: 'workspace-1' }, requests: 2, idle_secs: 3 }],
            queue_depth: 0,
            total_ram_mb: 16384,
            free_ram_mb: 8192,
          };
    ...
    window.__WORKSPACE_LIVE_TESTING_STATE = state;
    ...
    if (tool === 'list_all_files') {
      ...
      return {
        ...
        entries: [
          ...
          { rel: 'workspace/src/App.svelte', path: path.join(root, 'workspace', 'src', 'App.svelte').replace(/\\/g, '/') },
        ],
      };
    }
...
async function scenarioVisionContextInjection(page) {
  ...
  await page.evaluate(() => {
    window.__WORKSPACE_TEST_VISION_CONTEXT = {
      active: true,
      snapshot: {
  ...
  await page.evaluate(() => {
    window.__WORKSPACE_TEST_VISION_CONTEXT = undefined;
  });
...
function printSummaryAndExit() {
  ...
  log('');
  log('Workspace Live-Testing Feature Summary');
...
async function main() {
  log(`Workspace Live-Testing Feature`);
  log(`UI base: ${UI_BASE}`);
  ...
  try {
    ...
    try {
      ...
      if (SCENARIO_FILTER && filteredScenarioPlan.length === 0) {
        throw new Error(`No scenarios matched WORKSPACE_SCENARIO_FILTER="${SCENARIO_FILTER}"`);
      }
