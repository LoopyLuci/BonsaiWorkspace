...
function resolveApk(explicitPath) {
  ...
  const candidates = [
    path.join(ROOT, 'workspace/src-tauri/gen/android/app/build/outputs/apk/universal/release/app-universal-release-unsigned.apk'),
    path.join(ROOT, 'workspace/src-tauri/gen/android/app/build/outputs/apk/universal/debug/app-universal-debug.apk'),
    path.join(ROOT, 'workspace/src-tauri/gen/android/app/build/outputs/apk/arm64-v8a/release/app-arm64-v8a-release-unsigned.apk'),
    path.join(ROOT, 'workspace/src-tauri/gen/android/app/build/outputs/apk/arm64-v8a/debug/app-arm64-v8a-debug.apk'),
  ];
  for (const c of candidates) {
...
function main() {
  ...
  steps.push({
    ...
    hint: packageInstalled
      ...
      : requireApp
        ...
        : 'Package not installed; continuing (WORKSPACE_REQUIRE_APP not set).',
  });
  ...
  if (packageInstalled || requireApp) {
    ...
    steps.push({
      ...
      hint: `Package ${packageName} is not installed; set WORKSPACE_REQUIRE_APP=1 to make this mandatory.`,
    });
  }
