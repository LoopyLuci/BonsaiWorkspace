...
const ORCHESTRATED_API_PORT = Number(process.env.WORKSPACE_ORCHESTRATED_API_PORT || 11369);

function getConfiguredApiBase() {
  const fallback = 'http://127.0.0.1:11369';
  try {
    if (process.env.WORKSPACE_API_BASE) return process.env.WORKSPACE_API_BASE;

    const appData = process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming');
    const cfgPath = path.join(appData, 'com.workspace.workspace', 'workspace-config.json');
...
function appDataConfigPath() {
  const appData = process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming');
  return path.join(appData, 'com.workspace.workspace', 'workspace-config.json');
}
...
async function run() {
  ...
  console.log('Starting Workspace stack...');
  ...
  tauriDev.stdout.on('data', (chunk) => {
    ...
    const m = text.match(/\[api\]\s+Workspace API server listening on\s+(http:\/\/[^\s]+)/i);
    if (m && m[1]) {
  ...
  try {
    ...
    const smoke = spawn(process.platform === 'win32' ? 'npm.cmd' : 'npm', ['run', 'test:agent-all'], {
      ...
      env: {
        ...process.env,
        WORKSPACE_API_BASE: API_BASE,
        WORKSPACE_UI_BASE: UI_BASE,
      },
      stdio: 'inherit',
    });
    ...
    console.log('Tearing down Workspace stack...');
    killTree(tauriDev);
  }
}
