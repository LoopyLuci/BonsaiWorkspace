...
async function setupListeners() {
  ...
  await listen<boolean>('low-memory-mode', (e) => {
    console.warn('[Workspace] Low memory mode:', e.payload);
  });
  ...
  await listen('sidecars-ready',   () => console.info('[Workspace] Sidecars ready ✓'));
  await listen('sidecars-timeout', () => console.warn('[Workspace] Sidecars timed out — AI unavailable'));
}
