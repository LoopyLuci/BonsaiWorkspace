...
impl Tool for DataProfileTool {
    ...
    fn description(&self) -> &str {
        "Generate a comprehensive data profile from a CSV: column types, null counts, min/max, distributions, and top values."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let csv_input = args["csv"]
            .as_str()
            .map(|s| s.to_string())
            .or_else(|| {
                args["path"]
...
impl Tool for CorrelationMatrixTool {
    ...
    fn description(&self) -> &str {
        "Compute pairwise Pearson or Spearman correlation for numeric columns in a CSV."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let csv = args["csv"]
            .as_str()
        ...
        if columns.len() < 2 {
            return Err("Need at least 2 numeric columns".into());
        }
        ...
        Ok(ToolResult::json(
            &json!({ "method": method, "columns": headers, "matrix": result_rows }),
        ))
    }
}
...
fn parse_numeric_cols(csv: &str) -> (Vec<String>, Vec<Vec<f64>>) {
    ...
    for (i, h) in headers.iter().enumerate() {
        let vals: Vec<f64> = all_rows
            .iter()
            .filter_map(|r| r.get(i).copied().flatten())
            .collect();
        if vals.len() > all_rows.len() / 2 {
...
impl Tool for OutlierDetectTool {
    ...
    fn description(&self) -> &str {
        "Detect outliers in a numeric column using IQR or Z-score method and return outlier rows."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let csv = args["csv"]
            .as_str()
        ...
        let col_idx = headers
            .iter()
            .position(|h| h.trim().trim_matches('"') == column)
            .ok_or(format!("Column '{}' not found", column))?;

        let rows: Vec<(usize, Vec<String>, f64)> = lines
            .enumerate()
            .filter_map(|(i, line)| {
        ...
        let outliers: Vec<Value> = rows
            .iter()
            .filter(|(_, _, v)| match method {
                "zscore" => ((v - mean) / std).abs() > threshold,
                _ => *v < q1 - threshold * iqr || *v > q3 + threshold * iqr,
            })
            .map(|(row_num, cols, v)| json!({ "row": row_num, "value": v, "columns": cols }))
            .collect();
...
impl Tool for RandomSampleTool {
    ...
    fn description(&self) -> &str {
        "Draw a random sample from a CSV dataset with or without replacement."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let csv = args["csv"]
            .as_str()
...
impl Tool for DataImputeTool {
    ...
    fn description(&self) -> &str {
        "Fill missing values in a CSV using mean, median, mode, or forward-fill strategy."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let csv = args["csv"]
            .as_str()
        ...
        let target_cols: Vec<usize> = if let Some(col) = column {
            headers
                .iter()
                .position(|h| h.trim().trim_matches('"') == col)
                .map(|i| vec![i])
...
impl Tool for AbTestAnalyzerTool {
    ...
    fn description(&self) -> &str {
        "Analyze A/B test results: statistical significance (chi-squared or t-test), effect size, confidence intervals, and sample size recommendations."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        // Conversion rate test (chi-squared)
        if let (Some(a_conversions), Some(a_total), Some(b_conversions), Some(b_total)) = (
            args["a_conversions"].as_u64(),
            args["a_total"].as_u64(),
            args["b_conversions"].as_u64(),
            args["b_total"].as_u64(),
        ) {
            let alpha = args["alpha"].as_f64().unwrap_or(0.05);
            let (p_a, p_b) = (
                a_conversions as f64 / a_total as f64,
                b_conversions as f64 / b_total as f64,
            );
            let p_pool = (a_conversions + b_conversions) as f64 / (a_total + b_total) as f64;
            let se =
                (p_pool * (1.0 - p_pool) * (1.0 / a_total as f64 + 1.0 / b_total as f64)).sqrt();
            ...
            let min_n = (2.0_f64 * (1.6449_f64 + 1.2816_f64).powi(2) * p_pool * (1.0 - p_pool)
                / (p_b - p_a).powi(2).max(1e-10))
            .ceil() as u64;
        ...
        if let (Some(a_arr), Some(b_arr)) =
            (args["a_values"].as_array(), args["b_values"].as_array())
        {
            let alpha = args["alpha"].as_f64().unwrap_or(0.05);
            ...
            let cohen_d = if (va + vb) > 0.0 {
                (mb - ma) / ((va + vb) / 2.0).sqrt()
            } else {
                0.0
            };
            let effect = if cohen_d.abs() < 0.2 {
                "negligible"
            } else if cohen_d.abs() < 0.5 {
                "small"
            } else if cohen_d.abs() < 0.8 {
                "medium"
            } else {
                "large"
            };
...
impl Tool for TimeSeriesDecomposeTool {
    ...
    fn description(&self) -> &str {
        "Decompose a time series into trend, seasonal, and residual components using moving average."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        if values.len() < period * 2 {
            return Err(format!("Need at least {} data points", period * 2));
        }
        let n = values.len();
        ...
        for i in half..n.saturating_sub(half) {
            trend[i] = values[i.saturating_sub(half)..=(i + half).min(n - 1)]
                .iter()
                .sum::<f64>()
                / window as f64;
        }
        ...
        let seasonal_avgs: Vec<f64> = seasonal_sums
            .iter()
            .zip(&seasonal_cnts)
        ...
        let residual: Vec<f64> = values
            ...
            .map(|(i, v)| {
                if trend[i].is_nan() {
                    ...
                    v - trend[i] - seasonal_avgs[i % period]
                }
            })
            .collect();
