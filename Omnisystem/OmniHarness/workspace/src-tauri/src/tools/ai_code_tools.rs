...
impl Tool for CodeExplainTool {
    ...
    fn description(&self) -> &str {
        "Explain what a code block does in plain language, including algorithmic complexity and design patterns."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        Ok(ToolResult::json(
            &json!({ "explanation": explanation, "language": lang, "code_length": code.len() }),
        ))
    }
}
...
impl Tool for CodeRefactorTool {
    ...
    fn description(&self) -> &str {
        "Suggest and apply refactoring improvements: extract method, rename variables, simplify conditionals, reduce complexity."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let code = args["code"].as_str().ok_or("Missing 'code'")?;
        let lang = args["language"].as_str().unwrap_or("auto-detect");
        let goals = args["goals"]
            .as_str()
            .unwrap_or("readability,maintainability,performance");
...
impl Tool for GenerateTestsTool {
    ...
    fn description(&self) -> &str {
        "Generate comprehensive unit tests for a function with edge cases, happy path, error cases, and mocks."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let code = args["code"].as_str().ok_or("Missing 'code'")?;
        let lang = args["language"].as_str().unwrap_or("auto-detect");
        ...
        Ok(ToolResult::json(
            &json!({ "tests": tests, "language": lang, "framework": framework }),
        ))
    }
}
...
impl Tool for CodeReviewTool {
    ...
    fn description(&self) -> &str {
        "Security-focused code review covering OWASP Top 10, injection, authentication, cryptography, input validation, and error handling."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let code = args["code"].as_str().ok_or("Missing 'code'")?;
        let lang = args["language"].as_str().unwrap_or("auto-detect");
        let focus = args["focus"]
            .as_str()
            .unwrap_or("security,quality,performance");
        ...
        let parsed: Value = serde_json::from_str(&raw)
            .unwrap_or(json!({ "summary": raw, "issues": [], "overall_score": null }));
        Ok(ToolResult::json(&parsed))
    }
}
...
impl Tool for GenerateDocumentationTool {
    ...
    fn description(&self) -> &str {
        "Generate comprehensive documentation: docstrings, README, API docs, or inline comments for a code file."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let code = args["code"].as_str().ok_or("Missing 'code'")?;
        let lang = args["language"].as_str().unwrap_or("auto-detect");
        ...
        let user =
            format!("Generate {format} documentation ({style} style) for:\n```{lang}\n{code}\n```");
        let docs = call_local_llm(&system, &user, 2048).await?;
        Ok(ToolResult::json(
            &json!({ "documentation": docs, "format": format, "style": style, "language": lang }),
        ))
    }
}
...
impl Tool for DependencyAuditTool {
    ...
    fn description(&self) -> &str {
        "Audit project dependencies for outdated packages, license issues, and known vulnerability patterns by parsing manifest files."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        if !findings.is_empty() {
            ...
            if let Ok(analysis) = call_local_llm(system, &user, 1024).await {
                let parsed: Value =
                    serde_json::from_str(&analysis).unwrap_or(json!({ "summary": analysis }));
                return Ok(ToolResult::json(
                    &json!({ "manifests": findings, "analysis": parsed }),
                ));
            }
        }
        Ok(ToolResult::json(
            &json!({ "manifests": findings, "message": "Scan complete" }),
        ))
    }
}
...
fn parse_deps_from_manifest(content: &str, lang: &str) -> Vec<Value> {
    match lang {
            ...
            for line in content.lines() {
                ...
                if in_deps && t.contains('=') {
                    if let Some((name, ver)) = t.split_once('=') {
                        deps.push(
                            json!({ "name": name.trim(), "version": ver.trim().trim_matches('"') }),
                        );
                    }
                }
            }
        ...
        "python" => content
            ...
            .map(|l| {
                let (name, ver) = l.split_once(&['=', '>', '<'][..]).unwrap_or((l, ""));
                json!({ "name": name.trim(), "version": ver.trim() })
            })
            .collect(),
        _ => content
            .lines()
...
impl Tool for GenerateApiClientTool {
    ...
    fn description(&self) -> &str {
        "Generate a typed API client (Rust/Python/TypeScript) from an OpenAPI spec, curl command, or API description."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let spec = args["spec"]
            .as_str()
            .ok_or("Missing 'spec' (OpenAPI JSON/YAML, curl command, or description)")?;
        ...
        Ok(ToolResult::json(
            &json!({ "client_code": client_code, "language": language, "style": style }),
        ))
    }
}
...
impl Tool for MigrateCodeTool {
    ...
    fn description(&self) -> &str {
        "Convert code between frameworks or languages (e.g., Express→Fastify, Python→Rust, callbacks→async/await)."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let code = args["code"].as_str().ok_or("Missing 'code'")?;
        let from = args["from"]
            .as_str()
            .ok_or("Missing 'from' (source language/framework)")?;
        let to = args["to"]
            .as_str()
            .ok_or("Missing 'to' (target language/framework)")?;
        ...
        let parsed: Value =
            serde_json::from_str(&raw).unwrap_or(json!({ "migrated_code": raw, "notes": [] }));
        Ok(ToolResult::json(&parsed))
    }
}
...
impl Tool for GenerateCiPipelineTool {
    ...
    fn description(&self) -> &str {
        "Generate a GitHub Actions, GitLab CI, or Jenkins pipeline for the current project based on detected languages and frameworks."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        let features = args["features"]
            .as_str()
            .unwrap_or("lint,test,build,deploy");
        ...
        Ok(ToolResult::json(
            &json!({ "pipeline": pipeline, "platform": platform, "filename": filename, "detected_languages": detected }),
        ))
    }
}
...
impl Tool for GenerateCommitMessageTool {
    ...
    fn description(&self) -> &str {
        "Generate a conventional commit message from a git diff or change description."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let diff_or_desc = args["diff"]
            .as_str()
        ...
        let parsed: Value = serde_json::from_str(&raw)
            .unwrap_or(json!({ "subject": raw.lines().next().unwrap_or(&raw), "body": "" }));
        Ok(ToolResult::json(&parsed))
    }
}
...
impl Tool for OptimizePromptTool {
    ...
    fn description(&self) -> &str {
        "Analyze and optimize an LLM prompt for clarity, specificity, structure, and expected output quality."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        let parsed: Value =
            serde_json::from_str(&raw).unwrap_or(json!({ "optimized_prompt": raw, "issues": [] }));
        Ok(ToolResult::json(&parsed))
    }
}
...
impl Tool for SqlExplainTool {
    ...
    fn description(&self) -> &str {
        "Explain a SQL query with execution plan analysis, optimization suggestions, and complexity estimate."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        let raw = call_local_llm(system, &user, 1024).await?;
        let parsed: Value = serde_json::from_str(&raw)
            .unwrap_or(json!({ "plain_english": raw, "optimizations": [] }));
        Ok(ToolResult::json(&parsed))
    }
}
...
impl Tool for QueryBuilderTool {
    ...
    fn description(&self) -> &str {
        "Convert natural language to SQL with proper joins, aggregations, and window functions given a schema."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        let raw = call_local_llm(&system, &user, 1024).await?;
        let parsed: Value =
            serde_json::from_str(&raw).unwrap_or(json!({ "sql": raw, "explanation": "" }));
        Ok(ToolResult::json(&parsed))
    }
}
...
impl Tool for DatabaseDiagramTool {
    ...
    fn description(&self) -> &str {
        "Generate an ER diagram in Mermaid format from a SQL schema or table definitions."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        Ok(ToolResult::json(
            &json!({ "diagram": diagram, "format": format }),
        ))
    }
}
...
impl Tool for SummarizeMeetingNotesTool {
    ...
    fn description(&self) -> &str {
        "Convert a raw meeting transcript into structured notes with action items, decisions, open questions, and key topics."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        let user =
            format!("Attendees: {attendees}\nContext: {context}\n\nTranscript:\n{transcript}");
        let raw = call_local_llm(system, &user, 1500).await?;
        let parsed: Value =
            serde_json::from_str(&raw).unwrap_or(json!({ "summary": raw, "action_items": [] }));
        Ok(ToolResult::json(&parsed))
    }
}
...
impl Tool for GenerateEmailTool {
    ...
    fn description(&self) -> &str {
        "Generate a professional email from bullet points with tone options (formal, casual, persuasive, apologetic)."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let points = args["points"]
            .as_str()
            .ok_or("Missing 'points' (bullet points or description)")?;
        ...
        let raw = call_local_llm(&system, &user, 1024).await?;
        let parsed: Value =
            serde_json::from_str(&raw).unwrap_or(json!({ "subject": subject_hint, "body": raw }));
        Ok(ToolResult::json(&parsed))
    }
}
...
impl Tool for GenerateFlashcardsTool {
    ...
    fn description(&self) -> &str {
        "Generate Anki-compatible flashcards from study material with question, answer, and difficulty rating."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        let mut parsed: Value =
            serde_json::from_str(&raw).unwrap_or(json!({ "cards": [], "topic": "Unknown" }));

        // Also generate CSV for Anki import
        if let Some(cards) = parsed["cards"].as_array() {
            let csv: String = cards
                .iter()
                .map(|c| {
...
impl Tool for GrammarCheckTool {
    ...
    fn description(&self) -> &str {
        "Check grammar, style, and clarity with specific suggestions (passive voice, wordiness, readability, consistency)."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        if lower.contains(" irregardless ") {
            quick_issues.push("'irregardless' is not standard — use 'regardless'");
        }
        if lower.contains(" alot ") {
            quick_issues.push("'alot' should be 'a lot'");
        }
        if lower.contains(" your welcome") {
            quick_issues.push("'your welcome' should be 'you're welcome'");
        }

        // Count passive voice indicators
        let passive = ["was ", "were ", "been ", "being ", "is ", "are "]
            .iter()
        ...
        let raw = call_local_llm(&system, &user, 2048).await?;
        let mut parsed: Value =
            serde_json::from_str(&raw).unwrap_or(json!({ "corrected_text": text, "issues": [] }));
...
impl Tool for PersonaSwitchTool {
    ...
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        let content = args["content"]
            .as_str()
            .ok_or("Missing 'content' to respond to")?;
        ...
        Ok(ToolResult::json(
            &json!({ "persona": persona, "response": response }),
        ))
    }
}
    ...
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        match operation {
            "save" => {
                ...
                .map_err(|e| e.to_string())?;
                Ok(ToolResult::json(
                    &json!({ "saved": true, "path": path.to_string_lossy(), "name": name }),
                ))
            }
            "load" => {
                let path = dir.join(format!("{}.json", sanitize_filename(name)));
                let raw = tokio::fs::read_to_string(&path)
                    .await
                    .map_err(|_| format!("Snapshot '{}' not found", name))?;
            ...
            "list" => {
                ...
                while let Ok(Some(entry)) = dir_read.next_entry().await {
                    ...
                    if fname.ends_with(".json") {
                        entries.push(fname.trim_end_matches(".json").to_string());
                    }
                }
...
impl Tool for SimulateConversationTool {
    ...
    fn description(&self) -> &str {
        "Simulate a multi-turn conversation between two personas (e.g., customer and support agent, interviewer and candidate)."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        let raw = call_local_llm(&system, &user, 2048).await?;
        let parsed: Value = serde_json::from_str(&raw)
            .unwrap_or(json!({ "conversation": [{"speaker": "A", "text": raw}] }));
        Ok(ToolResult::json(&parsed))
    }
}
...
impl Tool for GenerateTutorialTool {
    ...
    fn description(&self) -> &str {
        "Generate a structured, progressive tutorial for a topic with steps, code examples, and exercises."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        if format == "json" {
            let parsed: Value =
                serde_json::from_str(&tutorial).unwrap_or(json!({ "content": tutorial }));
            Ok(ToolResult::json(&parsed))
        } else {
            Ok(ToolResult::json(
                &json!({ "tutorial": tutorial, "topic": topic, "level": level }),
            ))
        }
    }
}
