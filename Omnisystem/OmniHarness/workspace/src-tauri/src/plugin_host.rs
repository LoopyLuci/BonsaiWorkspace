...
pub struct PluginHost {
    ...
    plugins_dir: PathBuf,
    /// Universal Module System lifecycle state (see `OmniModule` impl below).
    module_state: RwLock<omnisystem_core::ModuleState>,
}
...
impl PluginHost {
    pub fn new() -> Self {
        ...
        Self {
            ...
            plugins_dir: dir,
            module_state: RwLock::new(omnisystem_core::ModuleState::Unloaded),
        }
    }
...
//
// PluginHost is the reference implementation of `OmniModule`: it already does
// exactly what the trait models (load/unload, capability-gated execution,
// integrity-checked lifecycle) for its own plugins, so wiring it in makes it
// registrable through `omnisystem_core::ModuleRegistry` /
// `omnisystem_core::AppModuleLoader` alongside every other module in the
// system, instead of being a plugin system that only manages *itself*.
//
// `initialize`/`shutdown`/`set_config` are sync (trait requirement) bridging
// into this struct's async methods via `block_on` — the same sync/async
// bridge pattern already used elsewhere in this backend (see
// `AgentCore::load_adapter` in agent_core.rs).

impl omnisystem_core::OmniModule for PluginHost {
    fn name(&self) -> &str {
        "plugin_host"
    }
    ...
    fn initialize(&mut self) -> omnisystem_core::Result<()> {
        let handle = tokio::runtime::Handle::try_current()
            .map_err(|e| omnisystem_core::Error::RuntimeError(e.to_string()))?;
        handle.block_on(async {
            self.load_all().await;
            *self.module_state.write().await = omnisystem_core::ModuleState::Active;
        });
        Ok(())
    }
    ...
    fn shutdown(&mut self) -> omnisystem_core::Result<()> {
        ...
        handle.block_on(async {
            ...
            *self.module_state.write().await = omnisystem_core::ModuleState::Unloaded;
        });
        Ok(())
    }

    fn state(&self) -> omnisystem_core::ModuleState {
        tokio::runtime::Handle::try_current()
            .map(|h| h.block_on(async { *self.module_state.read().await }))
            .unwrap_or(omnisystem_core::ModuleState::Unloaded)
    }

    fn capabilities(&self) -> Vec<String> {
        // Union of every capability granted to any currently-loaded plugin —
        // this is what the module registry needs to know to route capability
        // requests toward "does some loaded plugin provide X?".
        ...
        handle.block_on(async {
            self.plugins
                ...
                .await
                .values()
                .flat_map(|p| p.manifest.capabilities.iter().map(|c| format!("{c:?}")))
