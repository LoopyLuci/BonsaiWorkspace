...
use crate::kernel::{
    lift, normalize, subst, AxiomKernel, Context, Environment, KernelError, KernelResult,
    ProofWitness, Sort, Term,
};
use thiserror::Error;
...
impl TacticEngine {
    ...
    pub fn with_env(env: Environment) -> Self {
        Self {
            kernel: AxiomKernel::with_env(env),
        }
    }
    ...
    pub fn intro(&self, state: &mut ProofState, name: impl Into<String>) -> TacticResult<()> {
        ...
        match normalize(&goal.ty, &state.env) {
            Term::Pi {
                ...
                state.push_goal(Goal::new(
                    new_goal_ty,
                    new_ctx,
                    format!("after intro {name}"),
                ));
                // The proof term will be a lambda; note it
                state.close_with(Term::lam(&name, *domain, Term::var(0)));
                Ok(())
            }
            other => Err(TacticError::Failed(format!(
                "intro: goal is not a Π-type, got {:?}",
                other
            ))),
        }
    }
    ...
    pub fn exact(&self, state: &mut ProofState, term: Term) -> TacticResult<()> {
        ...
        if !self
            .kernel
            .definitionally_equal(&inferred, &goal.ty, &goal.ctx)
        {
            return Err(TacticError::Failed(format!(
    ...
    pub fn apply(&self, state: &mut ProofState, func_term: Term) -> TacticResult<()> {
        ...
        match normalize(&func_ty, &state.env) {
            Term::Pi {
                ...
                if !self
                    .kernel
                    .definitionally_equal(&conclusion, &goal.ty, &goal.ctx)
                {
    ...
    pub fn assumption(&self, state: &mut ProofState) -> TacticResult<()> {
        ...
        let (goal_ty, ctx_len, matching_idx) = {
            ...
            let fast = if let Term::Var(j) = &goal_ty {
                if *j < ctx_len {
                    Some(*j)
                } else {
                    None
        ...
        if let Some(i) = matching_idx {
            ...
            Err(TacticError::Failed(
                "assumption: no matching hypothesis".into(),
            ))
        }
    }
    ...
    pub fn induction(&self, state: &mut ProofState, var_name: &str) -> TacticResult<()> {
        ...
        let var_idx = (0..goal.ctx.len())
            ...
            .ok_or_else(|| {
                TacticError::Failed(format!("induction: variable {var_name} not found"))
            })?;

        let (_, var_ty) = goal.ctx.lookup(var_idx).unwrap();
        if !matches!(normalize(var_ty, &state.env), Term::Nat) {
            return Err(TacticError::Failed(format!(
                "induction: {var_name} is not Nat"
            )));
        }
        ...
        state.push_goal(Goal::new(
            base_ty,
            goal.ctx.clone(),
            format!("induction base: {var_name}=0"),
        ));
        ...
        let step_ty = Term::pi("n", Term::Nat, Term::pi("ih", step_hyp, step_concl));
        state.push_goal(Goal::new(
            step_ty,
            goal.ctx.clone(),
            format!("induction step: {var_name}"),
        ));
    ...
    pub fn trivial(&self, state: &mut ProofState) -> TacticResult<()> {
        ...
        match normalize(&goal.ty, &state.env) {
            ...
            Term::Pi {
                domain, codomain, ..
            } if matches!(*domain, Term::Sort(Sort::Prop))
                && matches!(*codomain, Term::Sort(Sort::Prop)) =>
            {
                let _ = state.pop_goal()?;
            ...
            _ => Err(TacticError::Failed(
                "trivial: goal is not trivially true".into(),
            )),
        }
    }
...
mod tests {
    use super::*;
    use crate::kernel::{Environment, Sort, Term};
    ...
    fn trivial_proof() {
        ...
        let witness = engine
            .run(prop, empty_env(), |eng, state| eng.trivial(state))
            .unwrap();
