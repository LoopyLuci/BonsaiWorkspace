//! Aether seed — the Aether bootstrap interpreter driver (Rust host).
//!
//!     aether-seed run    program.aether
//!     aether-seed tokens program.aether
//!     aether-seed test   [dir]
...
/// Lex + parse only (no execution) — fast feedback, mirrors
...
fn cmd_test(dir: &str) -> i32 {
    ...
    let mut files: Vec<PathBuf> = rd.filter_map(|e| e.ok()).map(|e| e.path()).filter(|p| p.extension().is_some_and(|x| x == "aether")).collect();
    files.sort();
...
fn main() -> ExitCode {
    ...
    let code = match args.first().map(String::as_str) {
        ...
        _ => {
            println!("Aether seed {VERSION}\n\nUsage: aether-seed <run|tokens|test> [file|dir]");
            0
        }
    };
