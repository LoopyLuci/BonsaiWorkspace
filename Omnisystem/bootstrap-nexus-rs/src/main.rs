//! Nexus seed — the Nexus bootstrap constraint/layout solver driver.
//!
//!     nexus-seed run    layout.nexus
//!     nexus-seed tokens layout.nexus
//!     nexus-seed test   [dir]
...
fn cmd_run(file: &str) -> i32 {
    ...
    let mut solver = Solver::new(file);
    match solver.solve_module(&module) {
        Ok(code) => {
            print!("{}", solver.out);
            code
        }
...
/// Lex + parse only (no solving) — fast feedback, mirrors
/// `Omnisystem/bootstrap`'s Titan `check` command.
...
fn cmd_test(dir: &str) -> i32 {
    ...
    let mut files: Vec<PathBuf> = rd.filter_map(|e| e.ok()).map(|e| e.path()).filter(|p| p.extension().is_some_and(|x| x == "nexus")).collect();
    files.sort();
    ...
    for file in &files {
        ...
        match parse_file(&src, &path_str) {
            Ok(module) => {
                let mut solver = Solver::new(&path_str);
                match solver.solve_module(&module) {
                    Ok(code) => {
                        exit = code;
                        stdout = solver.out.clone();
                    }
                    Err(_) => {
                        errored = true;
                        stdout = solver.out.clone();
...
fn main() -> ExitCode {
    ...
    let code = match args.first().map(String::as_str) {
        ...
        _ => {
            println!("Nexus seed {VERSION}\n\nUsage: nexus-seed <run|check|tokens|test> [file|dir]");
            0
        }
    };
