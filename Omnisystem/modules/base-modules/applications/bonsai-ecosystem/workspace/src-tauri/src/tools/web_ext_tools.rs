...
impl Tool for WebScrapeStructuredTool {
    ...
    fn description(&self) -> &str {
        "Scrape a URL and return structured data: title, headings, links, tables, main text content, and meta tags."
    }
...
impl Tool for RssFeedsTool {
    ...
    fn description(&self) -> &str {
        "Fetch and merge multiple RSS/Atom feeds, returning items sorted by date."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        for url_val in urls {
            ...
            match client.get(url).send().await {
                Ok(resp) => match resp.text().await {
                    Ok(text) => match feed_rs::parser::parse(text.as_bytes()) {
                        Ok(feed) => {
...
impl Tool for PdfExtractTool {
    ...
    fn description(&self) -> &str {
        "Extract text content from a PDF file (offline, no network required)."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        let full_text: String = text_pages
            .iter()
...
impl Tool for SqlFormatTool {
    ...
    fn description(&self) -> &str {
        "Pretty-print SQL queries with consistent indentation, keyword case, and whitespace."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        let statements = if let Ok(stmts) = Parser::parse_sql(&GenericDialect {}, sql) {
            ...
            return Ok(ToolResult::json(
                &json!({ "formatted": format_sql_basic(sql, case, indent), "method": "basic_formatter", "original": sql }),
            ));
        };
...
impl Tool for BatteryInfoTool {
    ...
    fn description(&self) -> &str {
        "Return battery percentage, charge state, health, and estimated time remaining."
    }
    async fn run(&self, _args: &Value) -> Result<ToolResult, String> {
        ...
        if batteries.is_empty() {
            return Ok(ToolResult::json(
                &json!({ "batteries": [], "message": "No battery found (desktop system)" }),
            ));
        }
        Ok(ToolResult::json(
            &json!({ "batteries": batteries, "count": batteries.len() }),
        ))
    }
}
...
impl Tool for WebhookSendTool {
    ...
    fn description(&self) -> &str {
        "Send a JSON payload to a webhook URL. Requires explicit confirmation via the 'confirmed' field."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        Ok(ToolResult::json(
            &json!({ "status": status, "response": body, "url": url, "method": method }),
        ))
    }
}
...
impl Tool for BackupStrategyTool {
    ...
    fn description(&self) -> &str {
        "Design a backup plan for a directory: inventory files, calculate size, and write a shell backup script."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        let compressed_est_mb = if compress {
            total_mb * 6 / 10
        } else {
            total_mb
        };
...
"#,
        ...
        let top_exts: Vec<Value> = {
            ...
            v.into_iter()
                .take(5)
                .map(|(e, c)| json!({ "extension": e, "count": c }))
                .collect()
        };
...
impl Tool for GraphVisualizeTool {
    ...
    fn description(&self) -> &str {
        "Generate a Mermaid diagram from a structured description: flowchart, sequence, class, ER, or Gantt."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        ...
        if let Some(r) = raw {
            return Ok(ToolResult::json(
                &json!({ "diagram": r, "type": diagram_type }),
            ));
        }
        ...
        let diagram = match diagram_type {
            "flowchart" | "graph" => {
                ...
                for edge in &edges {
                    ...
                    if label.is_empty() {
                        d.push_str(&format!("    {from} {arrow} {to}\n"));
                    } else {
                        d.push_str(&format!("    {from} {arrow}|{label}| {to}\n"));
                    }
                }
                d
            }
        ...
        Ok(ToolResult::json(
            &json!({ "diagram": diagram, "type": diagram_type, "render_url": format!("https://mermaid.live/edit#base64:{}", base64_encode_str(&diagram)) }),
        ))
    }
}
...
impl Tool for DiffVersionsTool {
    ...
    fn description(&self) -> &str {
        "Compare two versions of a file or text with a semantic-aware unified diff."
    }
    async fn run(&self, args: &Value) -> Result<ToolResult, String> {
        let (old_text, new_text) = if let (Some(a), Some(b)) =
            (args["old"].as_str(), args["new"].as_str())
        {
            (a.to_string(), b.to_string())
        } else if let (Some(f1), Some(f2)) = (args["file_a"].as_str(), args["file_b"].as_str()) {
            (
                tokio::fs::read_to_string(f1)
                    .await
                    .map_err(|e| e.to_string())?,
                tokio::fs::read_to_string(f2)
                    .await
                    .map_err(|e| e.to_string())?,
            )
        } else {
