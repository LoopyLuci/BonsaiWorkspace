//! Vera seed — the Vera bootstrap interpreter driver (Rust host).
//!
//!     vera-seed run    program.vera
//!     vera-seed tokens program.vera
//!     vera-seed test   [dir]
...
fn cmd_test(dir: &str) -> i32 {
    ...
    let mut files: Vec<PathBuf> = rd.filter_map(|e| e.ok()).map(|e| e.path()).filter(|p| p.extension().is_some_and(|x| x == "vera")).collect();
    files.sort();
...
fn main() -> ExitCode {
    ...
    let code = match args.first().map(String::as_str) {
        ...
        _ => {
            println!("Vera seed {VERSION}\n\nUsage: vera-seed <run|check|tokens|test> [file|dir]");
            0
        }
    };
