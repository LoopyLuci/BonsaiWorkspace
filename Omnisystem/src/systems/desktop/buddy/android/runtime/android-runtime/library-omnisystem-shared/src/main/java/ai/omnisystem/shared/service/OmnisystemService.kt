package ai.omnisystem.shared.service
...
import ai.omnisystem.shared.IOmnisystemService
...
class OmnisystemService : Service() {
    private val binder = OmnisystemServiceImpl(this)

    companion object {
        private const val TAG = "OmnisystemService"

        init {
            try {
                System.loadLibrary("omnisystem_android_llm")
                Log.i(TAG, "Native library loaded: omnisystem_android_llm")
            } catch (e: UnsatisfiedLinkError) {
                Log.e(TAG, "Failed to load native library", e)
                throw RuntimeException("Failed to load omnisystem_android_llm", e)
            }
        }
    }
    ...
    override fun onCreate() {
        super.onCreate()
        Log.i(TAG, "OmnisystemService created")
        binder.initialize(this)
    }

    override fun onBind(intent: Intent): IBinder {
        Log.i(TAG, "OmnisystemService bound")
        return binder.asBinder()
    }

    override fun onDestroy() {
        super.onDestroy()
        Log.i(TAG, "OmnisystemService destroyed")
        binder.shutdown()
    }
}

/**
 * OmnisystemServiceImpl
 *
 ...
 */
class OmnisystemServiceImpl(private val service: OmnisystemService) : IOmnisystemService.Stub() {
    companion object {
        private const val TAG = "OmnisystemServiceImpl"
        private const val MODELS_DIR = "/sdcard/Omnisystem/models"
        private const val CACHE_DIR = "/data/local/tmp/omnisystem"
    }
    ...
    fun initialize(context: Context) {
        try {
            ...
            Log.i(TAG, "OmnisystemServiceImpl initialized")
        } catch (e: Exception) {
            Log.e(TAG, "Failed to initialize OmnisystemServiceImpl", e)
        }
    }
    ...
    override fun generateStream(handle: Long, prompt: String, callback: IOmnisystemCallback) {
        try {
    ...
    override fun shutdown() {
        try {
            ...
            Log.i(TAG, "OmnisystemServiceImpl shutdown complete")
        } catch (e: Exception) {
