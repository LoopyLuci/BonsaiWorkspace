...
test.describe('Omnisystem Critical Path', () => {
  ...
  test('MLP-01: App launches and shows model status', async ({ page }) => {
    ...
    await expect(page.locator('[data-omnisystem-action="Model:Select"]')).toBeVisible({ timeout: 10000 });
  });

  test('MLP-02: File tree container is present', async ({ page }) => {
    await page.goto(APP_URL);
    await expect(page.locator('[data-omnisystem-action="FileTree"]')).toBeVisible();
  });

  test('MLP-03: Chat message receives a response', async ({ page }) => {
    await page.goto(APP_URL);
    const input = page.locator('[data-omnisystem-action="Chat:Send"] textarea');
    await input.fill('Hello');
    await page.locator('[data-omnisystem-action="Chat:Send"] button').click();
