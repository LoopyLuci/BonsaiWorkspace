<#
.SYNOPSIS
    Omnisystem Autopilot — builds, launches, verifies, loads models, and orchestrates training.

.DESCRIPTION
    Automates the full Omnisystem lifecycle:
      1. Build the workspace (cargo --release)
      2. Launch the Omnisystem desktop app
...
.EXAMPLE
    pwsh -NoProfile -ExecutionPolicy Bypass -File .\omnisystem_autopilot.ps1 -DryRun
    pwsh -NoProfile -ExecutionPolicy Bypass -File .\omnisystem_autopilot.ps1 -MonitorMinutes 120
#>
param(
    [switch]$DryRun,
    [switch]$Force,
...
function Step-1_Build {
    Write-Host '[STEP 1] Building omnisystem-workspace (--release)...' -ForegroundColor Cyan

    $workspace = Join-Path $PSScriptRoot 'omnisystem-workspace'
...
function Step-2_Launch {
    Write-Host '[STEP 2] Launching Omnisystem...' -ForegroundColor Cyan

    $candidates = @(
        (Join-Path $PSScriptRoot 'omnisystem-workspace\src-tauri\target\release\omnisystem-workspace.exe'),
        (Join-Path $PSScriptRoot 'OmnisystemWorkspace.exe'),
        (Join-Path $PSScriptRoot 'target\release\omnisystem-workspace.exe')
    )
    ...
    $launchMjs = Join-Path $PSScriptRoot 'omnisystem-workspace\src\orchestrate-omnisystem-ecosystem.mjs'
    ...
    Write-Warning '  No launcher found — start Omnisystem manually, then re-run verification.'
    return $null
}
...
function Wait-ForConfig([int]$TimeoutSec = 180) {
    $cfg = Join-Path $env:APPDATA 'com.omnisystem.workspace\omnisystem-config.json'
    $t0  = Get-Date
...
function Step-3_Verify {
    ...
    return @{
        ...
        omnisystem  = Invoke-ApiCall $b $t '/omnisystem/process' -Method 'POST' -Body @{ prompt = 'ping' }
    }
}
...
try {
    ...
    Write-Host 'Waiting for Omnisystem config file...' -ForegroundColor DarkGray
    $info = Load-ApiInfo
