param(
  [string]$ShortcutName = 'Omnisystem Workspace',
...
$launcherCmd = Join-Path $workspaceRoot 'Launch-OmnisystemWorkspace.cmd'
$launcherPs1 = Join-Path $workspaceRoot 'Launch-OmnisystemWorkspace.ps1'
$iconPath = Join-Path $workspaceRoot 'omnisystem-workspace\src-tauri\icons\icon.ico'
...
foreach ($desktop in $desktopTargets) {
  ...
  $shortcut.Description = 'Launch Omnisystem Workspace'
...
if ($created.Count -eq 0) {
  ...
  Write-Host 'Done. You can now launch Omnisystem from the desktop icon.' -ForegroundColor Cyan
}
