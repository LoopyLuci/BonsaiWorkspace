...
<#
.SYNOPSIS
Validate Omnisystem CI/CD deployment setup
...
function Test-FilesExist {
    ...
    Add-Check "orchestrator.ps1" "CI/CD orchestrator exists" {
        Test-Path ".\scripts\omnisystem-ci-orchestrator-complete.ps1" -PathType Leaf
    } -Critical $true

    Add-Check "deployment script" "Deployment script exists" {
        Test-Path ".\scripts\deploy-omnisystem-ci.ps1" -PathType Leaf
    } -Critical $true

    Add-Check "health check script" "Health check endpoint exists" {
        Test-Path ".\scripts\omnisystem-ci-health-check.ps1" -PathType Leaf
    } -Critical $false

    Add-Check "CI config" "CI configuration file exists" {
        Test-Path ".\omnisystem-ci-config.json" -PathType Leaf
    } -Critical $true

    Add-Check "GitHub workflow" "GitHub workflow file exists" {
        Test-Path ".\.github\workflows\omnisystem-ci-native.yml" -PathType Leaf
    } -Critical $true
}
...
function Test-ConfigurationFiles {
    ...
    Add-Check "Config JSON syntax" "omnisystem-ci-config.json is valid JSON" {
        $config = Get-Content ".\omnisystem-ci-config.json" -Raw
...
function Test-Orchestrator {
    ...
    Add-Check "Orchestrator syntax" "Orchestrator script has valid syntax" {
        $ortho = Get-Content ".\scripts\omnisystem-ci-orchestrator-complete.ps1"
    ...
    Add-Check "Orchestrator dry-run" "Orchestrator runs without errors (dry-run)" {
        & ".\scripts\omnisystem-ci-orchestrator-complete.ps1" `
...
function Test-WorkflowSyntax {
    ...
    Add-Check "GitHub workflow YAML" "GitHub workflow is valid YAML" {
        ...
        $workflow = Get-Content ".\.github\workflows\omnisystem-ci-native.yml" -Raw
...
# Main execution
Write-Host "Omnisystem CI/CD Validation Report" -ForegroundColor Cyan
