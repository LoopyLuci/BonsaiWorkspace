...
$srcDir = Join-Path $workspaceRoot 'omnisystem-workspace\src'
...
try {
  ...
  $launchScript = Join-Path $srcDir 'orchestrate-omnisystem-ecosystem.mjs'
  ...
  # ── Survival Watchdog ────────────────────────────────────────────────────
  # Start the watchdog before the main app so it can monitor and restart Omnisystem
  # if it crashes.  Best-effort: skip silently if the binary isn't built yet.
  $watchdogCandidates = @(
    (Join-Path $workspaceRoot 'target\release\omnisystem-watchdog.exe'),
    (Join-Path $workspaceRoot 'omnisystem-workspace\src-tauri\target\release\omnisystem-watchdog.exe'),
    (Join-Path $env:USERPROFILE '.omnisystem\bin\omnisystem-watchdog.exe')
  )
  $watchdogExe = $watchdogCandidates | Where-Object { Test-Path $_ } | Select-Object -First 1

  if ($watchdogExe) {
    $existing = Get-Process -Name 'omnisystem-watchdog' -ErrorAction SilentlyContinue
    if (-not $existing) {
    ...
    Write-Host "[one-click] Watchdog binary not found — run OmnisystemExeLauncherBuilder.ps1 to build it." -ForegroundColor DarkYellow
  }
  # ─────────────────────────────────────────────────────────────────────────

  Write-Host "[one-click] Launching Omnisystem Workspace from $srcDir" -ForegroundColor Cyan
  ...
  # The watchdog will restart Omnisystem on its own if it detects a crash exit.
  if ($nodeExit -eq 0) {
    Get-Process -Name 'omnisystem-watchdog' -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
  }
