...
use crate::tool_core::{
    SideEffectProfile, Tool, ToolContext, ToolError, ToolOutput, ToolPolicyHint, ToolRegistry,
    ToolResult,
};
...
impl McpManager {
    ...
    pub async fn connect_all_into_registry(
        ...
        if allowed_commands.is_empty() {
            tracing::warn!(
                "[mcp] mcp_allowed_commands is empty; allowing all configured MCP commands"
            );
        }
