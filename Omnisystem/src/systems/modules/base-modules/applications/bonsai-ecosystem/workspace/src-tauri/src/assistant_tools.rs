use crate::tool_core::{
    RetryPolicy, SideEffectProfile, Tool, ToolContext, ToolError, ToolOutput, ToolPolicyHint,
...
fn resolve_and_jail(
    ...
    let root = std::fs::canonicalize(root_raw).unwrap_or_else(|_| PathBuf::from(root_raw));
...
async fn check_ssrf_policy(url_str: &str) -> Result<(), ToolError> {
    ...
    let url = reqwest::Url::parse(url_str).map_err(|e| ToolError::ValidationFailed {
        field: "url".into(),
        reason: e.to_string(),
    })?;
    ...
    for addr in addrs {
        if is_private_ip(addr.ip()) {
            return Err(ToolError::Permission {
                message: format!(
                    "fetch_url: {host} resolves to internal address {}",
                    addr.ip()
                ),
            });
        }
...
impl Tool for GetDatetime {
    ...
    async fn execute(&self, _args: &Value, _ctx: &ToolContext) -> ToolResult {
        ...
        Ok(ToolOutput::Complete(
            json!({ "datetime": dt, "unix_timestamp": secs }),
        ))
    }
}
...
impl Tool for GetSystemStats {
    ...
    fn description(&self) -> &'static str {
        "Returns full system specs: OS name/version, CPU model/cores/usage, RAM, swap, disk space per drive, and hostname."
    }
    fn tags(&self) -> &'static [&'static str] {
        &[
            "system", "cpu", "ram", "memory", "disk", "stats", "hardware",
        ]
    }
    fn side_effects(&self) -> SideEffectProfile {
        SideEffectProfile::None
...
impl Tool for GetWeather {
    ...
    fn description(&self) -> &'static str {
        "Returns current weather conditions for a location. No API key required."
    }
    fn tags(&self) -> &'static [&'static str] {
        &[
            "weather",
    ...
    async fn execute(&self, args: &Value, _ctx: &ToolContext) -> ToolResult {
        ...
        let (lat, lon, city) = if location == "auto" || location.is_empty() {
            ...
            geocode_location(location)
                ...
                .ok_or_else(|| ToolError::NotFound {
                    resource: format!("location: {location}"),
                })?
        };
        ...
        let resp: Value = client
            ...
            .await
            .map_err(|e| ToolError::Transient {
                message: format!("weather fetch: {e}"),
                retry_after_ms: None,
            })?
            .json()
            .await
            .map_err(|e| ToolError::Transient {
                message: format!("weather parse: {e}"),
                retry_after_ms: None,
            })?;

        let cur = resp.get("current").ok_or_else(|| ToolError::Internal {
            message: "no current field".into(),
        })?;
...
impl Tool for FetchUrl {
    ...
    fn description(&self) -> &'static str {
        "Fetches a URL and returns its text content (HTML stripped by default)."
    }
    fn tags(&self) -> &'static [&'static str] {
        &[
            "web", "url", "http", "fetch", "scrape", "browse", "internet", "page",
        ]
    }
    fn side_effects(&self) -> SideEffectProfile {
        SideEffectProfile::None
    ...
    async fn execute(&self, args: &Value, _ctx: &ToolContext) -> ToolResult {
        ...
        let text = resp.text().await.map_err(|e| ToolError::Transient {
...
impl Tool for FindFiles {
    ...
    fn description(&self) -> &'static str {
        "Searches for files matching a glob pattern under a directory."
    }
    fn tags(&self) -> &'static [&'static str] {
        &[
            "files",
    ...
    fn policy_hint(&self) -> ToolPolicyHint {
        ToolPolicyHint::filesystem_read()
    }
    fn cache_ttl_secs(&self) -> Option<u64> {
        Some(60)
    }
    ...
    async fn execute(&self, args: &Value, ctx: &ToolContext) -> ToolResult {
        let root = args.get("path").and_then(|v| v.as_str()).ok_or_else(|| {
            ToolError::ValidationFailed {
                field: "path".into(),
                reason: "required".into(),
            }
        ...
        Ok(ToolOutput::Complete(
            json!({ "files": results, "count": count }),
        ))
    }
}
...
impl Tool for ReadFileAssistant {
    ...
    fn description(&self) -> &'static str {
        "Reads a text file from disk and returns its contents (up to 64KB)."
    }
    fn tags(&self) -> &'static [&'static str] {
        &["file", "read", "open", "content", "text", "source"]
    }
    fn side_effects(&self) -> SideEffectProfile {
        SideEffectProfile::Read
    }
    ...
    async fn execute(&self, args: &Value, ctx: &ToolContext) -> ToolResult {
        ...
        let content = std::fs::read_to_string(&jailed).map_err(|e| match e.kind() {
        ...
        Ok(ToolOutput::Complete(
            json!({ "path": jailed.display().to_string(), "content": content, "truncated": truncated }),
        ))
    }
}
...
impl Tool for WriteFileAssistant {
    ...
    fn description(&self) -> &'static str {
        "Writes text content to a file on disk. Requires user confirmation."
    }
    fn tags(&self) -> &'static [&'static str] {
        &["file", "write", "save", "create", "edit", "modify"]
    }
    fn side_effects(&self) -> SideEffectProfile {
        SideEffectProfile::Write
    }
    fn policy_hint(&self) -> ToolPolicyHint {
        ToolPolicyHint::filesystem_write()
    }
    ...
    async fn execute(&self, args: &Value, ctx: &ToolContext) -> ToolResult {
        ...
        if let Some(parent) = jailed.parent() {
            std::fs::create_dir_all(parent).map_err(|e| ToolError::Permission {
                message: e.to_string(),
            })?;
        }
        std::fs::write(&jailed, content).map_err(|e| ToolError::Permission {
            message: format!("write {}: {e}", jailed.display()),
        })?;
        Ok(ToolOutput::Complete(
            json!({ "path": jailed.display().to_string(), "bytes_written": content.len() }),
        ))
    }
}
...
impl Tool for OpenUrl {
    ...
    fn tags(&self) -> &'static [&'static str] {
        &["browser", "open", "url", "web", "link", "navigate"]
    }
    fn side_effects(&self) -> SideEffectProfile {
...
impl Tool for RenderChart {
    ...
    fn description(&self) -> &'static str {
        "Renders a bar, line, or pie chart as inline SVG from JSON data."
    }
    fn tags(&self) -> &'static [&'static str] {
        &[
            "chart",
    ...
    fn cache_ttl_secs(&self) -> Option<u64> {
        None
    } // keyed on args, session-scoped cache is enough
    ...
    async fn execute(&self, args: &Value, _ctx: &ToolContext) -> ToolResult {
        ...
        let data: Vec<Value> =
            serde_json::from_str(data_str).map_err(|e| ToolError::ValidationFailed {
                field: "data_json".into(),
                reason: e.to_string(),
            })?;
        ...
        Ok(ToolOutput::Complete(
            json!({ "svg": svg, "chart_type": chart_type }),
        ))
    }
}
...
fn render_bar(data: &[Value], title: &str) -> String {
    ...
    let max: f64 = data
        .iter()
        .filter_map(|d| d.get("value")?.as_f64())
        .fold(0.0f64, f64::max)
        .max(1.0);
    ...
    for (i, item) in data.iter().enumerate() {
        ...
        bars.push_str(&format!(
            "<rect x=\"{x}\" y=\"{y}\" width=\"{bw}\" height=\"{bh}\" fill=\"#5ca4ea\" rx=\"2\"/>"
        ));
...
fn render_pie(data: &[Value], title: &str) -> String {
    ...
    let total: f64 = data
        ...
        .max(1.0);
    let colors = [
        "#5ca4ea", "#e05260", "#50c878", "#f5a623", "#b68bf7", "#40c4e0",
    ];
...
impl Tool for SendEmail {
    ...
    fn description(&self) -> &'static str {
        "Sends an email via configured SMTP. Requires user confirmation."
    }
    fn tags(&self) -> &'static [&'static str] {
        &["email", "mail", "send", "smtp", "message", "compose"]
    }
    fn side_effects(&self) -> SideEffectProfile {
        SideEffectProfile::External
    }
    ...
    async fn execute(&self, args: &Value, ctx: &ToolContext) -> ToolResult {
        ...
        let host = ctx
            .secrets
            .get(ACCOUNT_SMTP_HOST)
            .map_err(|e| ToolError::Internal { message: e })?
            .ok_or_else(|| mk_cfg_err("host"))?;
        let username = ctx
            .secrets
            .get(ACCOUNT_SMTP_USERNAME)
            .map_err(|e| ToolError::Internal { message: e })?
            .ok_or_else(|| mk_cfg_err("username"))?;
        let password = ctx
            .secrets
            .get(ACCOUNT_SMTP_PASSWORD)
            .map_err(|e| ToolError::Internal { message: e })?
            .ok_or_else(|| mk_cfg_err("password"))?;
        let from = ctx
            .secrets
            .get(ACCOUNT_SMTP_FROM)
            .map_err(|e| ToolError::Internal { message: e })?
            .ok_or_else(|| mk_cfg_err("from address"))?;

        let mut builder = Message::builder()
            .from(from.parse().map_err(|e| ToolError::ValidationFailed {
                field: "from".into(),
                reason: format!("{e}"),
            })?)
            .to(to.parse().map_err(|e| ToolError::ValidationFailed {
                field: "to".into(),
                reason: format!("{e}"),
            })?)
            .subject(subject)
        ...
        if let Some(cc) = args.get("cc").and_then(|v| v.as_str()) {
            builder = builder.cc(cc.parse().map_err(|e| ToolError::ValidationFailed {
                field: "cc".into(),
                reason: format!("{e}"),
            })?);
        }
        ...
        let mailer = SmtpTransport::relay(&host)
            .map_err(|e| ToolError::Configuration {
                message: format!("SMTP relay: {e}"),
                fix_hint: "Check SMTP host in Settings → Email".into(),
            })?
            .credentials(Credentials::new(username, password))
            .build();

        mailer.send(&email).map_err(|e| ToolError::Transient {
...
impl Tool for RunCommand {
    ...
    fn description(&self) -> &'static str {
        "Runs a shell command on the local machine and streams its output. Requires user confirmation."
    }
    fn tags(&self) -> &'static [&'static str] {
        &[
            "shell",
    ...
    async fn execute(&self, args: &Value, ctx: &ToolContext) -> ToolResult {
        ...
        let output = tokio::time::timeout(std::time::Duration::from_secs(timeout), output_fut)
            .await
