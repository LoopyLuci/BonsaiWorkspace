...
mod tests {
    ...
    #[tokio::test]
    async fn test_check_rpm_blocks_at_limit() {
        let limiter = RateLimitService::new();
        assert!(limiter.check_rpm("t1", "gpt-4", 2).await.unwrap());
        assert!(limiter.check_rpm("t1", "gpt-4", 2).await.unwrap());
    ...
    #[tokio::test]
    async fn test_check_tpm_blocks_when_would_exceed() {
        let limiter = RateLimitService::new();
        assert!(limiter.check_tpm("t1", "gpt-4", 800, 1000).await.unwrap());
        // 800 + 300 > 1000, should be rejected without mutating the counter.
        assert!(!limiter.check_tpm("t1", "gpt-4", 300, 1000).await.unwrap());
        // But a smaller request that still fits should succeed.
        assert!(limiter.check_tpm("t1", "gpt-4", 100, 1000).await.unwrap());
    }

    #[tokio::test]
    async fn test_get_remaining_reflects_usage() {
        let limiter = RateLimitService::new();
        limiter.check_rpm("t1", "gpt-4", 10).await.unwrap();
        limiter.check_rpm("t1", "gpt-4", 10).await.unwrap();
        limiter.check_tpm("t1", "gpt-4", 400, 1000).await.unwrap();

        let (rpm_remaining, tpm_remaining) = limiter.get_remaining("t1", "gpt-4", 10, 1000).await.unwrap();
        assert_eq!(rpm_remaining, 8);
        assert_eq!(tpm_remaining, 600);
    }

    #[tokio::test]
    async fn test_reset_tenant_clears_counters() {
        let limiter = RateLimitService::new();
        limiter.check_rpm("t1", "gpt-4", 10).await.unwrap();
        limiter.reset_tenant("t1").await.unwrap();

        let stats = limiter.get_stats("t1", "gpt-4").await.unwrap();
