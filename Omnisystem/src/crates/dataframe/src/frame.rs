...
impl BonsaiFrame {
    ...
    pub fn from_json_columns(cols: &[(String, Vec<JsonValue>)]) -> DfResult<Self> {
        let columns: DfResult<Vec<Column>> = cols
            .iter()
            .map(|(name, vals)| json_array_to_column(name, vals))
            .collect();
    ...
    pub fn sort(&self, specs: &[SortSpec]) -> DfResult<Self> {
        ...
        let col_names: Vec<PlSmallStr> = specs
            .iter()
            .map(|s| PlSmallStr::from(s.col.as_str()))
            .collect();
    ...
    pub fn to_json_columns(&self) -> DfResult<serde_json::Map<String, JsonValue>> {
        ...
        for col_data in self.0.get_columns() {
            let arr: Vec<JsonValue> = (0..col_data.len())
                .map(|i| column_get_json(col_data, i))
                .collect();
