...
use freellmapi_provider_cohere::CohereAdapter;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    let api_key = std::env::var("COHERE_API_KEY").unwrap_or_else(|_| "unset".to_string());
    let adapter = CohereAdapter::new(api_key);
