...
use crate::{CompositionStats, SceneGraph};
use bmn_common::{
...
impl RenderTarget {
    ...
    pub fn frame_size(&self) -> usize {
        match self.pixel_format {
            ...
            PixelFormat::YUV422 => (self.width * self.height * 2) as usize,
            PixelFormat::YUV420 => {
...
impl Compositor {
    ...
    pub async fn render(&self) -> BmnResult<VideoFrame> {
        ...
        let timestamp = std::time::SystemTime::now()
            ...
            .unwrap_or_default()
            .as_millis() as u64;
        let stride = match self.config.render_target.pixel_format {
            PixelFormat::RGBA | PixelFormat::BGRA => self.config.render_target.width * 4,
            PixelFormat::P010 => self.config.render_target.width * 2,
            PixelFormat::YUV420 | PixelFormat::YUV422 | PixelFormat::NV12 => {
                self.config.render_target.width
            }
        };
        let frame = VideoFrame::new(
            timestamp,
            self.config.render_target.width,
            self.config.render_target.height,
            self.config.render_target.pixel_format,
            frame_data,
            stride,
            true,
        );
        ...
        // Update stats
        let active_layers = scene.visible_elements().len() as u32;
