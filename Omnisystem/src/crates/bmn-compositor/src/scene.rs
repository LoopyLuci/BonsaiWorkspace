...
use crate::{ElementId, Layer, BlendMode, Effect};
...
pub struct Scene {
    ...
    /// Scene-level effects (transitions, whole-scene color grading, ...) as
    /// opposed to [`SceneElement::effects`], which apply to a single element.
...
impl Scene {
    ...
    /// Attach a scene-level effect (e.g. a fade-in transition)
    pub fn add_effect(mut self, effect: crate::Effect) -> Self {
        self.effects.push(effect);
        self
    }
