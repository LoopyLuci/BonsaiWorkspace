...
/// Build a wire frame: `[tag: u8][length: u32 LE][payload]`.
///
...
mod tests {
    ...
    fn test_build_frame_layout() {
        let frame = build_frame(TAG_CHUNK, b"hello");
        // tag byte
        assert_eq!(frame[0], TAG_CHUNK);
    ...
    fn test_build_frame_empty_payload() {
        let frame = build_frame(TAG_ACK, &[]);
        assert_eq!(frame, vec![TAG_ACK, 0, 0, 0, 0]);
    }

    #[test]
    fn test_build_frame_nack_tag_and_length() {
        let payload = 42u64.to_le_bytes();
        let frame = build_frame(TAG_NACK, &payload);
        assert_eq!(frame[0], TAG_NACK);
