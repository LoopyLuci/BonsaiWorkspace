...
mod tests {
    ...
    #[test]
    fn test_swarm_chunk_req_serde_roundtrip() {
        let req = SwarmChunkReq {
            data: vec![1, 2, 3, 4, 5],
        };
        let json = serde_json::to_string(&req).unwrap();
        let back: SwarmChunkReq = serde_json::from_str(&json).unwrap();
        assert_eq!(back.data, req.data);

        let encoded = bincode::serialize(&req).unwrap();
        let decoded: SwarmChunkReq = bincode::deserialize(&encoded).unwrap();
        assert_eq!(decoded.data, req.data);
    }

    #[test]
    fn test_swarm_chunk_resp_serde_roundtrip() {
        let resp = SwarmChunkResp { gsn: 42, ok: true };
        let json = serde_json::to_string(&resp).unwrap();
        let back: SwarmChunkResp = serde_json::from_str(&json).unwrap();
        assert_eq!(back.gsn, 42);
        assert!(back.ok);

        let encoded = bincode::serialize(&resp).unwrap();
        let decoded: SwarmChunkResp = bincode::deserialize(&encoded).unwrap();
        assert_eq!(decoded.gsn, resp.gsn);
        assert_eq!(decoded.ok, resp.ok);
    }

    #[test]
    fn test_extract_peer_id_present() {
        let peer_id = PeerId::random();
        let addr: Multiaddr = format!("/ip4/127.0.0.1/tcp/7001/p2p/{peer_id}")
            .parse()
            .unwrap();
    ...
    #[test]
    fn test_extract_peer_id_absent() {
