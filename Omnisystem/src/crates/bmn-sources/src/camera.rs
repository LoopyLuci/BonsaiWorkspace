...
impl CameraSource {
    ...
    fn create_test_frame(&mut self) -> VideoFrame {
        ...
        let format = self.capture_config.pixel_format;

        let frame_size = Self::frame_size_for(format, width, height);
        let data = vec![128u8; frame_size]; // Gray test pattern
        let stride = (frame_size / height.max(1)) as u32;
    ...
    /// Bytes needed for a full frame of `width`x`height` at `format`.
    fn frame_size_for(format: PixelFormat, width: usize, height: usize) -> usize {
        match format {
            PixelFormat::RGBA | PixelFormat::BGRA => width * height * 4,
            PixelFormat::YUV422 => width * height * 2,
            ...
            PixelFormat::P010 => {
                // 10-bit 4:2:0, stored in 16-bit samples: 2 bytes/px (Y) + 1 byte/px (UV)
