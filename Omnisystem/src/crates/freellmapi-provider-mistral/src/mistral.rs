...
mod tests {
    ...
    fn test_provider_name() {
        let adapter = MistralAdapter::new("test-key".to_string());
    ...
    async fn test_translate_request_maps_fields() {
        ...
        assert_eq!(req.provider, "mistral");
    ...
    #[tokio::test]
    async fn test_get_supported_models_includes_mistral_small() {
        let adapter = MistralAdapter::new("test-key".to_string());
        let models = adapter.get_supported_models().await.unwrap();
        assert!(models.contains(&"mistral-small".to_string()));
    }
}
