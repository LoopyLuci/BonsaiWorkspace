...
use freellmapi_provider_mistral::MistralAdapter;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    let api_key = std::env::var("MISTRAL_API_KEY").unwrap_or_else(|_| "unset".to_string());
    let adapter = MistralAdapter::new(api_key);
