//! Universal Language Layer Wrapper for app-manager-repository
//!
//! Registers app-manager-repository with the Universal Language Layer (ULL)
...
pub async fn register_with_ull(bridge: &LanguageBridge) -> UllResult<()> {
    log::info!("Registering app-manager-repository with ULL");
    bridge.register_module("app-manager-repository", Language::Rust)?;
    Ok(())
}
