...
pub fn map2(a: &NdArray, b: &NdArray, f: impl Fn(f64, f64) -> f64) -> Result<NdArray, ArrayError> {
    if a.is_scalar() {
        ...
        return Ok(NdArray {
            shape: b.shape.clone(),
            data: b.data.iter().map(|&x| f(av, x)).collect(),
        });
    }
    if b.is_scalar() {
        let bv = b.data[0];
        return Ok(NdArray {
            shape: a.shape.clone(),
            data: a.data.iter().map(|&x| f(x, bv)).collect(),
        });
    }
    if a.shape != b.shape {
...
pub fn add(a: &NdArray, b: &NdArray) -> Result<NdArray, ArrayError> {
    map2(a, b, |x, y| x + y)
}
pub fn sub(a: &NdArray, b: &NdArray) -> Result<NdArray, ArrayError> {
    map2(a, b, |x, y| x - y)
}
pub fn mul(a: &NdArray, b: &NdArray) -> Result<NdArray, ArrayError> {
    map2(a, b, |x, y| x * y)
}
pub fn div(a: &NdArray, b: &NdArray) -> Result<NdArray, ArrayError> {
    map2(a, b, |x, y| x / y)
}
pub fn rem(a: &NdArray, b: &NdArray) -> Result<NdArray, ArrayError> {
    map2(a, b, |x, y| x % y)
}
pub fn pow(a: &NdArray, b: &NdArray) -> Result<NdArray, ArrayError> {
    map2(a, b, |x, y| x.powf(y))
}
pub fn neg(a: &NdArray) -> NdArray {
    map1(a, |x| -x)
}
...
pub fn signum(a: &NdArray) -> NdArray {
    map1(a, f64::signum)
}
...
pub fn eq(a: &NdArray, b: &NdArray) -> Result<NdArray, ArrayError> {
    map2(a, b, |x, y| if x == y { 1.0 } else { 0.0 })
}
pub fn ne(a: &NdArray, b: &NdArray) -> Result<NdArray, ArrayError> {
    map2(a, b, |x, y| if x != y { 1.0 } else { 0.0 })
}
pub fn lt(a: &NdArray, b: &NdArray) -> Result<NdArray, ArrayError> {
    map2(a, b, |x, y| if x < y { 1.0 } else { 0.0 })
}
pub fn le(a: &NdArray, b: &NdArray) -> Result<NdArray, ArrayError> {
    map2(a, b, |x, y| if x <= y { 1.0 } else { 0.0 })
}
pub fn gt(a: &NdArray, b: &NdArray) -> Result<NdArray, ArrayError> {
    map2(a, b, |x, y| if x > y { 1.0 } else { 0.0 })
}
pub fn ge(a: &NdArray, b: &NdArray) -> Result<NdArray, ArrayError> {
    map2(a, b, |x, y| if x >= y { 1.0 } else { 0.0 })
}
...
pub fn reduce(a: &NdArray, f: impl Fn(f64, f64) -> f64, identity: f64) -> NdArray {
    ...
    if cols == 0 {
        return NdArray::scalar(identity);
    }
...
pub fn sum(a: &NdArray) -> NdArray {
    reduce(a, |x, y| x + y, 0.0)
}
pub fn product(a: &NdArray) -> NdArray {
    reduce(a, |x, y| x * y, 1.0)
}
pub fn max_reduce(a: &NdArray) -> NdArray {
    reduce(a, f64::max, f64::NEG_INFINITY)
}
pub fn min_reduce(a: &NdArray) -> NdArray {
    reduce(a, f64::min, f64::INFINITY)
}
...
pub fn scan_sum(a: &NdArray) -> NdArray {
    scan(a, |x, y| x + y)
}
pub fn scan_product(a: &NdArray) -> NdArray {
    scan(a, |x, y| x * y)
}
...
pub fn inner_product(
    ...
    if a.rank() != 2 || b.rank() != 2 {
        return Err(ArrayError::DomainError(
            "inner product requires rank-2 arrays".into(),
        ));
    }
    ...
    for i in 0..m {
        for j in 0..n {
            ...
            let dot = row
                ...
                .zip(col.iter())
                .skip(1)
                .fold(f_combine(row[0], col[0]), |acc, (&x, &y)| {
                    f_agg(acc, f_combine(x, y))
                });
            result[i * n + j] = dot;
        }
    }
