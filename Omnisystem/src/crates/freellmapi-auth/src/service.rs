...
mod tests {
    ...
    #[tokio::test]
    async fn test_jwt_issue_and_validate_roundtrip() {
        let auth = AuthService::new(Arc::new(MockStorage), "test-secret").unwrap();

        let token = auth.issue_jwt("tenant-1", vec!["chat", "models"]).await.unwrap();
        let (subject, scopes) = auth.validate_jwt(&token).await.unwrap();

        assert_eq!(subject, "tenant-1");
        assert_eq!(scopes, vec!["chat".to_string(), "models".to_string()]);
    }

    #[tokio::test]
    async fn test_jwt_rejects_tampered_signature() {
        let auth = AuthService::new(Arc::new(MockStorage), "test-secret").unwrap();
        let token = auth.issue_jwt("tenant-1", vec!["chat"]).await.unwrap();

        let mut parts: Vec<&str> = token.split('.').collect();
        let tampered_sig = if parts[2].starts_with('0') { "1" } else { "0" };
        let tampered = format!("{}.{}.{}{}", parts[0], parts[1], tampered_sig, &parts[2][1..]);
        parts.clear();

        assert!(auth.validate_jwt(&tampered).await.is_err());
    }

    #[tokio::test]
    async fn test_jwt_rejects_wrong_secret() {
        let issuer = AuthService::new(Arc::new(MockStorage), "secret-a").unwrap();
        let verifier = AuthService::new(Arc::new(MockStorage), "secret-b").unwrap();

        let token = issuer.issue_jwt("tenant-1", vec!["chat"]).await.unwrap();
        assert!(verifier.validate_jwt(&token).await.is_err());
    }

    #[tokio::test]
    async fn test_validate_api_key_looks_up_tenant() {
        let auth = AuthService::new(Arc::new(MockStorage), "test-secret").unwrap();
        let tenant = auth.validate_api_key("any-raw-key").await.unwrap();
