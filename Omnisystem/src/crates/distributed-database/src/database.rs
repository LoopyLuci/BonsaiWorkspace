...
mod tests {
    ...
    #[tokio::test]
    async fn test_create_shard_unknown_table_errors() {
        let db = DistributedDatabase::new();
        let result = db.create_shard(Uuid::new_v4(), 0, ("a".to_string(), "z".to_string())).await;
        assert!(matches!(result, Err(DatabaseError::TableNotFound)));
    }

    #[tokio::test]
    async fn test_create_partition_unknown_shard_errors() {
        let db = DistributedDatabase::new();
        let result = db.create_partition(Uuid::new_v4(), "a", "m").await;
        assert!(matches!(result, Err(DatabaseError::ShardNotFound)));
    }

    #[tokio::test]
    async fn test_plan_query_no_shards_errors() {
        let db = DistributedDatabase::new();
        let schema = TableSchema { columns: vec![], primary_key: "id".to_string() };
        let table = db.create_table("empty_table", schema, 1).await.unwrap();

        let result = db.plan_query(table.table_id, ConsistencyLevel::Eventual).await;
        assert!(matches!(result, Err(DatabaseError::RoutingFailed)));
    }

    #[tokio::test]
    async fn test_create_index_unknown_table_errors() {
        let db = DistributedDatabase::new();
        let result = db.create_index(Uuid::new_v4(), "email", IndexType::BTree).await;
        assert!(matches!(result, Err(DatabaseError::TableNotFound)));
    }
}
