...
mod tests {
    ...
    #[test]
    fn test_snapshot_missing_vault_errors() {
        let result = snapshot_vault(u64::MAX);
        assert!(matches!(result, Err(KernelError::VaultNotFound(id)) if id == u64::MAX));
    }

    #[test]
    fn test_destroy_missing_vault_errors() {
        let result = destroy_vault(u64::MAX - 1);
        assert!(matches!(result, Err(KernelError::VaultNotFound(id)) if id == u64::MAX - 1));
    }

    #[test]
    fn test_destroyed_vault_cannot_be_destroyed_twice() {
        let caps = CapabilityTable::new();
        let vault_id = create_vault("double_destroy", caps).unwrap();
        assert!(destroy_vault(vault_id).is_ok());
        assert!(matches!(
            destroy_vault(vault_id),
            Err(KernelError::VaultNotFound(id)) if id == vault_id
        ));
    }
}
