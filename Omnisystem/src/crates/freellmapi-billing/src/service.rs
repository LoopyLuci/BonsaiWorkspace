...
mod tests {
    ...
    struct MockStorage {
        budget: f64,
        spent: f64,
        logged: std::sync::Mutex<Vec<RequestLog>>,
    }

    #[async_trait]
    impl StorageRepository for MockStorage {
        async fn create_tenant(&self, _: &Tenant) -> Result<()> {
            Ok(())
        }
        async fn get_tenant(&self, id: &str) -> Result<Tenant> {
            Ok(Tenant {
                ...
                tier: "pro".to_string(),
                monthly_budget_usd: self.budget,
                created_at: 0,
            })
        }
        async fn create_api_key(&self, _: &ApiKey) -> Result<()> {
            Ok(())
        }
        ...
        async fn get_request_logs(&self, _: &str, _: u64, _: u64, _: u32) -> Result<Vec<RequestLog>> {
            Ok(vec![])
        }
        async fn get_tenant_costs(&self, _: &str, _: u32) -> Result<f64> {
            Ok(self.spent)
        }
        async fn create_webhook(&self, _: &Webhook) -> Result<()> {
            Ok(())
        }
        async fn get_webhooks(&self, _: &str) -> Result<Vec<Webhook>> {
            Ok(vec![])
        }
    }

    #[tokio::test]
    async fn test_check_budget_within_limit() {
        let storage = Arc::new(MockStorage { budget: 100.0, spent: 50.0, logged: Default::default() });
        let billing = BillingService::new(storage).await.unwrap();
        assert!(billing.check_budget("tenant-1", 10.0).await.unwrap());
    }

    #[tokio::test]
    async fn test_check_budget_exceeded() {
        let storage = Arc::new(MockStorage { budget: 100.0, spent: 95.0, logged: Default::default() });
        let billing = BillingService::new(storage).await.unwrap();
        assert!(!billing.check_budget("tenant-1", 10.0).await.unwrap());
    }

    #[tokio::test]
    async fn test_record_usage_logs_correct_cost() {
        let storage = Arc::new(MockStorage { budget: 100.0, spent: 0.0, logged: Default::default() });
        let billing = BillingService::new(storage.clone()).await.unwrap();

        billing
            .record_usage("tenant-1", "gpt-4", "openai", 1000, 500, 250)
            .await
            .unwrap();

        let logged = storage.logged.lock().unwrap();
        assert_eq!(logged.len(), 1);
        assert!((logged[0].cost_usd - 0.06).abs() < 0.0001); // (1000*0.03 + 500*0.06)/1000
        assert_eq!(logged[0].tenant_id, "tenant-1");
        assert_eq!(logged[0].provider, "openai");
    }

    #[tokio::test]
    async fn test_forecast_monthly_cost_projects_from_weekly_average() {
        let storage = Arc::new(MockStorage { budget: 1000.0, spent: 70.0, logged: Default::default() });
        let billing = BillingService::new(storage).await.unwrap();

        let forecast = billing.forecast_monthly_cost("tenant-1").await.unwrap();
        assert!((forecast - 300.0).abs() < 0.001); // (70/7)*30
    }

    #[tokio::test]
    async fn test_warn_if_exceeds_threshold() {
        let storage = Arc::new(MockStorage { budget: 100.0, spent: 85.0, logged: Default::default() });
        let billing = BillingService::new(storage).await.unwrap();

        assert!(billing.warn_if_exceeds_threshold("tenant-1", 80.0).await.unwrap());
        assert!(!billing.warn_if_exceeds_threshold("tenant-1", 90.0).await.unwrap());
    }
}
