...
pub fn lex(src: &str) -> LexResult<Vec<Spanned>> {
    ...
    while i < chars.len() {
            ...
            while i < chars.len() && chars[i] != '"' {
                if chars[i] == '\\' {
                    ...
                    match chars.get(i) {
                        ...
                        other => {
                            return Err(LexError(format!("unknown escape \\{:?}", other), line))
                        }
                    }
                } else {
                    if chars[i] == '\n' {
        ...
        // numbers
        if c.is_ascii_digit()
            || (c == '-' && chars.get(i + 1).map_or(false, |x| x.is_ascii_digit()))
        {
            let start = i;
            let start_col = col;
            if c == '-' {
                ...
                && chars.get(i + 1).map_or(false, |x| x.is_ascii_digit())
            {
                ...
                let f: f64 = s
                    .parse()
                    .map_err(|_| LexError(format!("bad float: {s}"), line))?;
                out.push(Spanned {
                    token: Token::Float(f),
                    line,
                    col: start_col,
                ...
                let n: i64 = s
                    .parse()
                    .map_err(|_| LexError(format!("bad int: {s}"), line))?;
                out.push(Spanned {
                    token: Token::Int(n),
                    line,
                    col: start_col,
        ...
        macro_rules! try2 {
            ($next:expr, $tok2:expr, $tok1:expr) => {
                if chars.get(i + 1) == Some(&$next) {
                    out.push(spanned!($tok2));
                    i += 2;
                    col += 2;
                    continue;
                } else {
                    out.push(spanned!($tok1));
                    i += 1;
                    col += 1;
                    continue;
                }
        ...
        match c {
            ...
            '=' => {
                if chars.get(i + 1) == Some(&'>') {
                    out.push(spanned!(Token::FatArrow));
                    i += 2;
                    col += 2;
                    continue;
                }
            ...
            '|' => {
                if chars.get(i + 1) == Some(&'>') {
                    out.push(spanned!(Token::Pipe));
                    i += 2;
                    col += 2;
                    continue;
                }
                if chars.get(i + 1) == Some(&'|') {
                    out.push(spanned!(Token::Or));
                    i += 2;
                    col += 2;
                    continue;
                }
            ...
            '&' => {
                if chars.get(i + 1) == Some(&'&') {
                    out.push(spanned!(Token::And));
                    i += 2;
                    col += 2;
                    continue;
                }
                out.push(spanned!(Token::Bang));
                i += 1;
                col += 1;
                continue; // treat lone & as bang (not used)
            }
            '.' => {
                if chars.get(i + 1) == Some(&'.') {
                    out.push(spanned!(Token::DotDot));
                    i += 2;
                    col += 2;
                    continue;
                }
                out.push(spanned!(Token::Dot));
                i += 1;
                col += 1;
                continue;
            }
            '*' => {
                out.push(spanned!(Token::Star));
                i += 1;
                col += 1;
                continue;
            }
            '/' => {
                out.push(spanned!(Token::Slash));
                i += 1;
                col += 1;
                continue;
            }
            '%' => {
                out.push(spanned!(Token::Percent));
                i += 1;
                col += 1;
                continue;
            }
            '(' => {
                out.push(spanned!(Token::LParen));
                i += 1;
                col += 1;
                continue;
            }
            ')' => {
                out.push(spanned!(Token::RParen));
                i += 1;
                col += 1;
                continue;
            }
            '{' => {
                out.push(spanned!(Token::LBrace));
                i += 1;
                col += 1;
                continue;
            }
            '}' => {
                out.push(spanned!(Token::RBrace));
                i += 1;
                col += 1;
                continue;
            }
            '[' => {
                out.push(spanned!(Token::LBracket));
                i += 1;
                col += 1;
                continue;
            }
            ']' => {
                out.push(spanned!(Token::RBracket));
                i += 1;
                col += 1;
                continue;
            }
            ',' => {
                out.push(spanned!(Token::Comma));
                i += 1;
                col += 1;
                continue;
            }
            ':' => {
                out.push(spanned!(Token::Colon));
                i += 1;
                col += 1;
                continue;
            }
            ';' => {
                out.push(spanned!(Token::Semicolon));
                i += 1;
                col += 1;
                continue;
            }
            '#' => {
                out.push(spanned!(Token::Hash));
                i += 1;
                col += 1;
                continue;
            }
            ...
            '?' => {
                out.push(spanned!(Token::Question));
                i += 1;
                col += 1;
                continue;
            }
