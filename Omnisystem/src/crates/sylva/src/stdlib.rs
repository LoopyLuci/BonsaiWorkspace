...
use crate::vm::{SylvaValue, SylvaVm, VmError, VmResult};
use std::sync::Arc;
...
pub fn register_stdlib(vm: &mut SylvaVm) {
    ...
    native(vm, "to_float", |args| match args.first() {
    ...
    // ── Strings ───────────────────────────────────────────────────────────────
    native(vm, "len", |args| match args.first() {
    ...
    native(vm, "split", |args| {
        ...
        Ok(SylvaValue::List(
            s.split(sep.as_str())
                .map(|p| SylvaValue::Str(p.into()))
                .collect(),
        ))
    });

    native(vm, "join", |args| match (args.get(0), args.get(1)) {
    ...
    native(vm, "starts_with", |args| match (args.get(0), args.get(1)) {
    ...
    native(vm, "ends_with", |args| match (args.get(0), args.get(1)) {
        (Some(SylvaValue::Str(s)), Some(SylvaValue::Str(p))) => {
            Ok(SylvaValue::Bool(s.ends_with(p.as_str())))
        }
        _ => Err(VmError::TypeError("ends_with(str, suffix)".into())),
    });
    native(vm, "contains", |args| match (args.get(0), args.get(1)) {
        (Some(SylvaValue::Str(s)), Some(SylvaValue::Str(p))) => {
            Ok(SylvaValue::Bool(s.contains(p.as_str())))
        }
        (Some(SylvaValue::List(l)), val) => {
            if let Some(val) = val {
                Ok(SylvaValue::Bool(l.contains(val)))
            } else {
                Err(VmError::TypeError("contains: missing argument".into()))
            }
        }
    ...
    native(vm, "push", |mut args| {
        if args.len() < 2 {
            return Err(VmError::TypeError("push(list, item)".into()));
        }
        let item = args.remove(1);
        match args.remove(0) {
            SylvaValue::List(mut l) => {
                l.push(item);
                Ok(SylvaValue::List(l))
            }
            other => Err(VmError::TypeError(format!(
    ...
    native(vm, "pop", |args| match args.into_iter().next() {
    ...
    native(vm, "reverse", |args| match args.into_iter().next() {
        Some(SylvaValue::List(mut l)) => {
            l.reverse();
            Ok(SylvaValue::List(l))
        }
    ...
    native(vm, "range", |args| match (args.get(0), args.get(1)) {
        (Some(SylvaValue::Int(start)), Some(SylvaValue::Int(end))) => Ok(SylvaValue::List(
            (*start..*end).map(SylvaValue::Int).collect(),
        )),
    ...
    native(vm, "map", |_args| {
        Err(VmError::Runtime(
            "map(list, fn) requires calling the VM recursively — use for loop instead".into(),
        ))
    });

    native(vm, "filter", |_args| {
        Err(VmError::Runtime(
            "filter(list, fn) requires calling the VM recursively — use for loop instead".into(),
        ))
    });

    native(vm, "zip", |args| match (args.get(0), args.get(1)) {
    ...
    // ── Math ──────────────────────────────────────────────────────────────────
    native(vm, "abs", |args| match args.first() {
    ...
    native(vm, "min", |args| match (args.get(0), args.get(1)) {
        (Some(SylvaValue::Int(a)), Some(SylvaValue::Int(b))) => Ok(SylvaValue::Int(*a.min(b))),
        (Some(SylvaValue::Float(a)), Some(SylvaValue::Float(b))) => {
            Ok(SylvaValue::Float(a.min(*b)))
        }
        _ => Err(VmError::TypeError("min(a, b)".into())),
    });
    native(vm, "max", |args| match (args.get(0), args.get(1)) {
        (Some(SylvaValue::Int(a)), Some(SylvaValue::Int(b))) => Ok(SylvaValue::Int(*a.max(b))),
        (Some(SylvaValue::Float(a)), Some(SylvaValue::Float(b))) => {
            Ok(SylvaValue::Float(a.max(*b)))
        }
    ...
    native(vm, "from_json", |args| match args.first() {
        Some(SylvaValue::Str(s)) => {
            let v: serde_json::Value =
                serde_json::from_str(s).map_err(|e| VmError::Runtime(format!("from_json: {e}")))?;
    ...
    // ── Type checks ───────────────────────────────────────────────────────────
    native(vm, "is_nil", |args| {
        Ok(SylvaValue::Bool(matches!(
            args.first(),
            Some(SylvaValue::Nil)
        )))
    });
    native(vm, "is_bool", |args| {
        Ok(SylvaValue::Bool(matches!(
            args.first(),
            Some(SylvaValue::Bool(_))
        )))
    });
    native(vm, "is_int", |args| {
        Ok(SylvaValue::Bool(matches!(
            args.first(),
            Some(SylvaValue::Int(_))
        )))
    });
    native(vm, "is_float", |args| {
        Ok(SylvaValue::Bool(matches!(
            args.first(),
            Some(SylvaValue::Float(_))
        )))
    });
    native(vm, "is_str", |args| {
        Ok(SylvaValue::Bool(matches!(
            args.first(),
            Some(SylvaValue::Str(_))
        )))
    });
    native(vm, "is_list", |args| {
        Ok(SylvaValue::Bool(matches!(
            args.first(),
            Some(SylvaValue::List(_))
        )))
    });
    native(vm, "is_fn", |args| {
        Ok(SylvaValue::Bool(matches!(
            args.first(),
            Some(SylvaValue::Closure(_) | SylvaValue::Native(_))
        )))
    });

    // ── Map operations ────────────────────────────────────────────────────────
    native(vm, "keys", |args| match args.first() {
        Some(SylvaValue::Map(m)) => {
            Ok(SylvaValue::List(m.iter().map(|(k, _)| k.clone()).collect()))
        }
        _ => Err(VmError::TypeError("keys(map)".into())),
    });
    native(vm, "values", |args| match args.first() {
        Some(SylvaValue::Map(m)) => {
            Ok(SylvaValue::List(m.iter().map(|(_, v)| v.clone()).collect()))
        }
        _ => Err(VmError::TypeError("values(map)".into())),
    });
    native(vm, "has_key", |args| match (args.get(0), args.get(1)) {
...
where
    F: Fn(Vec<SylvaValue>) -> VmResult<SylvaValue> + Send + Sync + 'static,
{
    vm.env
        .set_global(name.into(), SylvaValue::Native(Arc::new(f)));
}
