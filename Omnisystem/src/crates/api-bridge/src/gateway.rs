...
pub async fn run(host: &str, port: u16) -> Result<()> {
    ...
    // Note: the archived crate also spawned a gRPC gateway here
    // (protocol::grpc::run_grpc_gateway) built from proto/bridge.proto via
    // tonic::include_proto!. That module needed a build.rs invoking
    // `protoc`, which is not available in this environment, so it was left
    // out of this restoration; the REST/WebSocket/JSON-RPC gateway below is
    // unaffected.
...
async fn forward_to_backend(
    ...
    // Note: the archived crate also handled a "td://<peer>/<service>" URL
    // scheme here, forwarded over the TransferDaemon P2P mesh via
    // TransferDaemonAdapter. That adapter depends on the `transfer-client`
    // crate, which is still archived and out of scope for this
    // restoration, and no route currently in `routing::route_to_service`
    // produces a "td://" URL, so this is a clean removal rather than a
    // behavior change.
...
mod tests {
    ...
    fn test_state() -> BridgeState {
        BridgeState {
            client: reqwest::Client::new(),
            telemetry: TelemetryBus::new(),
            mcp_jsonrpc_url: "http://127.0.0.1:0/mcp".to_string(),
            rate_limits: Arc::new(Mutex::new(HashMap::new())),
            circuits: Arc::new(Mutex::new(HashMap::new())),
        }
    }

    fn backend(url: &str) -> BackendInstance {
        BackendInstance {
    ...
    #[tokio::test]
    async fn test_rate_limit_allows_up_to_default_then_blocks() {
        let state = test_state();
        for _ in 0..DEFAULT_RATE_LIMIT {
            assert!(check_rate_limit(&state, "subject-a".to_string()).await.is_ok());
        }
        // One more over the limit within the same window should be rejected.
        assert_eq!(
            check_rate_limit(&state, "subject-a".to_string()).await,
    ...
    async fn test_rate_limit_is_per_subject() {
        let state = test_state();
        for _ in 0..DEFAULT_RATE_LIMIT {
            check_rate_limit(&state, "subject-a".to_string()).await.unwrap();
        }
        // A different subject has its own independent bucket.
        assert!(check_rate_limit(&state, "subject-b".to_string()).await.is_ok());
    }

    #[tokio::test]
    async fn test_circuit_opens_after_threshold_failures_and_recovers_on_success() {
        let state = test_state();
        let be = backend("http://backend-under-test");

        assert!(!is_circuit_open(&state, &be).await);

        for _ in 0..CIRCUIT_FAILURE_THRESHOLD {
            record_failure(&state, &be).await;
        }
        assert!(is_circuit_open(&state, &be).await);

        record_success(&state, &be).await;
        assert!(!is_circuit_open(&state, &be).await);
    }
    ...
    async fn test_circuit_stays_closed_below_threshold() {
        let state = test_state();
        let be = backend("http://another-backend");
    ...
    #[tokio::test]
    async fn test_forward_to_discovery_backend_returns_peers() {
        let state = test_state();
        let be = backend("memory://discovery");

        let result = forward_to_backend(&state, &be, "trace-1", &serde_json::json!({}))
            .await
            .unwrap();
    ...
    #[test]
    fn test_authorize_rejects_missing_credentials() {
        let headers = HeaderMap::new();
        let result = authorize(&headers, "ApiCap:inference");
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().0, StatusCode::UNAUTHORIZED);
    }

    #[test]
    fn test_authorize_rejects_insufficient_capability() {
        let mut headers = HeaderMap::new();
        let token = CapabilityToken {
            subject: Some("user".to_string()),
            capabilities: vec!["ApiCap:discovery".to_string()],
            exp: None,
            sig: None,
        };
        ...
        headers.insert(
            "Authorization",
            axum::http::HeaderValue::from_str(&format!("Bearer {json}")).unwrap(),
        );
    ...
    #[test]
    fn test_authorize_accepts_wildcard_capability() {
        let mut headers = HeaderMap::new();
        let token = CapabilityToken {
        ...
        assert!(authorize(&headers, "ApiCap:inference").is_ok());
    }
}
