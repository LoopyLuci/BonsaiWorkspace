...
impl AxiomVerifier {
    ...
    pub fn verify_disclaimer_present(response: &str, has_medical_content: bool) -> bool {
        ...
        // Normalize whitespace (collapse newlines/extra spaces) before matching,
        // so a disclaimer that wraps across lines still matches.
...
mod tests {
    ...
    #[test]
    fn test_disclaimer_not_required_without_medical_content() {
        assert!(AxiomVerifier::verify_disclaimer_present(
            "Sure, let's chat about your day.",
            false
        ));
    }

    #[test]
    fn test_disclaimer_detected_when_present() {
        let response = "This is general information and not a substitute for professional advice.";
        assert!(AxiomVerifier::verify_disclaimer_present(response, true));
    }

    #[test]
    fn test_disclaimer_missing_is_detected() {
        let response = "Here is some general information about symptoms.";
        assert!(!AxiomVerifier::verify_disclaimer_present(response, true));
    }

    #[test]
    fn test_disclaimer_matches_across_wrapped_whitespace() {
        // A disclaimer that wraps across a newline should still be found once
        // whitespace is normalized.
        let response = "This is general information and not a substitute\nfor professional advice, please consult a doctor.";
        assert!(AxiomVerifier::verify_disclaimer_present(response, true));
    }
}
