...
mod tests {
    ...
    async fn test_translate_request_maps_fields() {
        ...
        assert_eq!(req.provider, "huggingface");
        assert_eq!(req.model, "mistralai/Mistral-7B-Instruct-v0.1");
    ...
    #[tokio::test]
    async fn test_get_supported_models_includes_falcon() {
        let adapter = HuggingFaceAdapter::new("test-key".to_string());
        let models = adapter.get_supported_models().await.unwrap();
        assert!(models.contains(&"tiiuae/falcon-7b-instruct".to_string()));
    }
}
