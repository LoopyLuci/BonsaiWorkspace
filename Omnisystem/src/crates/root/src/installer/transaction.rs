...
mod tests {
    ...
    fn build_sample_zip(zip_path: &Path) {
    ...
    #[test]
    fn extract_zip_recreates_files_and_directories() {
        let dir = TempDir::new().unwrap();
        ...
        assert_eq!(readme, "hello from zip");

        let nested = fs::read_to_string(extract_dir.join("nested").join("inner.txt")).unwrap();
        assert_eq!(nested, "nested content");
    }

    #[test]
    fn commit_moves_staged_files_into_destination() {
        let staging_root = TempDir::new().unwrap();
        let dest_root = TempDir::new().unwrap();

        let src = staging_root.path().join("component-a");
        fs::create_dir_all(&src).unwrap();
        fs::write(src.join("file.txt"), b"payload").unwrap();

        let dst = dest_root.path().join("component-a");

        // Build a Transaction without going through `new()` (which touches
        // the real user rollback directory) by constructing the fields
        // directly via a throwaway staging TempDir just to satisfy the type.
        let throwaway_staging = TempDir::new().unwrap();
        let tx = Transaction {
            staging_dir: throwaway_staging,
            commit_plan: vec![(src.clone(), dst.clone())],
            rollback_snapshot: dest_root.path().join("rollback-unused"),
        };

        tx.commit().unwrap();

        assert!(dst.join("file.txt").exists());
        assert!(!src.exists(), "staged source should have been moved, not copied");
    }
}
