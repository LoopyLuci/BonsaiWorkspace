...
mod tests {
    use super::*;
    use crate::panels::{chat_panel::ChatPanel, files_panel::FilesPanel};
    use std::sync::Arc;
    use tokio::sync::Mutex;

    async fn offline_client() -> Arc<Mutex<DaemonClient>> {
        // Port 1 is a reserved TCP port that is never listened on; loopback
        // connections to it fail (near-)instantly with connection refused,
        // exercising DaemonClient's offline fallback without needing a
        // real daemon or network access.
        let (client, _rx) = DaemonClient::connect("127.0.0.1".to_string(), 1, String::new())
            .await
            .expect("connect() should fall back to offline mode instead of erroring");
        Arc::new(Mutex::new(client))
    }

    async fn test_app(panels: Vec<Box<dyn Panel>>, active_panel: PanelId) -> App {
        App {
            client: offline_client().await,
            active_panel,
            panels,
            mode: Mode::Normal,
            command_buf: String::new(),
    ...
    #[tokio::test]
    async fn offline_connect_reports_disconnected() {
        let client = offline_client().await;
        assert!(!client.lock().await.is_connected());
    }

    #[tokio::test]
    async fn quit_command_sets_quit_sentinel() {
        let mut app = test_app(vec![Box::new(ChatPanel::new())], PanelId::Chat).await;
        app.handle_command("quit");
        assert_eq!(app.status_msg, "__QUIT__");
    }

    #[tokio::test]
    async fn theme_command_switches_between_light_and_dark() {
        let mut app = test_app(vec![Box::new(ChatPanel::new())], PanelId::Chat).await;
    ...
    #[tokio::test]
    async fn unknown_command_delegates_to_active_panel() {
        let mut app = test_app(vec![Box::new(ChatPanel::new())], PanelId::Chat).await;
        app.handle_command("send hello world");
        assert_eq!(app.status_msg, "Message sent");
    }

    #[tokio::test]
    async fn panel_metas_reports_all_registered_panels() {
        let app = test_app(
            vec![Box::new(ChatPanel::new()), Box::new(FilesPanel::new())],
            PanelId::Chat,
        )
        .await;
        let metas = app.panel_metas();
        assert_eq!(metas.len(), 2);
        assert_eq!(metas[0].name, "Chat");
        assert_eq!(metas[1].name, "Files");
    }

    #[tokio::test]
    async fn active_panel_name_reflects_active_panel() {
        let app = test_app(
            vec![Box::new(ChatPanel::new()), Box::new(FilesPanel::new())],
            PanelId::Files,
        )
    ...
    #[tokio::test]
    async fn set_active_by_index_updates_active_panel() {
        let mut app = test_app(
            vec![Box::new(ChatPanel::new()), Box::new(FilesPanel::new())],
            PanelId::Chat,
        )
        .await;
        app.set_active_by_index(1);
        assert_eq!(app.active_panel, PanelId::Files);
    }
}
