...
mod tests {
    ...
    #[test]
    fn new_panel_has_welcome_message() {
        let panel = ChatPanel::new();
        assert_eq!(panel.messages.len(), 1);
        assert_eq!(panel.messages[0].role, "system");
    }

    #[test]
    fn run_command_send_appends_user_message() {
        let mut panel = ChatPanel::new();
        let result = panel.run_command("send", &["hello", "world"]);
        assert_eq!(result, Some("Message sent".to_string()));
        assert_eq!(panel.messages.len(), 2);
        assert_eq!(panel.messages[1].role, "user");
        assert_eq!(panel.messages[1].content, "hello world");
    }

    #[test]
    fn insert_mode_enter_pushes_message_and_clears_input() {
        let mut panel = ChatPanel::new();
        let mut mode = Mode::Insert;
        panel.input = "hi there".to_string();
        let consumed = panel.handle_key(key(KeyCode::Enter), &mut mode);
        assert!(consumed);
        assert_eq!(panel.messages.len(), 2);
    ...
    fn normal_mode_i_switches_to_insert() {
        ...
        let consumed = panel.handle_key(key(KeyCode::Char('i')), &mut mode);
        assert!(consumed);
        assert_eq!(mode, Mode::Insert);
    }

    #[test]
    fn daemon_event_appends_assistant_message() {
        let mut panel = ChatPanel::new();
        panel.handle_daemon_event(&serde_json::json!({"params": {"message": "hello from daemon"}}));
        assert_eq!(panel.messages.len(), 2);
        assert_eq!(panel.messages[1].role, "assistant");
        assert_eq!(panel.messages[1].content, "hello from daemon");
    }
}
