...
mod tests {
    ...
    #[test]
    fn new_panel_has_no_sessions() {
        let panel = CollaborationPanel::new();
        assert!(panel.sessions.is_empty());
        assert!(panel.invite_code.is_empty());
    }

    #[test]
    fn collab_create_generates_a_session_and_invite_code() {
        let mut panel = CollaborationPanel::new();
        let result = panel.run_command("collab", &["create"]);
        assert!(result.unwrap().starts_with("Session created."));
        assert_eq!(panel.sessions.len(), 1);
        assert_eq!(panel.invite_code.len(), 8);
        assert!(panel.sessions[0].active);
    }

    #[test]
    fn collab_join_reports_stub_message() {
        let mut panel = CollaborationPanel::new();
        let result = panel.run_command("collab", &["join", "ABCD1234"]);
        assert_eq!(result, Some("Joined session with code: ABCD1234 (stub)".to_string()));
    }

    #[test]
    fn unknown_command_returns_none() {
        let mut panel = CollaborationPanel::new();
        assert_eq!(panel.run_command("nope", &[]), None);
    }
}
