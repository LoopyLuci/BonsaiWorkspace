...
mod tests {
    ...
    #[test]
    fn new_panel_has_no_result_yet() {
        let panel = EstimatePanel::new();
        assert!(panel.result.is_none());
    }

    #[test]
    fn run_estimate_computes_positive_eta_and_cost() {
        let mut panel = EstimatePanel::new();
        panel.field_inputs = ["train".to_string(), "10".to_string(), "0".to_string()];
        panel.run_estimate();
        let result = panel.result.expect("estimate should have produced a result");
        assert_eq!(panel.units, 10.0);
        assert!(result.eta_min > 0.0 && result.eta_max > result.eta_min);
        assert!(result.credits > 0.0);
        // paid_urv == 0 means the free tier ETA should be populated
        assert!(result.free_eta.is_some());
    }

    #[test]
    fn paid_urv_disables_free_eta() {
        let mut panel = EstimatePanel::new();
        panel.field_inputs = ["train".to_string(), "10".to_string(), "5".to_string()];
        panel.run_estimate();
        let result = panel.result.expect("estimate should have produced a result");
        assert!(result.free_eta.is_none());
    }

    #[test]
    fn run_command_returns_formatted_summary() {
        let mut panel = EstimatePanel::new();
        panel.field_inputs = ["train".to_string(), "4".to_string(), "0".to_string()];
        let summary = panel.run_command("estimate", &[]).expect("should produce a summary");
        assert!(summary.starts_with("ETA:"));
    }
}
