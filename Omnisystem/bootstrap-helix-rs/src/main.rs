//! Helix seed — the Helix bootstrap GPU/compute-kernel DSL driver.
//!
//!     helix-seed run    program.helix
//!     helix-seed tokens program.helix
//!     helix-seed test   [dir]
...
fn cmd_test(dir: &str) -> i32 {
    ...
    let mut files: Vec<PathBuf> = rd.filter_map(|e| e.ok()).map(|e| e.path()).filter(|p| p.extension().is_some_and(|x| x == "helix")).collect();
    files.sort();
...
fn main() -> ExitCode {
    ...
    let code = match args.first().map(String::as_str) {
        ...
        _ => {
            println!("Helix seed {VERSION}\n\nUsage: helix-seed <run|check|tokens|test> [file|dir]");
            0
        }
    };
