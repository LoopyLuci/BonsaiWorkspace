...
impl SylvaComposer {
    pub fn new(
        ...
        tool_registry: Arc<tool_registry::ToolRegistry>,
    ) -> Self {
        Self {
            orchestrator,
            tool_registry,
    ...
    pub async fn execute(&self, script: String) -> anyhow::Result<GenerationResult> {
        ...
        let tool_fn: sylva::vm::ToolFn =
            Arc::new(move |method: String, args: serde_json::Value| {
                ...
                .map_err(|e| sylva::VmError::ToolCallFailed(e.to_string()))
            });

        let mut vm = sylva::SylvaVm::with_tool_fn(tool_fn);
        ...
        let cas_key = cas::CasKey::from_hex(cas_key_hex)
