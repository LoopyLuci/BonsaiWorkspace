...
pub struct Backup_Agent;
#[async_trait]
impl Agent for Backup_Agent {
    fn name(&self) -> &str { "backup-agent" }
    async fn init(&self) -> Result<()> { info!("Init"); Ok(()) }
    async fn execute(&self, input: AgentInput) -> Result<AgentOutput> {
        Ok(AgentOutput {
            agent_name: "backup-agent".to_string(),
...
mod tests {
    ...
    async fn test_execute() {
        let r = Backup_Agent.execute(AgentInput {
    ...
    #[tokio::test]
    async fn test_health() { assert!(Backup_Agent.health_check().await.is_ok()); }
    #[tokio::test]
