...
- Omnisystem Workspace running (`cargo tauri dev` or installed build)
- Pair token from Settings or `omnisystem-config.json`
- `curl` available
...
```bash
curl -s $BASE/health
# Expected: {"service":"omnisystem-workspace","status":"ok"}
```
...
```bash
# From omnisystem-workspace root
wc -l data/omnisystem_core/omnisystem_core_train.jsonl
wc -l data/omnisystem_core/omnisystem_core_val.jsonl
wc -l data/omnisystem_core/omnisystem_core_test.jsonl
