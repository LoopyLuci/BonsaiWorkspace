...
exports.parsePythonFamily = parsePythonFamily;
exports.generatePythonFamily = generatePythonFamily;
...
// ─── Parse ────────────────────────────────────────────────────────────────────
function parsePythonFamily(source, langId) {
...
function buildBlockMap(lines, langId) {
    const units = [];
    let i = 0;
    while (i < lines.length) {
        ...
        if (defMatch && langId === 'python') {
            ...
            units.push({
                ...
                signature: {
                    ...
                    returns: retStr ? { ...ULIR_1.UNKNOWN_TYPE, name: retStr, originalSrc: retStr } : ULIR_1.UNKNOWN_TYPE,
                    throws: [],
                },
                body: [],
        ...
        if (rubyDef && (langId === 'ruby' || langId === 'crystal')) {
            units.push({
                ...
                name: rubyDef[1],
                visibility: 'public',
                signature: { params: parseSimpleParams(rubyDef[2] ?? ''), returns: ULIR_1.UNKNOWN_TYPE, throws: [] },
                body: [],
                attributes: [],
                docComment: '',
                sourceLines: [i, i],
...
function parsePyParams(raw) {
    ...
    return raw.split(',').map((p) => {
        ...
        if (m) {
            return { name: m[1], type: { ...ULIR_1.UNKNOWN_TYPE, name: m[2], originalSrc: m[2] }, defaultValue: undefined, isVariadic: t.startsWith('**'), isKeyword: t.startsWith('**'), isRef: false, isMut: false };
        }
        const name = t.replace(/[*=].*/, '').trim();
        return name ? { name, type: ULIR_1.ANY_TYPE, defaultValue: undefined, isVariadic: t.startsWith('*'), isKeyword: false, isRef: false, isMut: false } : null;
    }).filter((p) => p !== null && p.name.length > 0);
}
function parseSimpleParams(raw) {
    if (!raw.trim()) {
        return [];
    }
    return raw.split(',').map(p => ({
        name: p.trim().replace(/[*&:].*/, '').trim() || 'param',
        type: ULIR_1.ANY_TYPE,
...
function extractDecorators(lines, fnLine) {
    const attrs = [];
...
function extractDocstring(blockLines) {
...
// ─── Generate ─────────────────────────────────────────────────────────────────
function generatePythonFamily(ir, targetLangId, opts = ULIR_1.DEFAULT_OPTIONS) {
    ...
    switch (lang) {
        case 'python':
            if (imp.names.length > 0) {
                return `from ${imp.path.replace(/\//g, '.')} import ${imp.names.join(', ')}`;
            }
...
function renderUnit(unit, lang, opts, srcLang) {
    if (unit.kind === 'class') {
        return renderClass(unit, lang, srcLang);
    }
...
function renderClass(unit, lang, srcLang) {
    const src = srcLang ?? unit.sourceLanguage ?? 'python';
    const body = (0, BodyTranslator_1.translateBody)(unit.originalSource ?? '', src, lang);
    switch (lang) {
...
//# sourceMappingURL=PythonFamilyHandler.js.map