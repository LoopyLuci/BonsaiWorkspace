...
exports.parseDataFamily = parseDataFamily;
exports.generateDataFamily = generateDataFamily;
...
// ─── Parse ────────────────────────────────────────────────────────────────────
function parseDataFamily(source, langId) {
    const lang = (0, LanguageRegistry_1.getLang)(langId);
    const lines = source.split('\n');
    const units = [];
    switch (langId) {
        case 'sql':
            extractSQLUnits(source, units);
            break;
        case 'graphql':
            extractGraphQLUnits(source, units);
            break;
        case 'json':
            extractJSONUnits(source, units);
            break;
        case 'yaml':
        case 'yml':
            extractYAMLUnits(source, lines, units);
            break;
        case 'toml':
            extractTOMLUnits(source, lines, units);
            break;
        case 'xml':
            extractXMLUnits(source, units);
            break;
        case 'protobuf':
            extractProtobufUnits(source, units);
            break;
        case 'hcl':
            extractHCLUnits(source, units);
            break;
        default:
            extractGenericDataUnits(source, lines, units);
            break;
    }
    const meta = {
...
function extractSQLUnits(source, units) {
    // Tables
...
function extractGraphQLUnits(source, units) {
...
function extractJSONUnits(source, units) {
    try {
...
function extractYAMLUnits(source, lines, units) {
    // Top-level keys
    const topKeys = [];
...
function extractTOMLUnits(source, lines, units) {
...
function extractXMLUnits(source, units) {
...
function extractProtobufUnits(source, units) {
...
function extractHCLUnits(source, units) {
...
function extractGenericDataUnits(source, lines, units) {
    // INI sections
    for (const m of source.matchAll(/^\[([^\]]+)\]/gm)) {
        units.push(makeDataUnit('variable', m[1], [], m[0]));
    }
}
function makeDataUnit(kind, name, fields, src) {
    return {
        kind,
...
// ─── Generate ─────────────────────────────────────────────────────────────────
function generateDataFamily(ir, targetLangId, opts = ULIR_1.DEFAULT_OPTIONS) {
    switch (targetLangId) {
...
function generateSQL(ir, opts) {
    const lines = [`-- ${ir.name} — Converted to SQL`, `-- Source: ${ir.sourceLanguage}`, ''];
    for (const unit of ir.units) {
        if (unit.kind === 'interface' || unit.kind === 'class' || unit.kind === 'struct' || unit.kind === 'table') {
            ...
            lines.push(`LANGUAGE plpgsql AS $$\nBEGIN\n${(0, BodyTranslator_1.translateBody)(unit.originalSource ?? '', ir.sourceLanguage, 'sql', '  ')}\nEND;\n$$;`);
        }
        else {
...
function generateGraphQL(ir, opts) {
...
function generateXML(ir, opts) {
    ...
    for (const unit of ir.units) {
        ...
        if (unit.attributes.length > 0) {
            const fields = unit.attributes[0].replace('fields: ', '').split(', ');
            for (const f of fields) {
                lines.push(`    <field name="${f}" type="string"/>`);
            }
        }
...
function generateProtobuf(ir, opts) {
...
//# sourceMappingURL=DataFamilyHandler.js.map