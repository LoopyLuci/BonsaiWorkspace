...
exports.parseShellFamily = parseShellFamily;
exports.generateShellFamily = generateShellFamily;
...
// ─── Parse ────────────────────────────────────────────────────────────────────
function parseShellFamily(source, langId) {
...
function extractBashFunctions(source, lines, units, langId) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const name = m[1] ?? m[2];
        if (!name || ['if', 'while', 'for', 'case'].includes(name)) {
            continue;
        }
        ...
        const params = usedParams.map(n => ({
            name: `arg${n}`, type: ULIR_1.STRING_TYPE, defaultValue: undefined,
...
function extractPowerShellFunctions(source, lines, units) {
    for (let i = 0; i < lines.length; i++) {
        ...
        const params = paramMatch
            ? paramMatch[1].split(',').map((p) => {
                ...
                return n ? { name: n[1], type: ULIR_1.STRING_TYPE, defaultValue: undefined, isVariadic: false, isKeyword: false, isRef: false, isMut: false } : null;
            }).filter((p) => p !== null)
            : [];
        units.push({
            kind: 'function', name,
...
function extractMakeTargets(source, lines, units) {
    for (let i = 0; i < lines.length; i++) {
...
function extractDockerStages(source, lines, units) {
    for (let i = 0; i < lines.length; i++) {
...
function extractBashDoc(lines, fnLine) {
    const docs = [];
...
// ─── Generate ─────────────────────────────────────────────────────────────────
function generateShellFamily(ir, targetLangId, opts = ULIR_1.DEFAULT_OPTIONS) {
...
//# sourceMappingURL=ShellFamilyHandler.js.map