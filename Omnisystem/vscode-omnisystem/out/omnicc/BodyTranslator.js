...
exports.commentPfx = commentPfx;
exports.usesSemicolon = usesSemicolon;
exports.lits = lits;
function blockStyle(lang) {
...
function extractBodyLines(src, srcLang) {
    const raw = src.split('\n');
    if (raw.length === 0) {
        return [];
    }
...
function transformVarDecl(line, tgtLang) {
...
// ─── I/O transforms ───────────────────────────────────────────────────────────
function transformIO(line, srcLang, tgtLang) {
...
function applyLiterals(line, srcLang, tgtLang) {
...
function transformOperators(line, srcLang, tgtLang) {
...
function transformStringOps(line, srcLang, tgtLang) {
...
function transformComment(line, srcLang, tgtLang) {
...
function transformReturn(line, tgtLang) {
...
function ensureSemicolon(line, tgtLang) {
    if (!usesSemicolon(tgtLang)) {
...
function transformControlFlow(line, srcLang, tgtLang) {
    ...
    if (srcStyle === 'brace' && tgtStyle === 'indent') {
        ...
        if (m) {
            return m[2] === '0' ? `${ind}for ${m[1]} in range(${m[3]}):` : `${ind}for ${m[1]} in range(${m[2]}, ${m[3]}):`;
        }
        // while (cond) { → while cond:
        m = body.match(/^while\s*\((.+)\)\s*\{?\s*$/);
        if (m) {
            return `${ind}while ${m[1]}:`;
        }
        // switch (x) { → (leave as comment — Python has no switch)
        m = body.match(/^switch\s*\((.+)\)\s*\{?\s*$/);
        if (m) {
            return `${ind}# match ${m[1]}:`;
        }
        // case x: → case x:
        ...
        if (m) {
            return `${ind}except Exception as ${m[1]}:`;
        }
        // } finally { → finally:
    ...
    if (srcStyle === 'brace' && tgtLang === 'rust') {
        ...
        m = body.match(/^for\s*\(\s*(?:const|let|var)\s+(\w+)\s+of\s+(\w+)\s*\)\s*\{?\s*$/);
        if (m) {
            return `${ind}for ${m[1]} in ${m[2]}.iter() {`;
        }
        m = body.match(/^for\s*\(\s*(?:let|var|int)\s+(\w+)\s*=\s*(\d+)\s*;\s*\1\s*<\s*(\w+|\d+)\s*;\s*\1\+\+\s*\)\s*\{?\s*$/);
        if (m) {
            return m[2] === '0' ? `${ind}for ${m[1]} in 0..${m[3]} {` : `${ind}for ${m[1]} in ${m[2]}..${m[3]} {`;
        }
        m = body.match(/^while\s*\((.+)\)\s*\{?\s*$/);
        if (m) {
            return `${ind}while ${m[1]} {`;
    ...
    if (srcStyle === 'brace' && tgtLang === 'go') {
        ...
        m = body.match(/^for\s*\(\s*(?:const|let|var)\s+(\w+)\s+of\s+(\w+)\s*\)\s*\{?\s*$/);
        if (m) {
            return `${ind}for _, ${m[1]} := range ${m[2]} {`;
        }
        m = body.match(/^for\s*\(\s*(?:let|var|int)\s+(\w+)\s*=\s*(\d+)\s*;\s*\1\s*<\s*(\w+|\d+)\s*;\s*\1\+\+\s*\)\s*\{?\s*$/);
        if (m) {
            return `${ind}for ${m[1]} := ${m[2]}; ${m[1]} < ${m[3]}; ${m[1]}++ {`;
        }
        m = body.match(/^while\s*\((.+)\)\s*\{?\s*$/);
        if (m) {
            return `${ind}for ${m[1]} {`;
...
function convertBracesToIndent(lines) {
    const out = [];
...
function convertIndentToBraces(lines, tgtLang) {
    const out = [];
    const indents = [0];
...
// ─── Per-line transform ────────────────────────────────────────────────────────
function transformLine(line, srcLang, tgtLang) {
    if (line.trim() === '') {
        return line;
    }
    // 1. Comment style
...
// ─── Main export ──────────────────────────────────────────────────────────────
function emptyBody(tgtLang, baseIndent) {
    ...
    if (style === 'functional') {
        if (tgtLang === 'haskell') {
            return `${baseIndent}undefined`;
        }
        if (tgtLang === 'erlang') {
            return `${baseIndent}ok`;
        }
        if (tgtLang === 'elixir') {
            return `${baseIndent}:ok`;
        }
        if (tgtLang === 'clojure') {
            return `${baseIndent}nil`;
        }
        return `${baseIndent}()`; // OCaml, F#, etc.
    }
    return ''; // brace-style: empty body `{}` is valid
}
function translateBody(originalSource, srcLang, tgtLang, baseIndent = '    ') {
...
//# sourceMappingURL=BodyTranslator.js.map