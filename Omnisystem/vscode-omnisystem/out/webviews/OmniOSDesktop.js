...
exports.OmniOSDesktopPanel = void 0;
const vscode = __importStar(require("vscode"));
const fs = __importStar(require("fs"));
const child_process_1 = require("child_process");
const RuntimeClient_1 = require("../runtime/RuntimeClient");
const PtyManager_1 = require("../runtime/PtyManager");
const OmniHarnessClient_1 = require("../harness/OmniHarnessClient");
function getNonce() {
    let text = '';
...
class OmniOSDesktopPanel {
    static createOrShow(extensionUri, ctx) {
        if (ctx)
            OmniOSDesktopPanel._extensionContext = ctx;
        ...
        const panel = vscode.window.createWebviewPanel(OmniOSDesktopPanel.viewType, 'OmniOS Desktop', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    constructor(panel, extensionUri) {
        this._disposables = [];
        this._activeProcs = new Map();
        this._lastProc = null;
        this._lastDiagCount = -1;
        this._panel = panel;
        this._extensionUri = extensionUri;
        // Initialize IPC runtime client and PTY manager
        this._runtime = (0, RuntimeClient_1.getRuntimeClient)(extensionUri.fsPath);
        this._pty = (0, PtyManager_1.getPtyManager)();
        // Wire PTY output back to webview
        this._pty.on('output', (sessionId, data) => {
            if (!this._panel.visible)
                return;
            this._panel.webview.postMessage({ type: 'ptyOutput', sessionId, data });
        });
        this._pty.on('exit', (sessionId, code) => {
            this._panel.webview.postMessage({ type: 'ptyExit', sessionId, code });
        });
        // Wire RuntimeClient notifications to webview
        this._runtime.on('notification', (method, params) => {
            if (method === 'build/progress') {
                this._panel.webview.postMessage({ type: 'buildProgress', ...params });
            }
            else if (method === 'system/metrics') {
                this._panel.webview.postMessage({ type: 'runtimeMetrics', ...params });
            }
        });
        // Start runtime client (non-blocking — degrades gracefully on failure)
        this._runtime.start().catch(() => { });
    ...
    _update() {
        ...
        const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'media', 'desktop-client.js'));
    ...
    async _handleMessage(msg) {
        switch (msg.command) {
            case 'openApp':
                await vscode.commands.executeCommand('omnisystem.' + msg.app);
                break;
            case 'openFile': {
                const uri = vscode.Uri.file(msg.text);
            ...
            case 'runTerminalCommand': {
                ...
                term.show();
                term.sendText(msg.text);
                break;
            }
            case 'owThemeChange':
                if (msg.theme && OmniOSDesktopPanel.onThemeChange) {
                    OmniOSDesktopPanel.onThemeChange(msg.theme);
                }
                break;
            case 'applyTheme':
                await vscode.workspace.getConfiguration().update('workbench.colorTheme', 'Omnisystem Dark', vscode.ConfigurationTarget.Global);
                break;
            case 'openSettings':
                await vscode.commands.executeCommand('workbench.action.openSettings', '@ext:omnisystem.omnisystem');
                break;
            case 'scaffold': {
                const lang = msg.lang;
                const cmdMap = {
                ...
                await vscode.commands.executeCommand(cmd, msg.name);
                break;
            }
            case 'desktopLaunch':
            ...
            case 'getFiles': {
                ...
                const root = msg.path ?? folders[0].uri.fsPath;
                try {
                ...
                outer: for (const root of searchRoots) {
                    for (const name of names) {
                        ...
                        if (fs2.existsSync(candidate)) {
                            omniccPath = candidate;
                            break outer;
                        }
                    }
                }
                try {
                    const proc = (0, child_process_1.spawn)(omniccPath, args, { cwd, shell: true });
            ...
            case 'execCommand': {
                const cmdStr = msg.text;
                const folders3 = vscode.workspace.workspaceFolders;
                const cwd3 = msg.cwd || folders3?.[0]?.uri.fsPath || process.cwd();
                const procId = Date.now();
                try {
                    const proc = (0, child_process_1.spawn)(cmdStr, [], { cwd: cwd3, shell: true });
                    this._activeProcs.set(procId, proc);
                    this._lastProc = proc;
                    proc.stdout.on('data', (chunk) => {
                        chunk.toString().split('\n').forEach(line => {
                            if (line)
                                this._panel.webview.postMessage({ type: 'termLine', text: line });
                        });
                    });
                    proc.stderr.on('data', (chunk) => {
                        chunk.toString().split('\n').forEach(line => {
                            if (line)
                                this._panel.webview.postMessage({ type: 'termLine', text: line, cls: 'err' });
                        });
                    });
                    proc.on('close', (code) => {
                    ...
                    this._panel.webview.postMessage({ type: 'termLine', text: `Failed to spawn: ${e.message}`, cls: 'err' });
                    this._panel.webview.postMessage({ type: 'termDone', code: 1 });
                }
                break;
            }
            case 'killProc': {
                this._activeProcs.forEach(p => { try {
                    p.kill();
                }
                catch { /* ignore */ } });
            ...
            case 'shellInput': {
                ...
                if (this._lastProc && this._lastProc.stdin && !this._lastProc.killed) {
                    try {
                        this._lastProc.stdin.write(text);
                    }
                    catch { /* ignore */ }
                }
                break;
            ...
            case 'getSandboxStatus': {
                ...
                const wsRoot3 = (() => {
                    ...
                    if (fs.existsSync(pathMod3.join(sub, 'BUILD.omnisystem')))
                        return sub;
                    if (fs.existsSync(pathMod3.join(wsRoot3Raw, 'BUILD.omnisystem')))
                        return wsRoot3Raw;
                    return sub;
                })();
            ...
            case 'saveSettings': {
                try {
                    await OmniOSDesktopPanel._extensionContext?.workspaceState.update('omnisystem.desktopSettings', msg.settings);
                }
                catch { /* ignore */ }
                break;
            }
            case 'loadSettings': {
                const settings = OmniOSDesktopPanel._extensionContext?.workspaceState.get('omnisystem.desktopSettings', {}) ?? {};
            ...
            case 'saveWindowState': {
                try {
                    await OmniOSDesktopPanel._extensionContext?.workspaceState.update('omnisystem.windowState', msg.state);
                }
                catch { /* ignore */ }
                break;
            }
            case 'loadWindowState': {
                const winState = OmniOSDesktopPanel._extensionContext?.workspaceState.get('omnisystem.windowState', null) ?? null;
            ...
            case 'deleteFile': {
                try {
                    ...
                    this._panel.webview.postMessage({ type: 'fileError', error: e.message });
                }
                break;
            }
            case 'createFile': {
            ...
            case 'renameFile': {
                try {
                    const oldUri = vscode.Uri.file(msg.oldPath);
                    const newUri = vscode.Uri.file(msg.newPath);
            ...
            case 'ptyCreate': {
                ...
                const cwd8 = msg.cwd || vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
            ...
            case 'ptyWrite': {
                const written = this._pty.write(msg.sessionId, msg.data);
                if (!written) {
            ...
            case 'ptyResize': {
                this._pty.resize(msg.sessionId, msg.cols || 80, msg.rows || 24);
                break;
            }
            case 'ptyKill': {
                this._pty.kill(msg.sessionId, msg.signal || 'SIGTERM');
            ...
            case 'applyBugFix': {
                const fixFile = msg.file;
                const fixLine = msg.line || 1;
                if (fixFile) {
                    try {
            ...
            case 'saveBugHunterProfile': {
                try {
                    await OmniOSDesktopPanel._extensionContext?.workspaceState.update('omnisystem.bugHunterProfile', msg.profile);
                }
                catch { /* ignore */ }
                break;
            }
            case 'loadBugHunterProfile': {
                const bhProfile = OmniOSDesktopPanel._extensionContext?.workspaceState.get('omnisystem.bugHunterProfile', null) ?? null;
            ...
            case 'getHarnessStatus': {
                ...
                const serverUrl = cfg.get('harness.serverUrl', 'http://localhost:8080');
                const client = new OmniHarnessClient_1.OmniHarnessClient(serverUrl);
                try {
    ...
    dispose() {
        ...
        this._panel.dispose();
        (0, PtyManager_1.disposePtyManager)();
        (0, RuntimeClient_1.disposeRuntimeClient)();
    ...
    _getHtml(scriptUri, widgetStyleUri, widgetScriptUri) {
        return `<!DOCTYPE html>
...
exports.OmniOSDesktopPanel = OmniOSDesktopPanel;
OmniOSDesktopPanel.viewType = 'omnisystem.omniOsDesktop';
//# sourceMappingURL=OmniOSDesktop.js.map