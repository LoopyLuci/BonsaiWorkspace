...
class WidgetConverterPanel {
    ...
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel(WidgetConverterPanel.viewType, 'Widget Converter', column, {
            enableScripts: true,
    ...
    async _handleMessage(msg) {
        switch (msg.command) {
            case 'convert': {
                ...
                const result = (0, WidgetConversionEngine_1.convert)({ source, sourceLang, targetLang, widgetNameHint: widgetName });
                const previewHtml = (0, WidgetConversionEngine_1.renderOWPreview)(result.widgetType, result.widgetName);
            ...
            case 'detectLang': {
                const source = String(msg.source ?? '');
                const detected = (0, WidgetConversionEngine_1.detectLanguage)(source);
            ...
            case 'createFile': {
                ...
                const ext = WidgetIR_1.LANGUAGE_EXTENSIONS[langKey] ?? '.vera';
            ...
            case 'openInEditor': {
                const code = String(msg.code ?? '');
                const langKey = String(msg.targetLang ?? 'vera');
                const ext = WidgetIR_1.LANGUAGE_EXTENSIONS[langKey] ?? '.txt';
    ...
    _getHtml(webview, cssUri, jsUri) {
        const nonce = getNonce();
        const langOptions = Object.entries(WidgetIR_1.LANGUAGE_LABELS)
...
exports.WidgetConverterPanel = WidgetConverterPanel;
WidgetConverterPanel.viewType = 'omnisystem.widgetConverter';
