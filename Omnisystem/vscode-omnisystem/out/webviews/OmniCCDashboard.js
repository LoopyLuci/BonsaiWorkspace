...
exports.OmniCCDashboardPanel = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
const ConversionEngine_1 = require("../omnicc/ConversionEngine");
const LanguageDetector_1 = require("../omnicc/LanguageDetector");
const LanguageRegistry_1 = require("../omnicc/LanguageRegistry");
class OmniCCDashboardPanel {
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel(OmniCCDashboardPanel.viewType, 'OmniCC — Universal Language Converter', column, {
            enableScripts: true,
    ...
    constructor(panel, extensionUri) {
        ...
        this._history = [];
        this._panel = panel;
        this._engine = new ConversionEngine_1.OmniCCConversionEngine();
    ...
    async _handleMessage(msg) {
        switch (msg.command) {
            ...
            case 'detect': {
                const source = String(msg.source ?? '');
                const filename = msg.filename ? String(msg.filename) : undefined;
                const detected = (0, LanguageDetector_1.detectLanguage)(source, filename);
                this._post({ command: 'detectResult', ...detected });
                break;
            }
            case 'searchLangs': {
                const query = String(msg.query ?? '');
                const results = query.length >= 2 ? (0, LanguageRegistry_1.searchLanguages)(query).slice(0, 50) : (0, LanguageRegistry_1.allLanguages)().slice(0, 100);
            ...
            case 'getLangInfo': {
                const lang = (0, LanguageRegistry_1.getLang)(String(msg.langId ?? ''));
            ...
            case 'exportProject': {
                ...
                if (results.length === 0) {
                    vscode.window.showWarningMessage('No converted files to export.');
                    break;
                }
...
exports.OmniCCDashboardPanel = OmniCCDashboardPanel;
OmniCCDashboardPanel.viewType = 'omnisystem.omniCC';
//# sourceMappingURL=OmniCCDashboard.js.map