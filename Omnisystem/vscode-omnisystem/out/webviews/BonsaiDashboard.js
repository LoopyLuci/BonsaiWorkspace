...
exports.BonsaiDashboardPanel = void 0;
...
// ─── BonsaiDashboardPanel ─────────────────────────────────────────────────────
class BonsaiDashboardPanel {
    static postMessage(msg) {
        BonsaiDashboardPanel.currentPanel?._panel.webview.postMessage(msg);
    }
    static createOrShow(extensionUri) {
        const column = vscode.window.activeTextEditor
            ? vscode.window.activeTextEditor.viewColumn
            : undefined;
        if (BonsaiDashboardPanel.currentPanel) {
            BonsaiDashboardPanel.currentPanel._panel.reveal(column);
            return;
        }
        const panel = vscode.window.createWebviewPanel(BonsaiDashboardPanel.viewType, 'Bonsai Ecosystem Dashboard', column || vscode.ViewColumn.One, {
            enableScripts: true,
            localResourceRoots: [extensionUri],
            retainContextWhenHidden: true,
        });
        BonsaiDashboardPanel.currentPanel = new BonsaiDashboardPanel(panel, extensionUri);
    }
    ...
    async _handleMessage(message) {
        ...
        switch (message.command) {
            case 'getStatus': {
                const platform = os.platform() === 'win32' ? 'Windows x64' :
            ...
            case 'getEcosystemFiles': {
                ...
                const ecoBase = wsPath
                    ? path.join(wsPath, 'src', 'systems', 'modules', 'base-modules', 'applications', 'bonsai-ecosystem')
                    : null;
                const titanFiles = [
            ...
            case 'bonsaiLaunch':
                this._run('npx', ['tauri', 'dev'], workspacePath, 'Bonsai Launch');
                break;
            case 'bonsaiBuild':
                this._run('npx', ['tauri', 'build'], workspacePath, 'Bonsai Build');
                break;
            case 'bonsaiBuddyBuild':
                this._run('npx', ['react-native', 'build-android', '--mode=release'], workspacePath, 'Buddy APK');
                break;
            case 'bonsaiBuddyConnect':
                this._run('adb', ['reverse', 'tcp:8081', 'tcp:8081'], workspacePath, 'ADB Connect');
                break;
            case 'openBuddyApp':
                vscode.window.showInformationMessage(`Opening Bonsai Buddy sub-app: ${message.arg}`);
                break;
            case 'bonsaiBrowserExtBuild':
                this._run('npm', ['run', 'build'], workspacePath, 'Browser Ext Build');
                break;
            case 'bonsaiBrowserExtInstall':
                vscode.window.showInformationMessage('Load unpacked extension from the dist/ folder in Chrome at chrome://extensions');
                break;
            case 'bonsaiControlPanel':
                // Control Panel Titan server runs on port 12345 (api_server.ti)
                vscode.env.openExternal(vscode.Uri.parse('http://localhost:12345'));
                break;
            case 'bonsaiControlPanelStart':
                this._run('npm', ['run', 'dev'], workspacePath, 'Control Panel');
                break;
            case 'bonsaiSharedUiBuild':
                this._run('npm', ['run', 'build:lib'], workspacePath, 'Shared UI');
                break;
            case 'bonsaiNotifications':
                this._log('[Bonsai] Notification System — notifications/notification_daemon.ti');
                vscode.commands.executeCommand('omnisystem.bonsaiNotifications');
                break;
            case 'bonsaiSystemTray':
                this._log('[Bonsai] System Tray — system-tray/core.ti');
                vscode.commands.executeCommand('omnisystem.bonsaiSystemTray');
                break;
            case 'bonsaiInit':
                this._log('[Bonsai Init] Triggering ecosystem initialization — INITIALIZATION.ti');
                vscode.commands.executeCommand('omnisystem.bonsaiInit');
                break;
            case 'bonsaiDiagnostics':
                this._log('[Bonsai] Running diagnostics mode — INITIALIZATION.ti::bonsai_ecosystem_diagnostics()');
                vscode.window.showInformationMessage('Bonsai Ecosystem: Diagnostics mode triggered.');
                break;
            case 'bonsaiRepair':
                this._log('[Bonsai] Running repair mode — INITIALIZATION.ti::bonsai_ecosystem_repair()');
                vscode.window.showInformationMessage('Bonsai Ecosystem: Repair mode triggered.');
                break;
            case 'bonsaiDeployAll':
            case 'deployAll':
                this._log('[Deploy] Starting full Bonsai release build...');
    ...
    _update() {
        ...
        this._panel.title = 'Bonsai Ecosystem Dashboard';
...
<head>
  ...
  <title>Bonsai Ecosystem Dashboard</title>
...
<body>
  ...
  <div class="grid">
    ...
    <!-- Bonsai Launcher -->
    <div class="card">
      <h3><span class="status-dot active"></span>Bonsai Launcher <span class="badge badge-green" style="margin-left:auto;font-size:10px;">Tauri</span></h3>
      ...
      <div class="actions">
        <button class="btn" onclick="sendCommand('bonsaiLaunch')">&#9654; Launch Dev</button>
        <button class="btn btn-secondary" onclick="sendCommand('bonsaiBuild')">&#9109; Build Release</button>
      </div>
    </div>

    <!-- Buddy Android -->
    <div class="card">
      <h3><span class="status-dot pending" id="buddy-dot"></span>Bonsai Buddy <span class="badge badge-yellow" style="margin-left:auto;">Android</span></h3>
      ...
      <div class="actions">
        <button class="btn" onclick="sendCommand('bonsaiBuddyBuild')">&#9109; Build APK</button>
        <button class="btn btn-secondary" onclick="sendCommand('bonsaiBuddyConnect')">&#128246; Connect ADB</button>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="actions">
        <button class="btn" onclick="sendCommand('bonsaiBrowserExtBuild')">&#9109; Build</button>
        <button class="btn btn-secondary" onclick="sendCommand('bonsaiBrowserExtInstall')">&#128268; Install Dev</button>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="actions">
        <button class="btn" onclick="sendCommand('bonsaiControlPanel')">&#127758; Open on :12345</button>
        <button class="btn btn-secondary" onclick="sendCommand('bonsaiControlPanelStart')">&#9654; Start Server</button>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="actions">
        <button class="btn btn-secondary" onclick="sendCommand('bonsaiNotifications')">&#128276; Open Dashboard</button>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="actions">
        <button class="btn btn-secondary" onclick="sendCommand('bonsaiSystemTray')">&#128190; View Source</button>
      </div>
    </div>

    <!-- Shared UI Library -->
    <div class="card">
      <h3><span class="status-dot active"></span>@bonsai/shared-ui <span class="badge badge-blue" style="margin-left:auto;">Library</span></h3>
      <p class="card-desc">Shared Svelte component library used across all Bonsai apps</p>
      <div class="metric">
      ...
      <div class="actions">
        <button class="btn btn-secondary" onclick="sendCommand('bonsaiSharedUiBuild')">&#9109; Rebuild Library</button>
      </div>
    </div>
    ...
    <div class="card full-width" id="titan-source-card">
      ...
      <p class="card-desc" id="eco-status-label">Scanning Omnisystem codebase for Bonsai infrastructure modules...</p>
    ...
    <div class="card full-width" style="border-color:#003344;">
      <h3>&#9881;&#65039; Bonsai Ecosystem Initialization</h3>
      ...
      <div class="actions">
        <button class="btn" onclick="sendCommand('bonsaiInit')">&#9654; Initialize Ecosystem</button>
        <button class="btn btn-secondary" onclick="sendCommand('bonsaiDiagnostics')">&#128270; Diagnostics</button>
        <button class="btn btn-secondary" onclick="sendCommand('bonsaiRepair')">&#128295; Repair</button>
      </div>
    </div>
    ...
    <div class="card full-width">
      ...
      <div class="actions" style="margin-top:12px;">
        <button class="btn" onclick="sendCommand('bonsaiDeployAll')">&#128640; Deploy Release</button>
  ...
  <script nonce="${nonce}">
    ...
    window.addEventListener('message', event => {
      ...
      if (msg.type === 'ecosystemFiles') {
        ...
        rootEl.textContent = msg.ecoFound ? '$(check) bonsai-ecosystem/ found' : '$(warning) not found';
        labelEl.textContent = msg.ecoFound
          ? 'Live Titan source files — click to open in editor'
          : 'Ecosystem not found at expected path. Set omnisystem.bonsaiPath in settings.';
    ...
    dispose() {
        BonsaiDashboardPanel.currentPanel = undefined;
        this._panel.dispose();
        if (this._buildProcess) {
            this._buildProcess.kill();
            this._buildProcess = undefined;
...
exports.BonsaiDashboardPanel = BonsaiDashboardPanel;
BonsaiDashboardPanel.viewType = 'omnisystem.bonsaiDashboard';
//# sourceMappingURL=BonsaiDashboard.js.map