...
exports.generateVera = generateVera;
...
function renderVeraNode(node, indent_level = 2) {
    ...
    if (node.kind === 'select') {
        const options = (node.options ?? []).map(opt => `${pad}    <option value="${opt.value}"${opt.value === node.value ? ' selected' : ''}>${opt.label}</option>`).join('\n') || `${pad}    <option value="">Select...</option>`;
...
function renderProps(props, node) {
    const baseProps = [];
...
function renderState(node) {
    const stateLines = [];
    if (node.label !== undefined) {
        stateLines.push(`        label: String = "${node.label}"`);
    }
    if (node.placeholder !== undefined) {
        stateLines.push(`        placeholder: String = "${node.placeholder}"`);
    }
    ...
    if (node.disabled !== undefined) {
        stateLines.push(`        disabled: Bool = ${node.disabled}`);
    }
...
function renderHandlers(node) {
    ...
    for (const ev of (node.events ?? [])) {
        ...
        handlers.push(`    fn ${fnName}(${ev.params?.join(', ') ?? ''}) {
        self.${ev.name}()
    }`);
    }
    if (node.kind === 'toggle' || node.kind === 'checkbox') {
        handlers.push(`    fn handle_toggle() {
...
//# sourceMappingURL=VeraGenerator.js.map