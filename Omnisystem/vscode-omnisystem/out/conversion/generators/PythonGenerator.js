...
function renderTkinterClass(node, name) {
    ...
    if (node.kind === 'button') {
        ...
        if (node.value) {
            lines.push(`        self.var.set("${node.value}")`);
        }
...
function renderUsageExample(node, name) {
    const className = name.charAt(0).toUpperCase() + name.slice(1);
    return `
# ── Usage example ──────────────────────────────────────────────────────────────
