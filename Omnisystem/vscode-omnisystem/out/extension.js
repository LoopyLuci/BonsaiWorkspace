...
exports.broadcastTheme = broadcastTheme;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
const child_process_1 = require("child_process");
const node_1 = require("vscode-languageclient/node");
const OmnisystemExplorer_1 = require("./providers/OmnisystemExplorer");
const DesktopExplorer_1 = require("./providers/DesktopExplorer");
const OmniPMExplorer_1 = require("./providers/OmniPMExplorer");
const OmniOSExplorer_1 = require("./providers/OmniOSExplorer");
const TaskProvider_1 = require("./providers/TaskProvider");
const OmnisystemDashboard_1 = require("./webviews/OmnisystemDashboard");
const BuildDashboard_1 = require("./webviews/BuildDashboard");
const MlStudio_1 = require("./webviews/MlStudio");
const ShaderPreview_1 = require("./webviews/ShaderPreview");
const LayoutPreview_1 = require("./webviews/LayoutPreview");
const OmniOSDesktop_1 = require("./webviews/OmniOSDesktop");
const WidgetGallery_1 = require("./webviews/WidgetGallery");
const WelcomePanel_1 = require("./webviews/WelcomePanel");
const WidgetConverter_1 = require("./webviews/WidgetConverter");
const OmniCCDashboard_1 = require("./webviews/OmniCCDashboard");
const BuildEditor_1 = require("./editors/BuildEditor");
const SylvaNotebook_1 = require("./editors/SylvaNotebook");
const OmniHarnessViewProvider_1 = require("./harness/OmniHarnessViewProvider");
const HarnessStore_1 = require("./harness/HarnessStore");
// ─── Constants ────────────────────────────────────────────────────────────────
...
const OMNI_DOC_SELECTORS = OMNI_LANGUAGES.map((lang) => ({
    scheme: 'file',
    language: lang,
}));
// App manifest filename: primary (de-branded) name first, then legacy fallback.
const APP_MANIFEST_NAMES = ['app.omnisystem.toml', 'app.bonsai.toml'];
// ─── Module-level state ───────────────────────────────────────────────────────
let client;
let outputChannel;
let statusBarMain;
...
async function activate(ctx) {
    ...
    explorerProvider = new OmnisystemExplorer_1.OmnisystemExplorerProvider();
    desktopProvider = new DesktopExplorer_1.DesktopExplorerProvider(undefined);
    omnipmProvider = new OmniPMExplorer_1.OmniPMExplorerProvider();
    omniosProvider = new OmniOSExplorer_1.OmniOSExplorerProvider(undefined);
    // Register once — VS Code only accepts the first registration per view ID
    ctx.subscriptions.push(vscode.window.registerTreeDataProvider('omnisystemExplorer', explorerProvider), vscode.window.registerTreeDataProvider('desktopExplorer', desktopProvider), vscode.window.registerTreeDataProvider('omnipmExplorer', omnipmProvider), vscode.window.registerTreeDataProvider('omniOsExplorer', omniosProvider));
    ...
    try {
        const harnessStore = new HarnessStore_1.HarnessStore(ctx);
        harnessProvider = new OmniHarnessViewProvider_1.OmniHarnessViewProvider(ctx, harnessStore, outputChannel);
        ctx.subscriptions.push(vscode.window.registerWebviewViewProvider(OmniHarnessViewProvider_1.OmniHarnessViewProvider.viewType, harnessProvider, { webviewOptions: { retainContextWhenHidden: true } }), harnessProvider);
...
async function activateCore(ctx) {
    ...
    if (wsInfo.kind === 'omnisystem') {
        applyOmnisystemWorkspaceSettings().catch((err) => outputChannel.appendLine(`Workspace settings error (non-fatal): ${err}`));
    }
    ...
    // Task provider
    ctx.subscriptions.push(vscode.tasks.registerTaskProvider('omnisystem', new TaskProvider_1.OmnisystemTaskProvider()));
    // Custom editors
    ctx.subscriptions.push(vscode.window.registerCustomEditorProvider('omnisystem.buildEditor', new BuildEditor_1.BuildEditorProvider(ctx), { supportsMultipleEditorsPerDocument: false }));
    // Sylva notebook
    const sylvaSerializer = new SylvaNotebook_1.SylvaNotebookSerializer();
    ctx.subscriptions.push(vscode.workspace.registerNotebookSerializer('sylva-notebook', sylvaSerializer), new SylvaNotebook_1.SylvaNotebookKernel());
    // (OmniHarness AI panel is registered synchronously in activate() so the
    // webview resolves even if this async path fails.)
    // Code actions
    ctx.subscriptions.push(vscode.languages.registerCodeActionsProvider(OMNI_DOC_SELECTORS, new OmniCodeActionsProvider(), { providedCodeActionKinds: [vscode.CodeActionKind.QuickFix] }));
    // Semantic tokens
    ...
    ctx.subscriptions.push(vscode.languages.registerDocumentSemanticTokensProvider(OMNI_DOC_SELECTORS, new OmniSemanticTokensProvider(semanticLegend), semanticLegend));
    ...
    ctx.subscriptions.push(vscode.workspace.onDidChangeConfiguration(async (e) => {
        if (e.affectsConfiguration('omnisystem.lspServerPath') ||
        ...
        if (e.affectsConfiguration('omnisystem.buildTarget')) {
            const target = vscode.workspace.getConfiguration(EXTENSION_ID).get('buildTarget', 'x86_64-linux');
    ...
    const isFirstInstall = !ctx.globalState.get(FIRST_INSTALL_KEY, false);
    if (isFirstInstall) {
...
// ─── Workspace settings ───────────────────────────────────────────────────────
async function applyOmnisystemWorkspaceSettings() {
...
async function detectWorkspace() {
    ...
    if (fs.existsSync(vscode.Uri.joinPath(root, 'BUILD.omnisystem').fsPath) ||
    ...
    const settingPath = vscode.workspace.getConfiguration('omnisystem').get('desktopPath');
...
function updateLspStatusBar(status) {
    const labels = {
...
async function startLanguageClient() {
    ...
    const serverOptions = buildServerOptions(lspPath, config);
    const clientOptions = {
        documentSelector: OMNI_LANGUAGES.map(lang => ({ scheme: 'file', language: lang })),
        synchronize: {
            fileEvents: vscode.workspace.createFileSystemWatcher('**/*.{titan,vera,helix,aether,axiom,sylva,nexus}'),
        ...
        initializationOptions: {
            ...
            buildTarget: config.get('buildTarget', 'x86_64-linux'),
            optimizationLevel: config.get('optimizationLevel', 'O0'),
        },
    };
    client = new node_1.LanguageClient(LSP_CLIENT_ID, LSP_CLIENT_NAME, serverOptions, clientOptions);
    client.onDidChangeState((e) => {
        if (e.newState === node_1.State.Running) {
            updateLspStatusBar('running');
            outputChannel.appendLine('Language server is running.');
        }
        else if (e.newState === node_1.State.Stopped) {
            updateLspStatusBar('stopped');
            outputChannel.appendLine('Language server stopped.');
        }
        else if (e.newState === node_1.State.Starting) {
            updateLspStatusBar('starting');
    ...
    try {
        ...
        updateLspStatusBar('error');
        vscode.window.showErrorMessage('Omnisystem: Failed to start language server. Check the Output panel for details.');
    }
}
...
// ─── LSP path resolution ──────────────────────────────────────────────────────
function resolveLspServerPath(config) {
    const configured = config.get('lspServerPath', '').trim();
    if (configured && fs.existsSync(configured)) {
        return configured;
    }
    const cc = config.get('omniccPath', 'omnicc').trim();
...
function buildServerOptions(lspPath, _config) {
    // lspPath is either:
    ...
    if (lspPath.endsWith('.js')) {
        return {
            run: { command: 'node', args: [lspPath, 'lsp', '--stdio'], transport: node_1.TransportKind.stdio },
            debug: { command: 'node', args: [lspPath, 'lsp', '--stdio', '--debug'], transport: node_1.TransportKind.stdio },
        };
    }
    // .cmd / .ps1 / bare binary — pass through shell
    return {
        run: { command: lspPath, args: ['lsp', '--stdio'], transport: node_1.TransportKind.stdio, options: { shell: true } },
        debug: { command: lspPath, args: ['lsp', '--stdio', '--debug'], transport: node_1.TransportKind.stdio, options: { shell: true } },
    };
}
// ─── File watchers ────────────────────────────────────────────────────────────
function registerFileWatchers(ctx) {
...
// ─── Core build helper ────────────────────────────────────────────────────────
async function runCommandWithProgress(label, args, cwd) {
    setBuildStatus('running');
    outputChannel.show(true);
    outputChannel.appendLine(`\n[${label}] ${omniccPath()} ${args.join(' ')}`);
    outputChannel.appendLine('─'.repeat(60));
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: label, cancellable: true }, (progress, token) => new Promise((resolve, reject) => {
        const proc = (0, child_process_1.spawn)(omniccPath(), args, { cwd: cwd ?? workspaceRoot(), shell: true });
    ...
    })).then(() => { }, (err) => vscode.window.showErrorMessage(`Omnisystem: ${err.message}`));
}
// ─── Terminal runners ─────────────────────────────────────────────────────────
/** Run an omnicc subcommand in a terminal (prepends omnicc path). */
function runInTerminal(subArgs, terminalName) {
    const config = vscode.workspace.getConfiguration(EXTENSION_ID);
...
function runRawInTerminal(command, terminalName) {
    const cwd = workspaceRoot();
...
function resolvedOmniccPath() {
    const configured = vscode.workspace.getConfiguration(EXTENSION_ID).get('omniccPath', '').trim();
...
async function requireOmnicc() {
    if (omniccAvailable()) {
        return true;
    }
    const action = await vscode.window.showWarningMessage('Omnisystem: omnicc compiler not found. Set the path to the omnicc binary to use build commands.', 'Configure Path', 'Dismiss');
...
function harnessServerUrl() {
    return vscode.workspace.getConfiguration(EXTENSION_ID).get('harness.serverUrl', 'http://localhost:8080');
}
async function cmdSubstrateSwarm() {
    const { OmniHarnessClient } = await Promise.resolve().then(() => __importStar(require('./harness/OmniHarnessClient')));
    ...
    const topology = await vscode.window.showQuickPick(['orchestrator', 'debate', 'parallel', 'pipeline'], { placeHolder: 'Swarm topology' });
    if (!topology) {
        return;
    }
    const model = vscode.workspace.getConfiguration(EXTENSION_ID).get('harness.defaultModel', 'anthropic/claude-sonnet-4-6');
    const agents = [
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Running ${topology} swarm…`, cancellable: false }, async () => {
        try {
...
async function cmdSubstrateEnsemble() {
    const { OmniHarnessClient } = await Promise.resolve().then(() => __importStar(require('./harness/OmniHarnessClient')));
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Blending ${models.length} models…`, cancellable: false }, async () => {
        try {
...
async function cmdBuild() {
    if (!omniccAvailable()) {
        ...
        BuildDashboard_1.BuildDashboardPanel.createOrShow(extensionContext.extensionUri);
        return;
    }
    const config = vscode.workspace.getConfiguration(EXTENSION_ID);
    const target = config.get('buildTarget', 'x86_64-linux');
...
async function cmdBuildFile() {
    if (!await requireOmnicc()) {
        return;
    }
...
function bootstrapLauncher() {
    ...
    const cfg = vscode.workspace.getConfiguration(EXTENSION_ID).get('bootstrapPath', '').trim();
    if (cfg) {
        candidates.unshift(cfg);
    }
...
async function cmdRunTitanBootstrap() {
...
async function cmdOmniOsBoot() {
    ...
    const wsRoot = (() => {
        ...
        if (fs.existsSync(pathMod.join(sub, 'BUILD.omnisystem'))) {
            return sub;
        }
        if (fs.existsSync(pathMod.join(wsRootRaw, 'BUILD.omnisystem'))) {
            return wsRootRaw;
        }
        return sub;
    })();
    ...
    OmniOSDesktop_1.OmniOSDesktopPanel.createOrShow(extensionContext.extensionUri, extensionContext);
    setTimeout(() => {
        OmniOSDesktop_1.OmniOSDesktopPanel.postMessage({
            type: 'sandboxInit',
            vaultId,
            immuneId,
            platform,
...
function cmdOmniOsSystemServices() {
    OmniOSDesktop_1.OmniOSDesktopPanel.createOrShow(extensionContext.extensionUri, extensionContext);
}
function cmdOmniOsKernelLog() {
    outputChannel.show(true);
    outputChannel.appendLine('\n[OmniOS Kernel Log — streamed from OmniOS Desktop]');
    OmniOSDesktop_1.OmniOSDesktopPanel.createOrShow(extensionContext.extensionUri, extensionContext);
}
async function cmdOmniOsSelectPlatform() {
    const platforms = [
...
async function cmdOmniOsPackageOs() {
    if (!await requireOmnicc()) {
        return;
    }
    runInTerminal(['omnios', 'package'], 'Omnisystem: omnios package');
}
function cmdOpenSystemsExplorer() {
    vscode.commands.executeCommand('omniOsExplorer.focus');
}
// ─── OmniPM commands ──────────────────────────────────────────────────────────
async function cmdOmniPMInstall() {
    if (!omniccAvailable()) {
        // omnicc not configured — refresh the OmniPM tree view and focus it
        omnipmProvider.refresh();
        await vscode.commands.executeCommand('omnipmExplorer.focus');
        vscode.window.showInformationMessage('OmniPM: Dependencies listed in BUILD.omnisystem. Configure omnicc to run install.', 'Configure omnicc').then(action => {
...
async function cmdOmniPMRemove() {
...
async function cmdDesktopInit() {
    const pick = await vscode.window.showQuickPick(['Full Initialization', 'Diagnostics Mode', 'Repair Mode', 'Graceful Shutdown'], { placeHolder: 'Select Desktop Ecosystem operation', title: 'Desktop Initialization' });
    if (!pick) {
        return;
    }
...
async function cmdDesktopDeploy() {
    runRawInTerminal('npm run deploy', 'Desktop: Deploy');
}
function cmdDesktopModelManager() {
    MlStudio_1.MlStudioPanel.createOrShow(extensionContext.extensionUri);
}
function cmdDesktopWorkspace() {
    OmnisystemDashboard_1.OmnisystemDashboardPanel.createOrShow(extensionContext.extensionUri);
}
// ─── Workspace IDE ─────────────────────────────────────────────────────────────
...
let workspaceIdeProc;
function cmdOpenWorkspaceIde() {
    ...
    const proc = (0, child_process_1.spawn)('npm', ['run', 'tauri', 'dev'], { cwd: dir, shell: true });
...
async function scaffoldFile(lang) {
...
async function scaffoldDesktopApp() {
    const name = await vscode.window.showInputBox({
    ...
    try {
        ...
        await vscode.workspace.fs.writeFile(vscode.Uri.joinPath(appDir, APP_MANIFEST_NAMES[0]), Buffer.from(toml, 'utf8'));
        await vscode.workspace.fs.writeFile(vscode.Uri.joinPath(appDir, `${name.trim()}.titan`), Buffer.from(mainTs, 'utf8'));
...
async function scaffoldOmniOsService() {
    const name = await vscode.window.showInputBox({
    ...
    const template = `// ${name}.titan — OmniOS Service
service ${name} {
    fn start() {
...
function cmdOpenBuildDashboard() {
    BuildDashboard_1.BuildDashboardPanel.createOrShow(extensionContext.extensionUri);
}
function cmdOpenMlStudio() {
    MlStudio_1.MlStudioPanel.createOrShow(extensionContext.extensionUri);
}
function cmdOpenShaderPreview() {
    ShaderPreview_1.ShaderPreviewPanel.createOrShow(extensionContext.extensionUri);
}
function cmdOpenLayoutPreview() {
    LayoutPreview_1.LayoutPreviewPanel.createOrShow(extensionContext.extensionUri);
}
function cmdOpenOmniOsDesktop() {
    OmniOSDesktop_1.OmniOSDesktopPanel.createOrShow(extensionContext.extensionUri, extensionContext);
}
function cmdOpenWidgetGallery() {
    WidgetGallery_1.WidgetGalleryPanel.createOrShow(extensionContext.extensionUri);
}
function cmdOpenWelcome() {
    WelcomePanel_1.WelcomePanel.createOrShow(extensionContext.extensionUri);
}
// ─── OmniCC editor commands ───────────────────────────────────────────────────
async function cmdOmniccConvertSelection() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showWarningMessage('OmniCC: No active editor.');
        return;
    }
    const selection = editor.selection;
    const source = editor.document.getText(selection.isEmpty ? undefined : selection);
    if (!source.trim()) {
        vscode.window.showWarningMessage('OmniCC: Nothing to convert.');
        return;
    }
    const { quickConvert } = await Promise.resolve().then(() => __importStar(require('./omnicc/ConversionEngine')));
    const { allLanguages } = await Promise.resolve().then(() => __importStar(require('./omnicc/LanguageRegistry')));
    ...
    if (!result.success) {
        vscode.window.showErrorMessage(`OmniCC conversion failed: ${result.error ?? 'unknown error'}`);
        return;
    }
    const doc = await vscode.workspace.openTextDocument({ content: result.output, language: picked.description });
    await vscode.window.showTextDocument(doc, { viewColumn: vscode.ViewColumn.Beside, preview: false });
}
async function cmdOmniccConvertFile() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showWarningMessage('OmniCC: No active editor.');
        return;
...
async function cmdOmniccConvertFileAs() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showWarningMessage('OmniCC: No active editor.');
        return;
...
async function _omniccConvertDocument(doc, targetLangId) {
    const { quickConvert } = await Promise.resolve().then(() => __importStar(require('./omnicc/ConversionEngine')));
    const { allLanguages } = await Promise.resolve().then(() => __importStar(require('./omnicc/LanguageRegistry')));
...
// ─── Theme IPC broadcast ──────────────────────────────────────────────────────
function broadcastTheme(themeId) {
    const msg = { type: 'owThemeSync', theme: themeId };
    const senders = [
        () => OmnisystemDashboard_1.OmnisystemDashboardPanel.postMessage(msg),
        () => BuildDashboard_1.BuildDashboardPanel.postMessage(msg),
        () => MlStudio_1.MlStudioPanel.postMessage(msg),
        () => ShaderPreview_1.ShaderPreviewPanel.postMessage(msg),
        () => LayoutPreview_1.LayoutPreviewPanel.postMessage(msg),
        () => WidgetGallery_1.WidgetGalleryPanel.postMessage(msg),
        () => WelcomePanel_1.WelcomePanel.postMessage(msg),
        () => WidgetConverter_1.WidgetConverterPanel.postMessage(msg),
        () => OmniCCDashboard_1.OmniCCDashboardPanel.postMessage(msg),
    ];
    ...
    extensionContext.globalState.update('omnisystem.owTheme', themeId).then(undefined, (e) => outputChannel.appendLine(`[Theme IPC] Persist error: ${e}`));
    outputChannel.appendLine(`[Theme IPC] Broadcast: ${themeId}`);
}
function registerThemeIpcListener() {
    OmniOSDesktop_1.OmniOSDesktopPanel.onThemeChange = broadcastTheme;
}
// ─── VS Code workspace control commands ──────────────────────────────────────
async function cmdApplyOmnisystemTheme() {
    await vscode.workspace.getConfiguration('workbench').update('colorTheme', 'Omnisystem Dark', true);
    vscode.window.showInformationMessage('Omnisystem: Theme applied.');
}
async function cmdOptimizeLayout() {
    const wsConfig = vscode.workspace.getConfiguration();
...
function cmdToggleZenMode() {
    vscode.commands.executeCommand('workbench.action.toggleZenMode');
}
function cmdFocusExplorer() {
    vscode.commands.executeCommand('omnisystemExplorer.focus');
}
function cmdSplitEditorVertical() {
    vscode.commands.executeCommand('workbench.action.splitEditorRight');
}
function cmdShowAllSystems() {
    vscode.commands.executeCommand('omniOsExplorer.focus');
    omniosProvider.refresh();
}
// ─── Command registration ─────────────────────────────────────────────────────
function registerCommands(ctx) {
    const reg = (id, handler) => ctx.subscriptions.push(vscode.commands.registerCommand(id, handler));
    // Build & Run
    ...
    reg('omnisystem.widgetConverter', () => WidgetConverter_1.WidgetConverterPanel.createOrShow(ctx.extensionUri));
    reg('omnisystem.omniCC', () => OmniCCDashboard_1.OmniCCDashboardPanel.createOrShow(ctx.extensionUri));
    reg('omnisystem.openWelcome', () => cmdOpenWelcome());
    // Internal IPC command — triggered by panels when user changes OW theme
    reg('omnisystem._broadcastTheme', (themeId) => {
    ...
    reg('omnisystem.packageUpdate', (pkg) => {
        const name = typeof pkg === 'string' ? pkg : pkg?.label ?? '';
        if (name) {
            runInTerminal(['pm', 'update', name]);
        }
    });
    reg('omnisystem.packageRemove', (pkg) => {
        const name = typeof pkg === 'string' ? pkg : pkg?.label ?? '';
        if (name) {
            runInTerminal(['pm', 'remove', name]);
        }
    });
    // File open helper
    reg('omnisystem.openFile', async (uri) => {
    ...
    reg('omnisystem.check', async () => { if (await requireOmnicc()) {
        runInTerminal(['check']);
    } });
    reg('omnisystem.format', async () => { if (await requireOmnicc()) {
        runInTerminal(['fmt', '--all']);
    } });
    ...
    provideCodeActions(document, range, context) {
        ...
        for (const diag of context.diagnostics) {
            ...
            if (unknownSymbol) {
                ...
                const action = new vscode.CodeAction(`Add import for '${sym}'`, vscode.CodeActionKind.QuickFix);
                action.diagnostics = [diag];
                action.edit = new vscode.WorkspaceEdit();
                // Insert import at top of file
                action.edit.insert(document.uri, new vscode.Position(0, 0), `import ${sym};\n`);
            ...
            if (unusedVar) {
                ...
                const action = new vscode.CodeAction(`Prefix '${varName}' with _ to suppress warning`, vscode.CodeActionKind.QuickFix);
                action.diagnostics = [diag];
                action.edit = new vscode.WorkspaceEdit();
                const lineText = document.lineAt(diag.range.start.line).text;
                const idx = lineText.indexOf(varName);
                if (idx >= 0) {
                    action.edit.replace(document.uri, new vscode.Range(diag.range.start.line, idx, diag.range.start.line, idx + varName.length), `_${varName}`);
                }
                actions.push(action);
            }
...
class OmniSemanticTokensProvider {
    constructor(legend) {
        this.legend = legend;
    }
