...
exports.BuildEditorProvider = void 0;
const vscode = __importStar(require("vscode"));
// ── Provider ─────────────────────────────────────────────────────────────────
class BuildEditorProvider {
    static register(context) {
        const provider = new BuildEditorProvider(context);
        return vscode.window.registerCustomEditorProvider(BuildEditorProvider.viewType, provider, {
        ...
        for (const raw of lines) {
            ...
            if (section === 'project') {
                ...
                if (key === 'version')
                    cfg.version = val;
                if (key === 'authors')
                    cfg.authors = this._parseList(val);
            }
            else if (section === 'build') {
                if (key === 'sources')
                    cfg.sources = this._parseList(val);
                if (key === 'targets')
                    cfg.targets = this._parseList(val);
                if (key === 'opt')
                    cfg.optimizationLevel = val ?? 'debug';
                if (key === 'jobs')
                    cfg.parallelJobs = parseInt(val, 10) || 4;
            }
    ...
    async _applyEdits(document, cfg) {
...
exports.BuildEditorProvider = BuildEditorProvider;
BuildEditorProvider.viewType = 'omnisystem.buildEditor';
//# sourceMappingURL=BuildEditor.js.map