...
// ─── Nexus layout → HTML/CSS converter ───────────────────────────────────────

interface LayoutBox {
    type: 'grid' | 'flex' | 'stack' | 'container' | 'box' | 'header' | 'footer' | 'sidebar' | 'content' | 'nav';
    id?: string;
    columns?: number;
    rows?: number;
    gap?: string;
...
function parseNexusLayout(source: string): LayoutBox {
    // Simple heuristic parser — extracts named containers and grids
    const root: LayoutBox = { type: 'container', label: 'Root', children: [] };
    ...
    while ((m = blockRe.exec(source)) !== null) {
        ...
        if (['layout', 'root', 'app'].includes(kind)) continue;

        const spanMatch = body.match(/span:\s*(\d+)/);
        const child: LayoutBox = {
            type: kind as LayoutBox['type'],
            id: name,
            label: name,
    ...
    if (root.children.length === 0) {
        const demos: Array<{ label: string; color: string; span?: number }> = [
...
export class LayoutPreviewPanel {
    ...
    public static readonly viewType = 'omnisystem.layoutPreview';
    ...
    private _loadActiveLayout(): void {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
    ...
    private _analyzeLayout(source: string): object {
        ...
        if (gridCount + flexCount > 5 || responsiveRules > 3) complexity = 'Complex';
        else if (gridCount + flexCount > 2 || responsiveRules > 0) complexity = 'Moderate';
    ...
    private async _handleMessage(message: { command: string; breakpoint?: string }): Promise<void> {
