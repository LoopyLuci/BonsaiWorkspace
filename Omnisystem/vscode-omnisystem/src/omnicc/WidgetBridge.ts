...
import {
    ULIRModule, ULIRUnit,
    OmniCCConversionResult, ConversionOptions,
} from './ULIR';
import { convert as widgetConvert } from '../conversion/WidgetConversionEngine';
...
const WIDGET_KINDS = new Set<ULIRUnit['kind']>([
...
export function detectUIPatterns(ir: ULIRModule): { hasUI: boolean; uiUnits: ULIRUnit[]; confidence: number } {
    const uiUnits: ULIRUnit[] = [];
    let score = 0;

    if (ir.metadata.hasUI) { score += 30; }
...
export function runWidgetBridge(
    ir: ULIRModule,
    targetLangId: string,
    ...
    const widgetResults: ConversionResult[] = [];
    ...
    for (const unit of uiUnits.slice(0, 20)) { // cap at 20 widget units per conversion
        ...
        if (!widgetSource) { continue; }

        const srcLang = mapToWidgetLang(ir.sourceLanguage);
        const tgtLang = mapToWidgetLang(targetLangId);

        if (srcLang && tgtLang) {
            const result = widgetConvert({ source: widgetSource, sourceLang: srcLang, targetLang: tgtLang, widgetNameHint: guessWidgetName(unit) });
            if (result) {
                widgetResults.push(result);
                if (result.notes && result.notes.length > 0) {
                    mergedNotes.push(...result.notes.map((n: string) => `  [${unit.name}] ${n}`));
                }
            }
        } else {
...
export function mergeWidgetBridgeResults(
    mainOutput: string,
    bridgeResult: WidgetBridgeResult,
    targetLangId: string,
): string {
...
function buildWidgetSource(unit: ULIRUnit, langId: string): string | null {
...
function synthesizeWidgetSource(name: string, langId: string): string {
...
function guessWidgetName(unit: ULIRUnit): string {
    const name = unit.name.split('.').pop() ?? unit.name;
...
function mapToWidgetLang(langId: string): string | null {
...
function getLineComment(langId: string): string {
    switch (langId) {
        case 'python': case 'ruby': case 'bash': case 'yaml': return '#';
        case 'html': return '<!--';
        case 'css': case 'scss': return '/*';
        case 'sql': return '--';
        case 'haskell': case 'elm': case 'lua': return '--';
        default: return '//';
    }
}

// ─── OmniCC integration hook ──────────────────────────────────────────────────
...
export function buildBridgeSummary(result: WidgetBridgeResult, ir: ULIRModule): WidgetBridgeSummary {
...
function generatePreviewHtml(result: ConversionResult): string {
