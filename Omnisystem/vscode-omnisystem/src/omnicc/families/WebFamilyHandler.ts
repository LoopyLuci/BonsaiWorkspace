...
import {
    ...
    STRING_TYPE, VOID_TYPE, ANY_TYPE, UNKNOWN_TYPE,
...
export function parseWebFamily(source: string, langId: string): ULIRModule {
    ...
    switch (langId) {
        case 'html': extractHTMLUnits(source, units); break;
        case 'css': case 'scss': case 'less': extractCSSUnits(source, units, langId); break;
        case 'jsx': case 'tsx': case 'javascript': case 'typescript': extractJSXUnits(source, units, imports, langId); break;
        case 'vue': extractVueUnits(source, units, imports); break;
        case 'svelte': extractSvelteUnits(source, units, imports); break;
        case 'angular': extractAngularUnits(source, units, imports); break;
        case 'astro': extractAstroUnits(source, units, imports); break;
        default: extractHTMLUnits(source, units); break;
    }
...
function extractHTMLUnits(source: string, units: ULIRUnit[]): void {
...
function extractCSSUnits(source: string, units: ULIRUnit[], langId: string): void {
    // Rule sets
...
function extractJSXUnits(source: string, units: ULIRUnit[], imports: ULIRImport[], langId: string): void {
...
function extractVueUnits(source: string, units: ULIRUnit[], imports: ULIRImport[]): void {
    // Vue SFC sections
    const templateMatch = source.match(/<template>([\s\S]*?)<\/template>/);
    if (templateMatch) { units.push(makeWebUnit('widget-layout', 'template', templateMatch[0].slice(0, 100))); }

    const scriptMatch = source.match(/<script[^>]*>([\s\S]*?)<\/script>/);
    if (scriptMatch) {
        // Components, methods, computed
        for (const m of scriptMatch[1].matchAll(/(?:methods|computed|setup)\s*\(\s*\)\s*\{|(\w+)\s*\([^)]*\)\s*\{/g)) {
            if (m[1]) { units.push(makeWebUnit('function', m[1], m[0])); }
        }
    ...
    if (styleMatch) { units.push(makeWebUnit('widget-style', 'styles', styleMatch[0].slice(0, 100))); }
}

function extractSvelteUnits(source: string, units: ULIRUnit[], imports: ULIRImport[]): void {
    const scriptMatch = source.match(/<script[^>]*>([\s\S]*?)<\/script>/);
    if (scriptMatch) {
...
function extractAngularUnits(source: string, units: ULIRUnit[], imports: ULIRImport[]): void {
    ...
    for (const m of source.matchAll(/@(Component|Directive|Pipe|Injectable|NgModule)\s*\([^)]*\)[^]*?(?:class|export class)\s+(\w+)/g)) {
        ...
        units.push(makeWebUnit(kind as ULIRUnit['kind'], m[2], m[0].slice(0, 150)));
    }
    // Methods
...
function extractAstroUnits(source: string, units: ULIRUnit[], imports: ULIRImport[]): void {
    // Frontmatter
...
function extractImport(line: string): ULIRImport | null {
...
function makeWebUnit(kind: ULIRUnit['kind'], name: string, src: string): ULIRUnit {
    return {
        kind,
...
function detectModuleName(source: string, langId: string): string {
    ...
    if (a) { return a[1].replace(/^app-/, '').replace(/-(\w)/g, (_, c) => c.toUpperCase()); }
    return 'Component';
}

// ─── Generate ─────────────────────────────────────────────────────────────────

export function generateWebFamily(ir: ULIRModule, targetLangId: string, opts: ConversionOptions = DEFAULT_OPTIONS): string {
    switch (targetLangId) {
        case 'html': return generateHTML(ir, opts);
        case 'css': return generateCSS(ir, opts);
        case 'scss': return generateSCSS(ir, opts);
        case 'jsx': case 'tsx': return generateJSX(ir, opts, targetLangId === 'tsx');
...
function generateHTML(ir: ULIRModule, opts: ConversionOptions): string {
...
function generateCSS(ir: ULIRModule, opts: ConversionOptions): string {
...
function generateSCSS(ir: ULIRModule, opts: ConversionOptions): string {
    ...
    for (const unit of ir.units.filter(u => u.kind === 'function')) {
        ...
        lines.push(translateBody(unit.originalSource ?? '', ir.sourceLanguage, 'scss', '  '));
        lines.push('}');
        lines.push('');
    }
...
function generateJSX(ir: ULIRModule, opts: ConversionOptions, isTS: boolean): string {
    ...
    for (const unit of functions) {
        ...
        const body = translateBody(unit.originalSource ?? '', ir.sourceLanguage, isTS ? 'typescript' : 'javascript', '  ');
...
function generateSvelte(ir: ULIRModule, opts: ConversionOptions): string {
...
function generateAngular(ir: ULIRModule, opts: ConversionOptions): string {
    ...
    return [
        ...
        ...ir.units.filter(u => u.kind === 'function').map(u => {
            ...
            return `  ${u.name}(${pStr}): void {\n${translateBody(u.originalSource ?? '', ir.sourceLanguage, 'typescript', '    ')}\n  }`;
        }),
        '}',
    ].join('\n');
}

function generateAstro(ir: ULIRModule, opts: ConversionOptions): string {
