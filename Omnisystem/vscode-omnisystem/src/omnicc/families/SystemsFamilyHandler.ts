...
export function parseSystemsFamily(source: string, langId: string): ULIRModule {
    ...
    switch (langId) {
        case 'rust': extractRustUnits(source, lines, units, imports); break;
        case 'zig': extractZigUnits(source, lines, units, imports); break;
        case 'go': extractGoUnits(source, lines, units, imports); break;
        case 'odin': extractOdinUnits(source, lines, units, imports); break;
        case 'c': case 'cpp': extractCUnits(source, lines, units, imports, langId); break;
        case 'asm-x86': case 'asm-arm': extractAsmUnits(source, lines, units, langId); break;
        case 'wasm': extractWasmUnits(source, units); break;
        case 'v': extractVLangUnits(source, lines, units, imports); break;
        case 'd': extractDLangUnits(source, lines, units, imports); break;
        default: extractCUnits(source, lines, units, imports, 'c'); break;
    }
...
function extractRustUnits(source: string, lines: string[], units: ULIRUnit[], imports: ULIRImport[]): void {
    // use declarations
    ...
    for (const m of source.matchAll(/^(?:#\[(?:[^\]]+)\]\s*)*(?:pub(?:\([^)]+\))?\s+)?(?:async\s+)?fn\s+(\w+)(?:<([^>]*)>)?\s*\(([^)]*)\)(?:\s*->\s*([\w<>,:&'? ]+))?/gm)) {
        ...
        units.push({
            ...
            signature: {
                params,
                returns: retStr ? { ...UNKNOWN_TYPE, name: retStr.trim(), originalSrc: retStr } : VOID_TYPE,
                throws: [],
            },
            body: [],
            attributes: attrs,
...
function extractZigUnits(source: string, lines: string[], units: ULIRUnit[], imports: ULIRImport[]): void {
    // const imports
        ...
        units.push({
            ...
            signature: {
                params: m[2].split(',').filter(Boolean).map(p => {
                    ...
                    return { name: (parts[0] ?? 'arg').trim(), type: { ...UNKNOWN_TYPE, name: (parts[1] ?? '?').trim(), originalSrc: parts[1] }, defaultValue: undefined, isVariadic: false, isKeyword: false, isRef: false, isMut: false };
                }),
                returns: m[3] ? { ...UNKNOWN_TYPE, name: m[3], originalSrc: m[3] } : VOID_TYPE,
                throws: [],
            },
            body: [], attributes: [], docComment: '',
            sourceLines: [0, 0],
...
function extractGoUnits(source: string, lines: string[], units: ULIRUnit[], imports: ULIRImport[]): void {
    // import blocks
    ...
    for (const m of source.matchAll(/^func\s+(?:\((\w+)\s+\*?(\w+)\)\s+)?(\w+)\s*\(([^)]*)\)(?:\s+\(?[\w,*\[\] ]+\)?)?\s*\{/gm)) {
        ...
        const params = m[4].split(',').filter(Boolean).map(p => {
            ...
            return { name: parts[0] ?? 'arg', type: { ...UNKNOWN_TYPE, name: parts[1] ?? 'interface{}', originalSrc: parts[1] }, defaultValue: undefined, isVariadic: p.includes('...'), isKeyword: false, isRef: false, isMut: false };
        });
        units.push({
            kind: receiver ? 'method' : 'function', name,
            visibility: /^[A-Z]/.test(name) ? 'public' : 'private',
            signature: { params, returns: UNKNOWN_TYPE, throws: [] },
            body: [], attributes: [], docComment: '',
            sourceLines: [0, 0],
...
function extractOdinUnits(source: string, lines: string[], units: ULIRUnit[], imports: ULIRImport[]): void {
...
function extractCUnits(source: string, lines: string[], units: ULIRUnit[], imports: ULIRImport[], langId: string): void {
    // #include
    ...
    for (const m of source.matchAll(/^(?:(?:static|inline|extern|const|volatile|unsigned)\s+)*(?:[\w*]+\s+)+(\w+)\s*\(([^)]*)\)\s*\{/gm)) {
        const name = m[1];
        if (['if', 'while', 'for', 'switch', 'do'].includes(name)) { continue; }
        const params = m[2].split(',').filter(Boolean).map(p => {
            const parts = p.trim().split(/\s+/);
            const name = parts[parts.length - 1]?.replace(/[*&]/, '') ?? 'arg';
            return { name, type: ANY_TYPE, defaultValue: undefined, isVariadic: p.includes('...'), isKeyword: false, isRef: p.includes('&'), isMut: !p.includes('const') };
        });
        units.push({
            kind: 'function', name,
...
function extractAsmUnits(source: string, lines: string[], units: ULIRUnit[], langId: string): void {
    for (const m of source.matchAll(/^(\w+):/gm)) {
        const name = m[1];
        if (['section', 'global', 'extern', 'bits'].includes(name)) { continue; }
        units.push(makeTypeUnit('function', name, m[0], name.startsWith('.') ? 'private' : 'public'));
    }
}

function extractWasmUnits(source: string, units: ULIRUnit[]): void {
...
function extractVLangUnits(source: string, lines: string[], units: ULIRUnit[], imports: ULIRImport[]): void {
...
function extractDLangUnits(source: string, lines: string[], units: ULIRUnit[], imports: ULIRImport[]): void {
    for (const m of source.matchAll(/^import\s+([\w.]+);/gm)) {
        imports.push({ path: m[1].replace(/\./g, '/'), alias: undefined, names: [], isDefault: false, isWildcard: true, kind: 'external', originalSyntax: m[0] });
    }
...
function parseRustParams(raw: string): ULIRParam[] {
    ...
    return raw.split(',').map((p): ULIRParam | null => {
        ...
        if (m) {
            return { name: m[1], type: { ...UNKNOWN_TYPE, name: m[2].trim(), originalSrc: m[2] }, defaultValue: undefined, isVariadic: t.includes('..'), isKeyword: false, isRef: t.includes('&'), isMut: t.includes('mut') };
        }
        return { name: t.replace(/[^a-zA-Z0-9_]/g, '') || 'arg', type: ANY_TYPE, defaultValue: undefined, isVariadic: false, isKeyword: false, isRef: false, isMut: false };
    }).filter((p): p is ULIRParam => p !== null && p.name.length > 0);
}

function extractRustAttrs(lines: string[], source: string, index: number): string[] {
    const attrs: string[] = [];
    const pre = source.slice(Math.max(0, index - 200), index);
    for (const m of pre.matchAll(/#\[([^\]]+)\]/g)) { attrs.push(`#[${m[1]}]`); }
    return attrs.slice(-3);
}

function extractRustDoc(source: string, index: number): string {
    const pre = source.slice(Math.max(0, index - 500), index);
    const docs: string[] = [];
    for (const m of pre.matchAll(/^\/\/\/\s?(.*)$/gm)) { docs.push(m[1]); }
    return docs.slice(-5).join('\n');
}

function extractCDoc(source: string, index: number): string {
    const pre = source.slice(Math.max(0, index - 300), index);
    const m = pre.match(/\/\*\*([\s\S]*?)\*\//);
    return m ? m[1].replace(/^\s*\*\s?/gm, '').trim() : '';
}

function makeTypeUnit(kind: ULIRUnit['kind'], name: string, src: string, vis: ULIRUnit['visibility']): ULIRUnit {
    return {
...
export function generateSystemsFamily(ir: ULIRModule, targetLangId: string, opts: ConversionOptions = DEFAULT_OPTIONS): string {
    switch (targetLangId) {
...
function generateRust(ir: ULIRModule, opts: ConversionOptions): string {
    ...
    for (const unit of ir.units) {
        const body = translateBody(unit.originalSource ?? '', ir.sourceLanguage, 'rust');
        if (unit.kind === 'struct') {
...
function generateZig(ir: ULIRModule, opts: ConversionOptions): string {
    ...
    if (ir.imports.length > 0) { lines.push(''); }
    for (const unit of ir.units) {
        const body = translateBody(unit.originalSource ?? '', ir.sourceLanguage, 'zig');
        if (unit.kind === 'struct') {
...
function generateGo(ir: ULIRModule, opts: ConversionOptions): string {
    ...
    if (ir.imports.length > 0) {
        lines.push('import (');
        for (const imp of ir.imports) { lines.push(`\t"${imp.path}"`); }
        lines.push(')', '');
    }
    for (const unit of ir.units) {
        const name = toPascalCase(unit.name);
        const body = translateBody(unit.originalSource ?? '', ir.sourceLanguage, 'go');
        if (unit.kind === 'struct') {
...
function generateC(ir: ULIRModule, opts: ConversionOptions): string {
    ...
    if (ir.imports.length > 0) { lines.push(''); }
    for (const unit of ir.units) {
        const body = translateBody(unit.originalSource ?? '', ir.sourceLanguage, 'c');
        if (unit.kind === 'struct') {
...
function generateCpp(ir: ULIRModule, opts: ConversionOptions): string {
    ...
    for (const unit of ir.units) {
        const body = translateBody(unit.originalSource ?? '', ir.sourceLanguage, 'cpp');
...
function generateOdin(ir: ULIRModule, opts: ConversionOptions): string {
    ...
    if (ir.imports.length > 0) { lines.push(''); }
    for (const unit of ir.units) {
        const body = translateBody(unit.originalSource ?? '', ir.sourceLanguage, 'odin');
        if (unit.kind === 'struct') {
...
function generateV(ir: ULIRModule, opts: ConversionOptions): string {
    const lines = [`// ${ir.name} — Converted to V`, `// Source: ${ir.sourceLanguage}`, '', `module ${ir.name.toLowerCase()}`, ''];
    for (const imp of ir.imports) { lines.push(`import ${imp.path.replace(/\//g, '.')}`); }
    if (ir.imports.length > 0) { lines.push(''); }
    for (const unit of ir.units) {
        const body = translateBody(unit.originalSource ?? '', ir.sourceLanguage, 'v');
        if (unit.kind === 'struct') {
            const fields = unit.children.length > 0
                ? unit.children.map(c => `    ${toSnakeCase(c.name)} string`).join('\n')
                : (body || '    data string');
