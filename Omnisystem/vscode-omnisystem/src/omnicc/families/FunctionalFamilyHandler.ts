...
export function parseFunctionalFamily(source: string, langId: string): ULIRModule {
...
function extractHaskellDefs(source: string, lines: string[], units: ULIRUnit[]): void {
    ...
    for (const [name, typeSig] of sigMap) {
        ...
        if (new RegExp(`^${name}\\s+`).test(source)) {
            units.push(makeFnUnit(name, [], UNKNOWN_TYPE, 'haskell', `-- ${name} :: ${typeSig}`));
        }
    }
    // Data types
    for (const m of source.matchAll(/^data\s+(\w+)/gm)) {
        units.push({ kind: 'type-alias', name: m[1], visibility: 'public', signature: { params: [], returns: VOID_TYPE, throws: [] }, body: [], attributes: [], docComment: '', sourceLines: [0, 0], isAsync: false, isStatic: false, isAbstract: false, isFinal: false, isOverride: false, isExtern: false, generics: [], extends_: [], implements_: [], children: [], originalSource: m[0], confidence: 'high' });
    }
}

function extractMLDefs(source: string, lines: string[], units: ULIRUnit[], langId: string): void {
    // OCaml/F#: let name args = body
    for (const m of source.matchAll(/^let\s+(?:rec\s+)?(\w+)\s*((?:\w+\s+)*)/gm)) {
        const name = m[1];
        if (['in', 'and', 'type', 'module', 'open'].includes(name)) { continue; }
        const paramNames = m[2].trim().split(/\s+/).filter(Boolean);
        units.push(makeFnUnit(name, paramNames.map(n => ({ name: n, type: ANY_TYPE, defaultValue: undefined, isVariadic: false, isKeyword: false, isRef: false, isMut: false })), UNKNOWN_TYPE, langId, m[0]));
    }
    // Type definitions
...
function extractLispDefs(source: string, lines: string[], units: ULIRUnit[], langId: string): void {
    ...
    for (const p of patterns) {
        for (const m of source.matchAll(p)) {
            ...
            const params = paramStr.trim().split(/\s+/).filter(Boolean)
                .map(n => ({ name: n, type: ANY_TYPE, defaultValue: undefined, isVariadic: n.startsWith('&'), isKeyword: n.startsWith(':'), isRef: false, isMut: false }));
            units.push(makeFnUnit(name, params, UNKNOWN_TYPE, langId, m[0]));
        }
    }
}

function extractElmDefs(source: string, lines: string[], units: ULIRUnit[]): void {
    // Elm: name = ... / name params = ...
    for (const m of source.matchAll(/^(\w+)\s+((?:\w+\s+)*)?=/gm)) {
        const name = m[1];
        if (name === 'type' || name === 'import' || name === 'module') { continue; }
        units.push(makeFnUnit(name, [], UNKNOWN_TYPE, 'elm', m[0]));
    }
}

function extractBeamDefs(source: string, lines: string[], units: ULIRUnit[], langId: string): void {
    if (langId === 'elixir') {
        for (const m of source.matchAll(/def\s+(\w+)\(([^)]*)\)/g)) {
            const params = m[2] ? m[2].split(',').map(p => ({ name: p.trim().split(':')[0].trim() || 'arg', type: ANY_TYPE, defaultValue: undefined, isVariadic: false, isKeyword: false, isRef: false, isMut: false })) : [];
            units.push(makeFnUnit(m[1], params, UNKNOWN_TYPE, 'elixir', m[0]));
        }
    } else {
        for (const m of source.matchAll(/^(\w+)\(([^)]*)\)\s*->/gm)) {
            units.push(makeFnUnit(m[1], [], UNKNOWN_TYPE, 'erlang', m[0]));
        }
    }
}

function makeFnUnit(name: string, params: ULIRParam[], ret: ULIRType, langId: string, src: string): ULIRUnit {
    return {
        kind: 'function',
        name,
        visibility: 'public',
...
export function generateFunctionalFamily(ir: ULIRModule, targetLangId: string, opts: ConversionOptions = DEFAULT_OPTIONS): string {
    ...
    if (modDecl) { lines.push(modDecl); lines.push(''); }
...
function renderModuleDecl(name: string, lang: string): string {
    switch (lang) {
...
function renderImport(imp: ULIRImport, lang: string): string {
    ...
    switch (lang) {
        case 'haskell':
            if (imp.names.length > 0) { return `import ${path} (${imp.names.join(', ')})`; }
            return `import ${path}`;
        ...
        case 'elm':
            if (imp.names.length > 0) { return `import ${path} exposing (${imp.names.join(', ')})`; }
            return `import ${path}`;
...
function renderUnit(unit: ULIRUnit, lang: string, opts: ConversionOptions, srcLang?: string): string {
    if (unit.kind === 'type-alias') { return renderTypeAlias(unit, lang); }
...
function renderTypeAlias(unit: ULIRUnit, lang: string): string {
    switch (lang) {
