...
import {
    ULIRModule, ULIRUnit, ULIRStatement, ULIRParam, ULIRType,
    ULIRImport, ULIRMetadata, ConversionConfidence,
    STRING_TYPE, VOID_TYPE, BOOL_TYPE, INT_TYPE, ANY_TYPE, UNKNOWN_TYPE,
    rawExpr, DEFAULT_OPTIONS, ConversionOptions,
} from '../ULIR';
...
export function parseCFamily(source: string, langId: string): ULIRModule {
...
function parseFunctionBlock(header: string, body: string, langId: string): ULIRUnit | null {
...
function parseClassBlock(lines: string[], startIdx: number, langId: string): ULIRUnit | null {
...
function parseParamList(raw: string, langId: string): ULIRParam[] {
    if (!raw.trim()) { return []; }
    return raw.split(',').map(p => {
        ...
        if (m) {
            return { name: m[1], type: { ...STRING_TYPE, name: m[2], originalSrc: m[2] }, defaultValue: undefined, isVariadic: t.includes('...'), isKeyword: false, isRef: false, isMut: false };
        }
        // Java/C#: Type name
        m = t.match(/^([\w<>?,\[\]]+)\s+(\w+)$/);
        if (m) {
            return { name: m[2], type: { ...STRING_TYPE, name: m[1], originalSrc: m[1] }, defaultValue: undefined, isVariadic: false, isKeyword: false, isRef: false, isMut: false };
        }
        // Fallback: just name
        return { name: t.replace(/[^a-zA-Z0-9_]/g, '') || 'param', type: UNKNOWN_TYPE, defaultValue: undefined, isVariadic: false, isKeyword: false, isRef: false, isMut: false };
    }).filter(p => p.name.length > 0);
}

function extractReturnType(header: string, langId: string): ULIRType {
    // TypeScript: ): ReturnType
    let m = header.match(/\)\s*:\s*([\w<>?,\[\]|]+)/);
    if (m) { return { ...VOID_TYPE, name: m[1], originalSrc: m[1] }; }
    ...
    if (m && !['void','int','string','bool','boolean','float','double','char','byte','long','short'].includes(m[1].toLowerCase()) && !/\bclass\b|\bstruct\b/.test(m[1])) {
        return { ...VOID_TYPE, name: m[1], originalSrc: m[1] };
    }
    if (/\bvoid\b/i.test(header)) { return VOID_TYPE; }
    return UNKNOWN_TYPE;
}

function extractVisibility(header: string): ULIRUnit['visibility'] {
    if (/\bprivate\b/.test(header)) { return 'private'; }
    if (/\bprotected\b/.test(header)) { return 'protected'; }
    if (/\binternal\b/.test(header)) { return 'internal'; }
    if (/\bpublic\b|\bexport\b/.test(header)) { return 'public'; }
    return 'public'; // JS/TS default
}

function extractAttributes(header: string): string[] {
    const attrs: string[] = [];
...
function extractExports(source: string, langId: string): string[] {
    ...
    for (const m of source.matchAll(/module\.exports\s*=\s*\{([^}]+)\}/g)) {
        for (const n of m[1].split(',')) { exports.push(n.trim().split(':')[0].trim()); }
    }
    return exports;
}

function detectEntryPoint(source: string, langId: string): string | undefined {
    if (/\bfn main\b/.test(source)) { return 'main'; }
    if (/\bpublic static void main\b/.test(source)) { return 'main'; }
    if (/\bfunc main\b/.test(source)) { return 'main'; }
    if (/\bfunction main\b/.test(source) || /\bmain\(\)/.test(source)) { return 'main'; }
    return undefined;
}
...
function extractModuleDoc(source: string): string {
...
export function generateCFamily(ir: ULIRModule, targetLangId: string, opts: ConversionOptions = DEFAULT_OPTIONS): string {
...
function renderImport(imp: ULIRImport, targetLangId: string): string {
    const path = imp.path;
    switch (targetLangId) {
        case 'javascript':
            if (imp.isWildcard) { return `import * as ${imp.alias ?? 'mod'} from '${path}';`; }
            if (imp.names.length > 0) { return `import { ${imp.names.join(', ')} } from '${path}';`; }
            return `import '${path}';`;
        case 'typescript':
            if (imp.isWildcard) { return `import * as ${imp.alias ?? 'mod'} from '${path}';`; }
            if (imp.names.length > 0) { return `import { ${imp.names.join(', ')} } from '${path}';`; }
            return `import '${path}';`;
        case 'python':
            if (imp.names.length > 0) { return `from ${path.replace(/[/\\]/g, '.')} import ${imp.names.join(', ')}`; }
...
function renderUnit(unit: ULIRUnit, targetLangId: string, opts: ConversionOptions, srcLang?: string): string {
...
function renderClass(unit: ULIRUnit, lang: string, opts: ConversionOptions, srcLang?: string): string {
    const name = unit.name;
    const src = srcLang ?? unit.sourceLanguage ?? 'javascript';
    const body = translateBody(unit.originalSource ?? '', src, lang);
...
function mapType(t: ULIRType, lang: string): string {
    ...
    if (t.isArray) { return wrapArray(n, lang); }
    return n; // pass through unknown type names
}

function wrapArray(base: string, lang: string): string {
    switch (lang) {
