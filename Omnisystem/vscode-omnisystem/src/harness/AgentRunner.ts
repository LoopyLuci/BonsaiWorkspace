...
import { OmniHarnessClient, HarnessMessage, ToolCall, FunctionSchema } from './OmniHarnessClient';
import { VscodeTools, ToolDef, ToolDiff, ToolCheckpoint } from './VscodeTools';
...
export interface ExternalTools {
    functionSchemas(): FunctionSchema[];        // for native function calling
    describe(): string;                         // prose block for the text protocol
    has(name: string): boolean;
    execute(name: string, args: Record<string, unknown>): Promise<{ ok: boolean; summary: string; content: string }>;
}

export interface AgentEvents {
    onAssistantDelta: (text: string) => void;   // streaming assistant tokens
    onAssistantDone: (fullText: string) => void; // a full assistant turn finished
...
export class AgentRunner {
    ...
    constructor(
        private readonly client: OmniHarnessClient,
        private readonly tools: VscodeTools,
        private readonly events: AgentEvents,
        private readonly external?: ExternalTools,
    ) {}

    abort(): void { this.aborted = true; }

    /** MCP tools are offered when the agent allows all tools (tools = ['*']). */
    private externalEnabled(agent: AgentPreset): boolean {
    ...
    private async execTool(name: string, args: Record<string, unknown>): Promise<ExecOutcome> {
    ...
    private allowedTools(agent: AgentPreset): ToolDef[] {
        const all = VscodeTools.catalog();
        if (agent.tools.length === 1 && agent.tools[0] === '*') { return all; }
        const set = new Set(agent.tools as string[]);
        return all.filter((t) => set.has(t.name));
    }

    private parseAction(text: string, agent: AgentPreset): ParsedAction {
        ...
        if (typeof obj.tool === 'string') {
            ...
            return { kind: 'tool', tool: obj.tool, args: (obj.args as Record<string, unknown>) ?? {} };
        }
        return { kind: 'none', text };
    }
    ...
    async run(
        ...
        const messages: HarnessMessage[] = [...history, { role: 'user', content: userText }];

        for (let step = 0; step < maxSteps; step++) {
            if (this.aborted) { this.events.onStatus('Stopped.'); return; }
            ...
            try {
                full = await this.client.chatStream(
                    ...
                    (delta) => { if (!this.aborted) { this.events.onAssistantDelta(delta); } },
                );
            } catch (err) {
    ...
    async runNative(
        ...
        const schemas = [
            ...VscodeTools.toFunctionSchemas(agent.tools as string[]),
        ...
        for (let step = 0; step < maxSteps; step++) {
            ...
            const calls: ToolCall[] = res.tool_calls ?? [];
            if (calls.length === 0) {
    ...
    private buildNativeSystemPrompt(agent: AgentPreset): string {
        return [
            agent.systemPrompt.trim(),
            '',
            'You are operating as an autonomous agent inside the user\'s VS Code workspace.',
    ...
    async chatOnce(
        ...
        try {
            const full = await this.client.chatStream(
                { model_id: model, messages, system, temperature, max_tokens: maxTokens },
                (delta) => { if (!this.aborted) { this.events.onAssistantDelta(delta); } },
            );
            this.events.onAssistantDone(full);
            this.events.onFinal(full);
