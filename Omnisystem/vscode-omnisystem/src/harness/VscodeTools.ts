...
export interface ToolDef {
    ...
    description: string;
    params: ToolParam[];
    mutating: boolean;     // requires approval unless auto-approved
}
...
export interface ToolCheckpoint {
    fsPath: string;
    before: string | null; // null = the file did not exist before this call
}

export interface ToolResult {
    ok: boolean;
    summary: string;       // short line shown in the tool card
    content?: string;      // full output fed back to the model
    error?: string;
    diff?: ToolDiff;        // present on successful write_file/edit_file — renders a real diff
    checkpoint?: ToolCheckpoint; // present on successful write_file/edit_file — enables Undo
}
...
export class VscodeTools {
    constructor(private readonly approve: ApprovalFn) {}
     ...
     */
    static toFunctionSchemas(allowed?: string[]): Array<{ name: string; description: string; parameters: Record<string, unknown> }> {
    ...
    private resolve(p: string): vscode.Uri {
        ...
        if (!root) { return vscode.Uri.file(p); }
    ...
    private async readFile(args: Record<string, unknown>): Promise<ToolResult> {
        ...
        if (!p) { return { ok: false, summary: 'read_file: missing path', error: 'path is required' }; }
    ...
    private async search(args: Record<string, unknown>): Promise<ToolResult> {
        ...
        if (!query) { return { ok: false, summary: 'search: missing query', error: 'query is required' }; }
        ...
        for (const file of files) {
            ...
            try {
                ...
                if (bytes.byteLength > MAX_READ_BYTES) { continue; }
    ...
    private async openFile(args: Record<string, unknown>): Promise<ToolResult> {
        const p = String(args.path ?? '');
        if (!p) { return { ok: false, summary: 'open_file: missing path', error: 'path is required' }; }
        const uri = this.resolve(p);
    ...
    private async getDiagnostics(args: Record<string, unknown>): Promise<ToolResult> {
        ...
        const format = (uri: vscode.Uri, diags: readonly vscode.Diagnostic[]) =>
            diags.map((d) => `${this.rel(uri)}:${d.range.start.line + 1}:${d.range.start.character + 1}: ${sev(d.severity)}: ${d.message}`);
        ...
        if (args.path) {
            ...
            for (const [uri, diags] of vscode.languages.getDiagnostics()) {
                if (diags.length) { lines.push(...format(uri, diags)); }
    ...
    private async getSelection(): Promise<ToolResult> {
        const editor = vscode.window.activeTextEditor;
        if (!editor) { return { ok: true, summary: 'No active editor', content: '(no active editor)' }; }
    ...
    private async writeFile(args: Record<string, unknown>): Promise<ToolResult> {
        ...
        if (!p) { return { ok: false, summary: 'write_file: missing path', error: 'path is required' }; }
        const uri = this.resolve(p);
        ...
        const diff: ToolDiff = { relPath: this.rel(uri), before: beforeContent ?? '', after: content };
    ...
    private async editFile(args: Record<string, unknown>): Promise<ToolResult> {
        ...
        if (!p || !oldText) { return { ok: false, summary: 'edit_file: missing args', error: 'path and old_text are required' }; }
        const uri = this.resolve(p);
        const doc = await vscode.workspace.openTextDocument(uri);
        ...
        const diff: ToolDiff = { relPath: this.rel(uri), before: oldText, after: newText };
        ...
        if (!applied) { return { ok: false, summary: 'edit_file failed', error: 'applyEdit returned false' }; }
        await doc.save();
        return {
            ok: true,
    ...
    private async runCommand(args: Record<string, unknown>): Promise<ToolResult> {
        ...
        if (!command) { return { ok: false, summary: 'run_command: missing command', error: 'command is required' }; }
        ...
        return new Promise<ToolResult>((resolve) => {
            ...
            let out = '';
            let err = '';
            const cap = (buf: Buffer, isErr: boolean) => {
                if (isErr) { err += buf.toString(); } else { out += buf.toString(); }
                if (out.length + err.length > MAX_CMD_OUTPUT) { child.kill(); }
            };
            child.stdout.on('data', (d: Buffer) => cap(d, false));
            child.stderr.on('data', (d: Buffer) => cap(d, true));
