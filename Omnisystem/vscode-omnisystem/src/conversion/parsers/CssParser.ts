...
import {
    WidgetIR, WidgetNode, WidgetKind, WidgetStyle, ConversionConfidence, makeId,
} from '../WidgetIR';

const CLASS_TO_KIND: Array<[RegExp, WidgetKind]> = [
...
function cssPropsToWidgetStyle(props: Record<string, string>): WidgetStyle {
    const style: WidgetStyle = {};
    const map: Record<string, keyof WidgetStyle> = {
    ...
    for (const [cssProp, irProp] of Object.entries(map)) {
        if (props[cssProp]) {
            (style as Record<string, string>)[irProp] = props[cssProp];
        }
    }
    return style;
}

function classNameToKind(className: string): WidgetKind {
    for (const [rx, kind] of CLASS_TO_KIND) {
        if (rx.test(className)) { return kind; }
    }
    return 'unknown';
}

function classNameToHumanName(className: string): string {
    return className
        .replace(/-/g, ' ')
        .replace(/_/g, ' ')
...
export function parseCss(source: string): WidgetIR {
    ...
    const confidence: ConversionConfidence = kind !== 'unknown' ? 'high' : 'medium';
