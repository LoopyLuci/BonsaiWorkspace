// Titan Generator — converts WidgetIR to Titan (OW Runtime) source code
import { WidgetIR, WidgetNode, WidgetKind, WidgetProp, WidgetEvent } from '../WidgetIR';

function propToTitanType(type: string): string {
...
function kindToTitanStruct(kind: WidgetKind): string {
...
function renderStructFields(node: WidgetNode): string {
    const fields: string[] = [];
...
function renderImpl(node: WidgetNode, structName: string): string {
    const methods: string[] = [];

    // Constructor
    const ctorArgs: string[] = [];
    if (node.label !== undefined) { ctorArgs.push('label: &str'); }
    if (node.disabled !== undefined) { ctorArgs.push('disabled: bool'); }

    const ctorBody: string[] = [];
    if (node.label !== undefined) { ctorBody.push(`            label: label.to_string(),`); }
    if (node.placeholder !== undefined) { ctorBody.push(`            placeholder: String::new(),`); }
    if (['input', 'textarea', 'select'].includes(node.kind)) {
    ...
    if (node.disabled !== undefined) { ctorBody.push(`            disabled,`); }
    if (node.kind === 'button') { ctorBody.push(`            variant: WidgetVariant::Primary,`); }

    methods.push(
        `    pub fn new(${ctorArgs.join(', ')}) -> Self {\n        Self {\n${ctorBody.join('\n')}\n        }\n    }`
    );
    ...
    for (const ev of (node.events ?? [])) {
        ...
        methods.push(
            `    pub fn ${fnName}${genericPart}(${paramType}) {\n        ${dispatchExpr}\n    }`
        );
    }
...
export function generateTitan(ir: WidgetIR): string {
