// TypeScript Generator — converts WidgetIR to strongly-typed TypeScript widget factories
import { WidgetIR, WidgetNode, WidgetKind, WidgetEvent, WidgetProp } from '../WidgetIR';

function kindToOwClass(kind: WidgetKind): string {
...
function kindToTag(kind: WidgetKind): string {
    const map: Partial<Record<WidgetKind, string>> = {
...
function renderInterface(node: WidgetNode, name: string): string {
    const lines: string[] = [];

    if (node.label !== undefined)      { lines.push(`  label?: string;`); }
    if (node.placeholder !== undefined){ lines.push(`  placeholder?: string;`); }
...
function renderCreateFunction(node: WidgetNode, name: string): string {
    ...
    if (node.label !== undefined)      { destructured.push(`label = '${node.label ?? 'Label'}'`); }
    if (node.placeholder !== undefined){ destructured.push(`placeholder = '${node.placeholder}'`); }
    if (['input', 'textarea', 'select'].includes(node.kind)) {
        destructured.push(`value = ''`, `defaultValue`, `onChange`);
    }
    if (node.kind === 'button')   { destructured.push(`variant = '${node.variant ?? 'primary'}'`, `size = '${node.size ?? 'md'}'`, `onClick`, `type: btnType = 'button'`); }
    if (node.kind === 'toggle' || node.kind === 'checkbox') { destructured.push(`checked = false`, `onChange`); }
    if (node.kind === 'slider')   { destructured.push(`min = 0`, `max = 100`, `step = 1`, `value: sliderValue = 0`, `onChange`); }
    if (node.kind === 'select')   { destructured.push(`options = []`); }
    if (node.kind === 'progress') { destructured.push(`value: progressValue = 0`, `max = 100`); }
    if (node.disabled !== undefined) { destructured.push(`disabled = false`); }
    ...
    if (node.kind === 'button') {
        ...
        if (node.disabled !== undefined) { lines.push(`  el.disabled = disabled ?? false;`); }
        lines.push(`  if (id) el.id = id;`);
        ...
        lines.push(`  el.className = \`${owClass}\${className ? ' ' + className : ''}\`;`);
        if (node.label !== undefined) { lines.push(`  if (label) el.textContent = label;`); }
        if (node.disabled !== undefined) { lines.push(`  if (disabled && 'disabled' in el) (el as HTMLButtonElement).disabled = true;`); }
        lines.push(`  if (id) el.id = id;`);
    }
...
export function generateTs(ir: WidgetIR): string {
...
// ── Usage ──────────────────────────────────────────────────────────────────────
// const widget = create${capitalName}({${
    node.label !== undefined ? `\n//   label: '${node.label ?? name}',` : ''
}${node.events?.find(e => e.name === 'onClick') ? `\n//   onClick: (e) => console.log('clicked', e),` : ''
}${node.events?.find(e => e.name === 'onChange') ? `\n//   onChange: (v) => console.log('changed:', v),` : ''}
// });
