// Tauri Generator — converts WidgetIR to Tauri-compatible HTML+JS+Rust invoke patterns
...
function renderHtmlWidget(node: WidgetNode): string {
...
function renderTauriJs(node: WidgetNode): string {
...
function renderRustCommand(node: WidgetNode): string {
    const id = node.id ?? node.name?.toLowerCase() ?? 'widget';
...
export function generateTauri(ir: WidgetIR): string {
