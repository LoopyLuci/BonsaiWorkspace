// JavaScript Generator — converts WidgetIR to vanilla JS widget factory functions
...
function renderJsWidget(node: WidgetNode): string {
    ...
    if (node.kind === 'input' || node.kind === 'textarea') {
        ...
        if (node.label !== undefined) { bodyLines.push(`  el.textContent = label;`); }
...
function renderOWRequirement(): string {
...
export function generateJs(ir: WidgetIR): string {
...
// ── Usage example ──────────────────────────────────────────────────────────────
// const widget = create${name.charAt(0).toUpperCase() + name.slice(1)}({${
