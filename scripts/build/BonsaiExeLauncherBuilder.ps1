...
try {
  ...
  # ── Build the Survival Watchdog ───────────────────────────────────────────
  $watchdogManifest = Join-Path $WorkspaceRoot "crates\bonsai-watchdog\Cargo.toml"
  if (Test-Path $watchdogManifest) {
    Write-Info "Building bonsai-watchdog (standalone, release)"
    & cargo build --release --manifest-path $watchdogManifest
    if ($LASTEXITCODE -eq 0) {
      # Copy to workspace root for easy discovery by the launch script
      $watchdogBuilt = Join-Path $WorkspaceRoot "crates\bonsai-watchdog\target\release\bonsai-watchdog.exe"
      if (Test-Path $watchdogBuilt) {
        Copy-Item -Path $watchdogBuilt -Destination (Join-Path $WorkspaceRoot "target\release\bonsai-watchdog.exe") -Force -ErrorAction SilentlyContinue
      }
      Write-Success "bonsai-watchdog built successfully"
    } else {
      Write-Host "[builder] WARNING: bonsai-watchdog build failed (non-fatal — app still works without watchdog)" -ForegroundColor Yellow
    }
  } else {
    Write-Info "bonsai-watchdog manifest not found — skipping"
  }
  # ─────────────────────────────────────────────────────────────────────────
