<script lang="ts">
  ...
  async function loadScripts() {
    try {
      const list = await invoke<string[]>('sylva_list_scripts');
      scripts = list.map(s => typeof s === 'string' ? s : s.name);
    } catch (e) {
      console.warn('sylva_list_scripts failed', e);
      scripts = [];
    }
  }
  ...
  async function loadScript(name: string) {
    try {
      const src = await invoke<string>('sylva_get_script_content', { name });
      code = src;
      selectedScript = name;
    } catch (e) {
      code = `-- failed to load script: ${String(e)}`;
      selectedScript = name;
    }
  }
  ...
  async function runScript() {
    output = '';
    try {
      const res = await invoke<any>('sylva_exec_file', { path: `${selectedScript}.lua` });
  ...
  async function saveScript() {
    ...
    try {
      await invoke<string>('sylva_save_script', { name: selectedScript, source: code });
      await loadScripts();
    } catch (e) {
      output = `Save error: ${String(e)}`;
    }
  }

  async function loadHistory() {
    try {
      history = await invoke<any[]>('get_sylva_history');
    } catch (e) {
      history = [];
    }
  }
...
<div class="p-4">
  <h3 class="text-lg font-semibold mb-2">Sylva Script Editor</h3>
  <div class="flex gap-4">
    <div class="w-1/4">
      <div class="mb-2 font-medium">Scripts</div>
      <ul class="space-y-1 text-sm">
        {#each scripts as s}
          <li>
            <button class="text-left w-full text-blue-500 hover:underline" on:click={() => loadScript(s)}>{s}</button>
          </li>
        {/each}
      </ul>
      <div class="mt-2">
        <button class="px-3 py-1 bg-gray-800 text-white rounded text-sm" on:click={loadScripts}>Refresh</button>
      </div>
    </div>
    <div class="flex-1">
      <div class="mb-2 font-medium">Editor {selectedScript ? `- ${selectedScript}` : ''}</div>
      <textarea bind:value={code} rows="12" class="w-full font-mono text-sm p-2 bg-gray-900 text-white rounded"></textarea>
      <div class="mt-2 flex gap-2">
        <button class="px-3 py-1 bg-green-600 text-white rounded text-sm" on:click={runScript}>Run</button>
        <button class="px-3 py-1 bg-blue-600 text-white rounded text-sm" on:click={saveScript}>Save</button>
      </div>
      <div class="mt-3">
        <div class="font-medium mb-1">Output</div>
        <pre class="bg-black text-gray-200 p-2 rounded text-xs max-h-52 overflow-auto">{output}</pre>
      </div>
    </div>
    <div class="w-1/4">
      <div class="font-medium">Call History</div>
      <div class="mt-1 text-xs max-h-64 overflow-auto bg-gray-800 p-2 rounded">
        {#each history as h}
          <div class="border-b border-gray-700 py-1">
            <div class="text-xs text-gray-400">{new Date(h.timestamp).toLocaleString()} — {h.script_name}</div>
            <div class="text-sm text-gray-200 truncate">{JSON.stringify(h.result)}</div>
          </div>
        {/each}
      </div>
    </div>
...
-- Define a run(args) function to expose this as a tool.
...
`;
  ...
  onMount(async () => {
    ...
    // Hot-reload event from watcher
    const unlisten = await listen('sylva-reload', async (event) => {
      await loadScripts();
    });
    return unlisten;
  });
  ...
  async function loadScripts() {
    try {
      ...
      console.error('sylva_list_scripts failed:', e);
    }
  }
  ...
  async function loadHistory() {
    try {
      ...
      console.error('get_sylva_history failed:', e);
    }
  }
  ...
  async function saveScript() {
    ...
    try {
      ...
      await loadScripts();
      output = `✓ Saved and reloaded '${selectedName}'`;
      outputError = false;
    } catch (e) {
      output = `Save failed: ${e}`;
      outputError = true;
    } finally {
      saving = false;
    }
  ...
  function resultOk(r: SylvaCallRecord): boolean {
    return 'Ok' in (r.result as object);
  }
...
<div class="se-root" role="dialog" aria-modal="true" aria-label="Sylva Script Editor">
    ...
    <aside class="se-sidebar">
      ...
      <ul class="se-script-list">
        {#each scripts as script}
          <li
            class="se-script-item"
            class:selected={script.name === selectedName}
          ...
          >
            ...
            {#if script.error}
              <span class="se-error-dot" title={script.error}>⚠</span>
            {/if}
          </li>
        {/each}
        {#if scripts.length === 0}
    ...
    <div class="se-main">
      <div class="se-editor-bar">
        ...
        <div class="se-editor-actions">
          <span class="se-hint">Ctrl+Enter to run · Ctrl+S to save</span>
          {#if selectedName}
      ...
      <div class="se-output-bar">
        ...
        {#if output}
          <button class="se-btn se-btn-ghost se-btn-xs" on:click={() => { output = ''; }}>Clear</button>
        {/if}
      </div>
  ...
  <div class="se-history-bar">
    <button class="se-btn se-btn-ghost se-btn-sm" on:click={() => { showHistory = !showHistory; loadHistory(); }}>
...
<style>
  ...
  .se-sidebar-label { font-size: 11px; text-transform: uppercase; color: #888; letter-spacing: 0.06em; }
  .se-new-form {
    display: flex;
    gap: 4px;
    padding: 6px 8px;
  ...
  .se-output-label { font-size: 10px; text-transform: uppercase; color: #888; letter-spacing: 0.06em; }
  ...
  .se-history-row.se-history-error { background: #1a0a0a; }
  ...
  .se-history-status { min-width: 16px; }
